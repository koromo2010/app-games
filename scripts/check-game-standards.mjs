...
const sharedStoreRuntime = read("lib/online-room-store-runtime.ts");
...
for (const game of games) {
  ...
  if (game.playMode === "online-room") {
    ...
    if (!game.roomStoreFile || !existsSync(join(root, game.roomStoreFile))) fail(`${game.id}: roomStoreFile がありません。`);
      ...
      const store = read(game.roomStoreFile);
      const modules = [entry, store, ...(game.moduleBoundaryFiles || []).map(read)].join("\n");
      const usesStoreRuntime = store.includes("online-room-store-runtime");
      if (!store.includes("createPlatformOnlineRoomStoreRuntime")) {
        fail(`${game.id}: 共通Online Room Store Runtimeを使用していません。`);
      }
      const usesRoomTtl = store.includes("multiplayerRoomTtlSeconds")
        || store.includes("multiplayerRoomExpiryArgs")
        || (store.includes("online-room-persistence") && sharedPersistenceModule.includes("multiplayerRoomExpiryArgs"))
        || (usesStoreRuntime && sharedStoreRuntime.includes("isMultiplayerRoomExpired"));
      if (!usesRoomTtl) fail(`${game.id}: 共通の部屋TTLを使用していません。`);
      if (!store.includes("revision") && !store.includes("saveStoredWordWolfRoom") && !usesStoreRuntime) {
        fail(`${game.id}: サーバー側の部屋保存処理が見つかりません。`);
      }
      if (!modules.includes("abort-game") && !modules.includes("abortGame")) fail(`${game.id}: ゲーム開始前へ戻すデバッグ中断処理がありません。`);
      if (!modules.includes("confirm-lobby-return") || !modules.includes("remove-waiting-player") || !modules.includes("allRoomPlayersReturned")) fail(`${game.id}: 共通のロビー復帰確認・待機者管理を使用していません。`);
      const usesDissolutionPolicy = store.includes("canDissolveOnlineRoom")
        || (store.includes("online-room-dissolution") && sharedDissolutionModule.includes("canDissolveOnlineRoom"))
        || (usesStoreRuntime
          && sharedStoreRuntime.includes("online-room-dissolution")
          && sharedDissolutionModule.includes("canDissolveOnlineRoom"));
      if (!usesDissolutionPolicy) fail(`${game.id}: 進行中の部屋解散を防ぐ共通ポリシーがありません。`);
    }
    ...
    if (!registeredModuleSources.includes("OnlineRoomLifecycleActions")) fail(`${game.id}: ロビー・プレイ中・結果の部屋操作を切り替える共通UIがありません。`);

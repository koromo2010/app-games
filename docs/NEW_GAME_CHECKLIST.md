...
3. LLM利用、公開範囲、アカウント戦績の有無を実態どおりに宣言する。ゲームの世界観と遊び方が情景として伝わる、文字なしの横長キービジュアルを `public/game-visuals/<game-id>.webp` に追加する。推奨サイズは1200×500pxとし、ロビーカードでは枠全面へ `object-cover` で表示する。小さい表示でも主題が残る中央構図と十分なコントラストを確保し、高解像度の生成元ではなく最適化後のWebPだけを配信対象にする。タイトルやロゴは画像へ焼き込まず、HTMLのテキストとして画像から分離する。

...
MCP `initialize`、OAuth認証、Game Fields SDK handshakeは別の責務である。DownloadMeから始めるAIは最初に`get_sdk_handshake`へ環境、Platform版、SDK package版、contract schema、必須capabilityを提示し、`accepted=true`を確認してから制作者環境やゲーム仕様へ進む。`sdk`と`sdk-dev`は同じhandshake schemaを使い、環境とcanonical endpointだけを応答で区別する。

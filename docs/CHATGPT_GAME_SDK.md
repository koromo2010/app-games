...
公開可能なSDK契約は`@game-fields/game-sdk`の次の4 exportに限定する。
...
ソースは`packages/game-sdk/src`へ物理分離し、独立した`package.json`、SemVer、`exports`、TypeScript buildを持つ。現在のpreview版は`0.1.0`で、誤公開を防ぐため`private: true`かつ`UNLICENSED`としている。`npm run test:sdk-package`はtarballを一時外部projectへinstallし、4 exportをpackage名だけで利用できることを検査する。

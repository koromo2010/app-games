...
各ゲームはmanifestを宣言し、AIと自動監査が機能要件を確認できるようにする。
...
完全な初心者が一度の指示だけで安全なオンラインゲームを完成させる段階にはまだ達していない。まずはローカルゲームまたは単純なオンライン部屋ゲームでSDKを検証し、共通Runtimeを増やしていく。

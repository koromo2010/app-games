...
ただし公開packageはまだnpm registryへpublishしておらず、`private: true`と`UNLICENSED`を維持している。隔離preview用の専用非公開Git、読取／書込を分けた資格、Vercel Project、`preview-dev.game-fields.com`は外部設定前である。入口と公開スターターGitはまず運営者本人のPro版ChatGPT実機検証に使い、ゲーム作成、共有mock確認、提出ZIP返却、dev統合を確認した後に無料版の能力・利用枠を検証する。内部Runtime coreのstorage-neutralなRoom更新lifecycleと、本体8オンラインゲームへのRedis／active-room／解散／戦績・リプレイ注入は実装済みである。外部ゲーム向けの汎用HTTP route・Client Runtime、WebSocket、Developer Portalの正式チュートリアル・APIリファレンス・提出画面は未実装である。platform adapterとRedisキー実装はGame Fields内部専用で、公開SDKやPortalへ含めない。

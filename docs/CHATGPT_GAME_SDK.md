...
試用者は`GameFieldsDownloadMe-ver8.md`をChatGPTへ渡し、そこから`downloadMeVersion: 8`の公開スターターを取得する。最初に仕様を相談して`GAME_SPEC.md`を確定したあと、同じフォルダ内だけを実装させる。`npm run test:sdk-starter`は別ディレクトリへの展開、同梱SDK install、型検査、契約テスト、デモ完走までを検査する。`gameapp-dev`のほかのtoolは見えるのに`get_sdk_handshake`だけがない場合は、更新前のプラグインと判定し、更新後に新しいチャットで選択し直して同じDownloadMeを再添付するよう案内して停止する。

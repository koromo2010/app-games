...
ただしpackageはまだnpm registryへpublishしておらず、`private: true`と`UNLICENSED`を維持している。Game Fields本体のCookie認証・Redis CAS・WebSocket・戦績・リプレイへSDK moduleを自動接続するplatform adapter、Developer Portalのチュートリアル・APIリファレンス・提出画面も未実装である。したがって現段階は、ゲーム固有domainと閲覧者別presentationを本番資源なしで安全に開発し、配布物と同じpackage境界で契約テストできるところまでである。

...
SDK formal packageの共通共有はmodule profileの`result-share`が`required`のときだけ表示し、順位表示を`PLAYER1`形式へ匿名化する。packageが返す表示名、秘密state、認証付きfeedback／replay URLは共有文へ入れない。

...
- ことばソナー: テーマ、秘密語、探知文字、脱落状況、最後の生存者、同時脱落時の最短語判定
- ノーザンブランチ: 最終得点、建物数、ターン数、ゲーム内行動ログ
- ワードアウト（内部ID: nigoichi）: 言葉一覧、各プレイヤーの手札、グループ、連想語、余り予想、正誤
...
共有文へ説明本文、秘密語、お題の答え、参加者名、個別の投票先、認証付きプレイバックURLは原則含めない。ゲーム固有仕様で結果情報の共有を明示した場合だけ、結果画面で公開済みの内容をプレビュー付きで含められる。ワードアウトは言葉一覧、持ち主、手札、グループ、連想語を含めるが、参加者名は入室時の共有同意がONのときだけ表示し、それ以外は `PLAYER1` 形式で匿名化する。合言葉や認証付きURLは常に含めない。詳細本文は参加者本人がマイページで開くプレイバックだけに返す。

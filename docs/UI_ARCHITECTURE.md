...
- ホスト向け `DebugModeButton` は `GameTopBanner` に直接置き、DEBUGボタンから開くポップアップをデバッグ操作の唯一の入口とする。
- ダミーの追加・削除、代理操作対象の切替、フェーズ固有の一括入力、進行補助、プレイバック、中断、行動ログはポップアップ内に置く。通常のLayoutやフェーズ部品にはデバッグ状態の説明だけを残せる。

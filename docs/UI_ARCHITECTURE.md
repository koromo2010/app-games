...
- 最小化中も操作を残す必要がある機能は、`DebugToolWindow`の固定領域へ明示的に渡す。SDK-devの閲覧視点と操作対象はこの領域へ別々のボタン群として表示し、プルダウンへ戻さない。操作対象でダミーを選ぶと初期表示は同じダミー視点へ切り替えるが、その後も閲覧視点は独立して変更できる。
- ダミーの追加・削除、代理操作対象の切替、フェーズ固有の一括入力、進行補助、プレイバック、中断、行動ログは画面内ウィンドウへ置く。SDK正式Packageの代理操作はplaying中のゲーム固有Commandだけを対象とし、`room/*`共通Commandは常に署名済み本人として処理する。通常のLayoutやフェーズ部品にはデバッグ状態の説明だけを残せる。

...
db/vocabulary/010_tahoiya_zipf_zero.sql
db/vocabulary/011_tahoiya_difficulty_zipf_ranges.sql
```
...
管理画面から一つ目の単語をたほい屋候補へ送ると、実効Zipfが3以上または未計測の場合は秘境候補の2.9へ設定し、すでに3未満なら現在値を維持して、
`word_game_eligibility(game_id = 'tahoiya')`を有効にする。同時に、その評価を

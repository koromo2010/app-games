...
監査時の事象: たほい屋、ことばで数ならべ、ことば潜伏戦は、GET時のreconcileと操作後のreconcileが同じ締切を別経路で判定していた。締切後でも先に操作PATCHが届けば内容を取り込んでから遷移し、先にGETが届けば時間切れ遷移するため、結果がリクエスト順に依存していた。

影響: 同じ送信タイミングでも、ポーリングとの競合によって受付可否が変わる。

対応: `GAME_TIMEOUT_GRACE_MS`（標準5000ms）を共通化し、Tahoiya・ことばで数ならべ・ことば潜伏戦は期限後の操作を適用する前に同じreconcileを実行する。締切＋猶予の境界は `tests/game-timer-policy.test.ts` で固定した。

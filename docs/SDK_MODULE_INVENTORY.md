...
| --- | --- | --- | --- |
| `content-source` | catalog上の名前と固定3語Previewだけ。pool・型・取得APIが外部から不明 | `@game-fields/game-sdk/content-source`にword・pair・definitionと取得requestを公開。`@game-fields/game-sdk/resources`から注入adapterを取得 | `lib/game-sdk-content-source.ts`が一般語・難読語・ワードペア・語釈を読取専用で束ね、認証付き暗号化opaque IDへ変換して審査登録済みゲームへ注入。DB接続・既出履歴・抽選権限はPlatform内に保持 |
| `llm` | 共通gatewayは本体だけ。外部package用の注入型なし | `@game-fields/game-sdk/llm`にrequest・response・生成metaを公開。provider/APIキーは非公開 | `lib/game-llm.ts`がprovider選択、課金元、fallbackを所有 |

...
| --- | --- | --- | --- |
| `content-source` | catalog上の名前と固定3語Previewだけ。pool・型・取得APIが外部から不明 | `@game-fields/game-sdk/content-source`にword・pair・definitionと取得requestを公開。`@game-fields/game-sdk/resources`から注入adapterを取得 | DB接続・既出履歴・抽選権限はPlatform内に保持 |

...
| --- | --- | --- | --- |
| `content-source` | catalog上の名前と固定3語Previewだけ。pool・型・取得APIが外部から不明 | `@game-fields/game-sdk/content-source`にword・pair・definition、`easy | normal | hard`、取得request、返却fieldを公開。AppSetは`@game-fields/game-sdk/resources`、モックは`GameFieldsPreset.resources.contentSource`を利用 | `lib/game-sdk-content-source.ts`が一般語・難読語・ワードペア・語釈を読取専用で束ね、認証付き暗号化opaque IDへ変換。審査済みゲームへ直接注入し、未審査Previewは外側Shellの認証・profile・レート制限付きpostMessage bridgeから同じ3 APIだけを中継。DB接続・既出履歴・抽選権限はPlatform内に保持 |

...
承認済みSDKゲームも`/api/game-sdk/[gameId]/rooms`と`/sdk-games/[gameId]`から同じ非公開Runtimeへ接続する。AppSetの`standardResult`だけを共通結果・戦績・rating・安全なplaybackへ保存し、ゲーム固有の秘密stateは保存hookへ渡さない。期限切れはserver callback、観戦は署名grantとSDK専用の許可リストsnapshot、DEBUGダミーは権限付きhostのlobby Command、個人既定値とルール表示はmanifest宣言を正本とする。formal packageの外側は共通`GameSdkShellHeader`を必ず使い、同部品が`GameTopBanner`、ラウンジの直接戻り導線、Room内メニュー、ルール、プレイヤーメニューを所有する。Preview・採用済みiframe package・旧wordwolf clientで個別ヘッダーやラウンジ戻りリンクを複製しない。ルールは全phaseから開ける。部屋設定はロビーだけ、playingではゲームとtimer、resultでは標準結果・再戦・履歴・匿名共有・AI生成物feedbackをmodule profileに従って表示する。
...
AI、スターター、browser RuntimeはSDKの操作前にGame Fields固有handshakeを行う。MCP transportの`initialize`やOAuth成功だけを互換性確認に使わず、接続先環境、Platform／package release、SDK contract schema、必須capabilityを`get_sdk_handshake`または`/.well-known/game-fields-sdk`で合意する。handshakeはsessionやactorを発行せず、後続Routeの認証・認可を省略しない。

...
- 改善要望・バグ報告はRedisの`user-reports:v1`を一覧索引、`user-report:v1:<report-id>`を本文とする。本文は最終更新から180日保持し、索引上限1,000件から外れた本文も同時に削除する。旧形式の報告は`open`として読み込む。本文keyは有界`SCAN`でも列挙し、索引から欠落したparse可能な本文を一覧から消さない。詳細は[`USER_REPORT_STORAGE_RESILIENCE.md`](./USER_REPORT_STORAGE_RESILIENCE.md)を正本とする。
- 報告一覧はfull管理者セッションだけに返し、`updatedAt`降順で表示する。完全なreport IDによるread-only直接検索と、本文／索引／TTL／parseの安全なwarningを提供する。対応状態の変更には直近5分以内のMFA再確認を必須とし、本文を複製せず状態変更だけを監査ログへ記録する。

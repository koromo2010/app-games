...
公開package名は`@game-fields/game-sdk`、preview versionは`0.1.0`である。独立TypeScript build、SemVer、用途別の公開`exports`、公開ファイル限定、tarballの外部install・実行検査を持つ。初回公開承認に基づき`private`を解除し、MIT License、`publishConfig.access=public`、provenanceを固定した。npm registryへの実publishには`@game-fields` scope所有権と限定publish資格が必要である。
...
初心者向け制作導線は、作りたいアプリの聞き取り、`GAME_SPEC.md`の確定、`APP_REQUIREMENTS.md`への照合、静的画面モック作成、モックの説明と利用者確認、承認後のSDK契約実装の順とする。モックは`mock/`へprovider APIや外部URLを直接呼ばないHTML/CSS/JavaScriptとして作り、`MOCK_REVIEW.md`へ画面、操作、共通要件への対応、本実装まで動かない部分を記録する。`llm`必須Previewに限り公式`GameFieldsPreset.resources.llm` bridgeで実接続を確認できる。`npm run check:mock`は仕様と確認記録の未記入、モック必須ファイル、基本的なレスポンシブ設定を検査する。具体的なゲーム例を入口指示へ置かず、添付資料を今回のゲーム仕様として自動採用しない。

隔離Previewは、取得したHTMLへ公式`GameFieldsPreset` browser runtimeを自動注入する。参加者一覧、ダミー追加・削除、デバッグ表示、閲覧視点、フェーズ、開始、中断、再戦、自動進行の共通状態とCommandはPreviewが所有する。旧スターターの`data-action`属性も互換入力として扱い、静的な通知だけの飾りボタンへ戻さない。ゲーム固有JavaScriptは`registerGame`で石、カード、盤面等の固有状態の開始・初期化・自動進行だけを接続する。`llm`必須ゲームには`resources.llm.generate`を追加し、iframeからのtask、送信内容、prompt version、任意の返却schemaを外側Shellがログイン・module profile・レート制限付き本体APIへ中継する。provider APIキー、接続先、model、課金元、fallbackはiframeへ渡さない。これは未審査Preview用adapterであり、本体統合時の認証、最終認可、永続化は引き続きplatform Runtimeが担当する。

SDK-devの確認画面は、制作者の広場からゲーム固有iframeへ直接遷移させない。共通の部屋作成・参加、参加者と設定を確認するRoomロビー、開始条件、DEBUG、同期revision、時間管理、結果、再戦、解散、戦績等の結果投影を外側へ合成し、ゲーム固有iframeはその内側のslotに限定する。設定欄はゲームpackageが`settings`へ宣言した項目だけを描画し、最大人数・ラウンド数等を固定表示しない。`online-room`では制限時間1項目だけを必須とし、初期値と選択肢もpackage宣言を正本にする。選択値はRoom設定とpreset Runtimeへ同期し、iframe内に同じ設定UIを複製しない。ロビーからプレイ中へ移る際は同じiframeを保持し、ゲーム固有状態を画面Shellの都合で初期化しない。ゲームadapterの登録完了を外側Shellが確認できない間は開始を拒否し、共通module接続件数だけを根拠に正常扱いしない。iframeはpreset Runtimeが通知するゲーム固有slotの実高へ追従し、固定高の内部スクロールをページスクロールと二重化しない。プレイ中は外側Shellを最大1600pxまで使い、ゲーム固有領域を可変幅、Room情報を280pxとする。timerの締切・受付・手番完了後のリセットは共通moduleが所有し、ゲーム固有クライアントは表示位置と見た目だけを選ぶ。Previewでは成功した手だけ`timer:turn-complete`を通知し、本体AppSetでは成功transitionの`timer: "reset"`からサーバー時刻で次の締切を生成する。DEBUGはダミー、閲覧視点、自動進行、中断を提供するが、`lobby / playing / result`へ無条件遷移するショートカットは置かず、公式Lifecycle導線で各面を確認する。

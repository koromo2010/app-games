...
公開package名は`@game-fields/game-sdk`で、npm安定版は`0.1.0`、リポジトリの次期公開候補は`0.2.0`である。独立TypeScript build、SemVer、用途別の公開`exports`、公開ファイル限定、tarballの外部install・実行検査を持つ。MIT License、`publishConfig.access=public`、provenanceを固定する。
...
運営者本人が外部利用者と同じ流れを試す正式入口は、ChatGPT Work用`sdk/entry/START_GAME_FIELDS.md`とClaude Code用`sdk/entry/START_CLAUDE_CODE.md`である。正本`sdk/starter-template`、ZIP生成`scripts/build-game-sdk-starter.mjs`、公開Git用snapshot生成`scripts/build-game-sdk-starter-repository.mjs`を実装済みである。安定版は公開`koromo2010/app-games`の`sdk-starter`、development候補は`sdk-starter-dev`だけを浅く取得させ、通常チャットやその他のsurfaceを正式制作clientとして案内しない。版は`config/platform-release.json`のPlatform SemVer、環境別のStarter ref・プラグイン名・Portal URL・DownloadMe名は`config/sdk-release-profiles.json`を正本としてDownloadMeとmanifestへ同時反映する。productionは`game-fields`／`GameFieldsDownloadMe-ver<SemVer>.md`／`sdk-starter`、developmentは`dev-game-fields`／`GameFieldsDownloadMe-dev-ver<SemVer>.md`／`sdk-starter-dev`を使う。スターターにはSDK tarball、ChatGPT用指示、仕様書、APIリファレンス、型付きゲームmodule、契約テスト、完走デモ、提出ZIP生成器を含む。`npm run test:sdk-starter`で入口、公開Git用snapshotとZIPの同一性、SDK install、型検査、契約テスト、完走、提出ZIPまで検査する。Portalの一般向けDownloadMe導線は実装済みで、正式な公開ライセンスは未実装である。

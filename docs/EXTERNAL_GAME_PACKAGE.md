...
SDK専用Vercel Project `app-games-sdk`と初回Deploymentは作成済みで、暫定URL `https://app-games-sdk.vercel.app` はHTTP 200を返す。ただし初回はソースファイル直接送信であり、同一リポジトリへのGit接続、Root Directory、Production Branch、Preview運用、`sdk.game-fields.com`への移管は未完了である。本体のオンラインRoomへ接続するplatform adapter、外部配布用package、インストール手順、APIリファレンス、提出画面も次段階であり、現時点では実装済みと扱わない。

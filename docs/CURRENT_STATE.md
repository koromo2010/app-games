...
SDK制作者はPortalの`/support`またはOAuth MCPのsupport toolsから、本人の報告だけを一覧・閲覧・追記できる。AIによる新規報告は`prepare_support_report`で7日間の下書きだけを作成し、制作者本人がPortalの承認画面で内容を確認・修正して送信した場合だけ運営受信箱へ入る。AIだけで正式送信するtoolは提供しない。

報告への運営返信は、確認済みの復旧用メールがある送信者へ通知する。メールは会話の正本にせず、返信本文、該当スレッドを開くSDK Portal導線、別のGPTチャットで`get_support_thread`から経緯を引き継ぐための貼り付け文を載せる。貼り付け文は変更・返信前の人間確認も指示する。メール未登録・未確認・配送失敗でもPortalの会話履歴は保持し、管理画面に配送状態を表示する。

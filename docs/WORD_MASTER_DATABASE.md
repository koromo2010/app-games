...
`surface-quality-v2` は、難語・古語を残したまま、単独のゲーム用ワードとして不適切な表記を理由付きで分離する。判定結果は `surface_quality_status`、複数の理由は `surface_quality_flags`、ルール版は `surface_quality_policy_version` に保存する。元の単語は削除せず、既存のゲーム設定も変更しない。
...
ワードウルフは、ペア全件を辞書だけで固定生成する方式ではなく、共通ワードDBから起点語を選び、その起点語に対する相方をLLMで審査・生成して蓄積する方式を正とする。起点語は永久 `word_master_id` を持ち、相方が共通DBに存在する場合だけ相方IDも付ける。未登録の相方は文字列として保持し、ペアは順不同で利用時に村側・狼側をランダムに決める。

1回の生成では難易度の中心Zipf付近から起点語を3件抽出し、1回のLLMリクエストで各語を独立に処理する。LLMは安全性、不完全語、日常での使われにくさ、ワードウルフでの話しにくさを判定し、採用可能な語だけに相方を返す。採用候補は元Zipf、利用頻度補正、ゲーム固有補正、Good/Bad由来の知名度補正を合成した実質Zipfで重み付けする。判定とペアは再利用できる形で蓄積し、利用者評価によって後から順位を調整する。元のZipf値は書き換えない。

デバッグ画面は、最終ペアだけでなく、候補3語、補正値、採否、抽選重み、AI事業者の切替、DB保存結果を確認するためのものとする。診断内容はデバッグ中のホストだけへ返し、通常の部屋データやサーバーログには秘密語として残さない。

## 開発DBと本番公開の方針

プレビュー環境を本番DBへ直接つないでRAGを開発しない。先に本番 `shared_word_catalog` と必要な補助表だけを複製した開発用DBを作り、Vercel PreviewのDB接続先をそのコピーへ限定する。辞書原本、生成CSV、ローカルDB全体、利用者の認証情報やプレイ履歴はコピー対象にしない。

開発用コピーには、最低限 `shared_word_catalog`、`shared_word_game_evaluations`、`shared_wordwolf_pairs` のスキーマと、RAGの候補抽出を再現できるデータを含める。コピー後は件数、ID不変性、active件数、難易度別の候補取得、3語バッチ、判定・ペア保存を検証する。本番へ進める条件は、プレビューの診断画面で `DB候補 3語` と `共通ワードDBから3候補 → LLM一括審査` を確認でき、lint・test・buildを通し、最新mainとの競合を解消できていることとする。

2026-07-16時点では、Vercel Previewで診断画面の表示までは確認できたが、DB候補は0語で直接LLM生成へフォールバックした。最新mainとの統合にも競合があるため、本番mainへの公開は保留し、次の作業は開発用DBコピーの作成から再開する。

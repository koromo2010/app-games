...
| `SHARED_RESEND_API_KEY` | `RESEND_API_KEY` | Yes | Link済み | Link済み | Link予定 | Resendメール送信（復旧用メール確認・パスワード再設定・運用通知） | 両Project Link済み・コード対応済み |
...
| `SHARED_RESEND_API_KEY`（旧 `RESEND_API_KEY`） | Shared | Yes | 復旧用メールの所有確認、パスワード再設定、運用通知に使用。SDK一般公開時は共通キーを直接リンクしない |

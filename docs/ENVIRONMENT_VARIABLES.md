...
| `SDK_PORTAL_INTERNAL_URL` | Production | No | 2026-07-27、`app-games-dev`のProduction Variableへ`https://sdk-dev.game-fields.com`を登録済み（Vercel画面で「Added just now」を確認）。登録直後にVercelが自動でDeployment `dpl_7FdpHdwicpiNkBKTmkocPpTRHZah`（commit `bbb7efd`、develop）を作成し、`READY`・`dev.game-fields.com`へのalias反映を確認済み。moi-devの資格情報を運営・クライアントいずれも保有しておらず、ブラウザでのcatalog表示・SSO到達・非所有者非表示の実機確認はできていない。代わりに`lib/sdk-dashboard-navigation.ts`と`tests/sdk-dashboard-navigation.test.ts`をコードレビューし、所有者判定がクライアント自己申告ではなく`app/api/sdk-preview/[creatorSlug]/owner`経由でSDK Portal側DBへ照会するサーバー側チェックであること、`isLoggedIn && isCreatorOwner`が両方真の場合だけリンクを返す設計であることを確認済み | 実機確認（moi-devログイン時のリンク表示・クリック・SSO・`/dashboard`到達、非所有者アカウントでの非表示）は本監査スレッドでは実施しない。実機検証は別スレッド（検証専用）で行い、完了後に本欄と状態を更新する |

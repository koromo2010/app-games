...
SDK `content-source`の実DB adapterとmodule lab用sample APIは本体`app-games`／`app-games-dev`で動作し、各本体Projectに既存配置済みの`APP_DATABASE_URL`と共通`VOCABULARY_DATABASE_URL`を使用する。`app-games-sdk*`および`app-games-preview*`へこれらのDB変数を追加・Linkしない。opaque content IDの認証付き暗号化鍵は本体の既存`PLAYER_SESSION_SECRET`から導出し、新しい外部変数は追加しない。

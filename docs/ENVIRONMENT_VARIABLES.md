...
| `SHARED_RESEND_API_KEY` | `RESEND_API_KEY` | Yes | Link済み | Link済み | Link予定 | Resendメール送信（復旧用メール確認・パスワード再設定・運用通知） | 両Project Link済み・コード対応済み。Developmentはキー読込後の実送信を2026-07-23にResendが拒否。認証・送信元ドメイン・送信先制限のどれかは次回の分類ログで要確認 |
...
| `SHARED_RESEND_API_KEY`（旧 `RESEND_API_KEY`） | Shared | Yes | 復旧用メールの所有確認、パスワード再設定、運用通知に使用。SDK一般公開時は共通キーを直接リンクしない。Developmentでは変数が読み込まれている一方、2026-07-23の確認メール再送はResend拒否で502。送信元ドメインまたはAPIキーのResend側状態は未確認 |
...
- Resend Dashboard上の`game-fields.com`送信元ドメイン認証状態と、Shared APIキーの送信権限。Developmentの確認メール実送信は未成功

...
## 2026-07-26 — SDK昇格経路の環境対応

### 判断

- dev管理画面は`SDK-dev → dev`、main管理画面は`SDK → main`だけを実行し、画面から送るtargetと実行環境が一致しない要求は拒否する。
- 本体コードの`develop → main`はSDK作品の採用経路から分離したまま維持する。

### 実施結果

- 管理画面の表示、確認文、targetをdevとmainで切り替えた。
- 本体管理APIとSDK Portal内部APIの両方で、実行branchに対応しない昇格先を拒否するようにした。
- 当時のログ追記で`DEVELOPMENT_THREAD_LOG.md`がバイナリ化していたため、直前の正常なUTF-8版へ復元し、この項目で失われた経緯を補完した。

## 2026-07-26 — SDK本番DB接続とRedis共用の環境分離

### 利用者からの要望

- Upstash無料枠を増やさず、既存の`sdk-dev-redis`をSDK本番と開発で安全に共用する。
- SDK本番準備後にdevelopをmainへ同期し、rollback可能な順序で公開する。

### 判断

- PostgreSQLは本番専用`app-games-sdk-neon`を維持する。
- RedisはSDK Portal間だけで共用し、予約キーを本番`sdk:production:`、開発`sdk:development:`へ分離する。本体ゲーム用Redisとは共有しない。
- VercelのProduction branchが`main`または`develop`以外なら名前空間を解決せずfail-closedにする。
- `main`は`develop`より107コミット先行される一方でmain固有4コミットも持つため、単純なref更新をせず分岐を統合してから本番へ反映する。

### 実施結果

- `app-games-sdk-neon`を`app-games-sdk / Production`だけへLinkし、`SDK_DATABASE_URL`を正しい変数名で登録した。変数追加後の`main@0e7889c` DeploymentはREADY。
- SDK予約キーの環境別prefixと回帰テストを追加した。
- 新規予約は環境別キーだけへ保存しつつ、developは既存7日予約を失わないよう旧キーも期限切れまで読み取る。本番は旧キーを読まない。
- `SDK_REDIS_REST_URL` / `SDK_REDIS_REST_TOKEN`の本番Link依頼を環境変更マスターへ登録した。
- 本番・開発のRedis配置、Neon配置、Production Branchの現在状態を環境変数台帳へ反映した。

### 検証

- 名前空間と旧開発予約互換の単体テスト4件に成功した。
- `npm run lint`に成功し、コード参照61キーと変更依頼11件を含む全静的検査の整合を確認した。
- `npm test`に成功し、全549テストが通過した。
- `npm run build`と`npm run build:sdk`に成功した。ローカル環境ではmigrationのdeploy実行が安全にskipされることも確認した。
...
## 2026-07-26 — 運営受信箱と個人データ削除の横断監査

### 利用者からの要望

- 正式Package Room監査の外にも同系統の漏れがないか確認する。
- 共通メニューから送った不具合報告を、運営がどこで確認できるか明確にする。

### 判断

- Room lifecycleだけでなく、利用者入力の`受付 → 保存 → 運営閲覧 → 対応状態 →
  保持期限 → アカウント削除`を縦断する。
- メール通知は補助経路とし、通知設定なし・送信障害でも保存済み問い合わせを
  運営が管理画面から確認できることを必須とする。
- 自由記述とplayer IDを持つデータは、索引件数だけを切り詰めて本文を残さず、
  保持期限と本人アカウント削除の両方へ接続する。
- 未確認アカウントのCron削除も、本人操作による削除と同じ従属データ処理を通す。

### 実施結果

- `/admin`へ「報告」を追加し、共通メニューの改善要望・バグ報告を一覧・詳細・
  ページ・種別・対応状態付きで確認できるようにした。
- `/admin`へ「お問い合わせ」を追加し、公開フォームの名前・返信先・本文・分類・
  通知メール結果・対応状態を確認できるようにした。メール通知失敗後も保存内容を読める。
- 両受信箱の読取はfull管理者、対応状態変更は直近MFAを必須とし、監査ログへ本文を
  複製せず状態だけを記録する。旧recordは未対応として後方互換読込する。
- 送信者画面へ受付IDを表示する。
- 報告は180日、問い合わせは365日の保持期限を設定し、新着索引1,000件から外れた
  本文キーも同じLua処理で削除する。
- 共通フィードバックもtask索引500件から外れた本文を削除するようにした。
- アカウント削除の従属データへ、報告、共通フィードバック、組み込み／SDK設定既定値、
  ワードウルフ・たほい屋・一般ゲーム語履歴を追加した。
- Room設定既定値は最終保存から2年、ワードウルフのペア履歴はcooldown終了後3日で
  自動失効する。
- 未確認アカウントCronはPostgreSQL一括削除をやめ、対象IDごとに同じ従属データ処理が

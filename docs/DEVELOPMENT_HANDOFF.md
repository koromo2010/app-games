...
- 2026-07-25時点では分類移行コードは`develop`反映済みだが、旧DB読取ロールの`shared_word_pool_evaluations`権限不足により347語のDB同期は未完了である。現在の外部設定と次の操作は`docs/ENVIRONMENT_VARIABLES.md`を正本とする。

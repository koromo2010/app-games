...
- 秘密単語は同じチームだけ、現在暗号はそのチームの出題者だけに返す。敵の桁数選択、敵ヒント、相手の確定回答、秘密単語は公開可能フェーズまで閲覧者別sanitizerで除外する。ラウンド終了後の桁数・暗号・ヒント・傍受結果は過去ログとして常時表示する。
...
通常ゲームでは共通単語DBから実効Zipfが厳密に0の未経験語を抽出し、その候補一覧だけを高品質LLMへ渡す。LLMは候補の表記を変えずに1語を選び、読みと正解文を生成する。可能なら別プロバイダーが語の実在、読み、意味、指定難易度を独立校閲し、検証を通ったお題だけをラウンドへ採用する。生成・校閲に失敗した応答はキャッシュせず、同じラウンドから再試行できる。

正解説明の長さは、約10字、20字、30字、40字、50字、55〜60字の順に `35% / 28% / 20% / 10% / 5% / 2%` で選ぶ。上限は60文字。選ばれた長さは生成と校閲の両プロンプトへ渡すが、文字数合わせの水増しはさせない。

`lib/tahoiya-topic-catalog.ts` は共通DBの実効Zipf=0語を候補とし、誰が見たかはプレイヤー別Redis Setへ保存する。Bad評価で問題視された語と今回の参加者が経験済みの語を除外してからLLMへ渡す。旧Redis Hashと完成済み `active_tahoiya_topics` は移行・管理用資産として残し、管理画面のPreview限定移行からお題と既出履歴を冪等に移す。新しいLLM候補は共通DBのdraftへ入り、管理者採用後だけ完成済みカタログへ昇格する。
...
- 同じレビュー画面から一つ目の単語を既定対象として「たほい屋候補」へ送れる。辞書由来の`words.zipf`は変更せず、全ゲーム共通の`selection_zipf_override`を0へ設定し、通常ゲームの候補から外してたほい屋の抽出対象にする。この操作は同じ評価をワードウルフ候補として自動的に不採用へ最終確定し、紐づく未審査のpair draftも`rejected`へ変更して一覧から除外する。単語自体や他ゲームの適格性は無効化しない。
- 同じartifactへの評価は更新可能

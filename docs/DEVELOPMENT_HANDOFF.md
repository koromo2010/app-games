...
- `/nigoichi` は非公開アクセスキーの利用者だけが開ける、同一端末で順番に画面を渡す調整用モック。
- 3〜6人、各ラウンドで `2N+1` 個の単語を並べ、重複なしの2番号を各人へ配る。秘密入力、一斉公開、余り番号の推理までを確認できる。
- 正式な得点・ペナルティ・手札公開の詳細、オンライン部屋、ログイン、戦績、既出履歴は未実装。仕様決定後に `online-room` 実装へ移行する。

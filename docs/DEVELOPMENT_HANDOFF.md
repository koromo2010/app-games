...
- 広場のゲーム一覧はカード／簡易一覧を切り替え、`localStorage`へ選択を保存する。カード右上／簡易一覧右端の星でゲームをお気に入り登録でき、お気に入りは表示形式や検索結果内でも元の相対順を保って先頭へ並ぶ。お気に入りも端末の`localStorage`へ保存する。簡易一覧も公開状態、タグ、参加中Room、入室／復帰を維持する。
...
| 問い合わせ・改善要望・バグ報告 | `app/admin/AdminSupportInboxPanel.tsx`, `app/components/UserReportButton.tsx`, `app/contact/ContactForm.tsx`, `app/api/user-reports/route.ts`, `app/api/contact/route.ts`, `app/api/admin/user-reports/route.ts`, `app/api/admin/contact-messages/route.ts`, `lib/user-report-admin-notification.ts`, `lib/user-report-store.ts`, `lib/contact-store.ts`, `app/api/internal/sdk-support/route.ts`, `apps/sdk-portal/app/support` |
...
管理画面は改善要望・バグ報告と公開問い合わせを別タブにせず、`AdminSupportInboxPanel`の一つの「問い合わせ・報告」受信箱へ最終更新日時順で混在表示する。報告一覧は索引だけに依存せず、最大1,000件・最大100 pageの有界`SCAN`で本文inventoryを照合し、索引から欠落したparse可能な本文も表示する。本文／索引／TTL／parseの不整合は安全なwarningとして返し、完全なreport IDを指定する管理者向けread-only直接検索を提供する。既存本文への追記、返信、状態・通知更新は本文CASと同じLua transactionで対象IDを索引先頭へ1件だけ戻す。詳細は`docs/USER_REPORT_STORAGE_RESILIENCE.md`を正本とする。初回投稿は「内容」に一度だけ表示し、会話欄は実際の返信・追記がある場合だけ追加メッセージを表示する。状態フィルター、会話、返信、管理者通知の送信状態・安全な失敗理由・再送は共通操作とする。`/api/admin/contact-messages`と`/api/admin/user-reports`は一覧の成功件数または安全な失敗分類を構造化ログへ出す。どちらか一方でも取得または応答検証に失敗した場合、`AdminSupportInboxPanel`は以前の件数と一覧を消して状態フィルターを隠し、古い結果を最新取得済みとして表示しない。新着、追記、受付確認、運営返信のメールは`lib/support-email-content.ts`の共通ID表現を使い、保存済みの`contact_...`または`report_...`を件名とtext／HTML本文冒頭へそのまま載せる。メール専用IDを作らず、同じスレッドの再送・返信も同じrecord IDを維持する。運営返信はfull管理者セッションと監査ログを必須とするが、追加のパスキー再確認は求めない。対応状態変更と管理者通知再送は直近MFAを維持する。返信結果は操作中のフォーム直上へ表示し、通信結果が曖昧な再試行では同じrequest IDを再利用する。利用者向け返信メールが失敗した場合は保存済みメッセージ本文と同じResend冪等キーでメールだけを再送し、会話履歴を追加しない。管理者アカウントの既存`receive_contacts`列は「問い合わせ・報告を受け取る」購読を表し、公開問い合わせだけでなく、本体ゲーム・SDK Portalからの新規報告と問い合わせ者・報告者の追記も同じ宛先へ送る。

管理者WebAuthnは端末内platform authenticatorだけを登録対象にし、登録optionsの`authenticatorAttachment: "platform"`と登録応答の`internal` transportを両方検査する。認証も登録済みCredential IDを`internal`へ限定する。メール・パスワード後の代替MFAとして、登録済みAuthenticatorの6桁TOTPを使える。secretはowner-only enrollment中に一度だけ返し、DBへAES-GCM暗号化して保存する。TOTPは30秒・前後1 period・DB compare-and-setによるcounter replay拒否・identity別fail-closed rate limitを必須とし、secret・URI・codeをログや監査へ残さない。復旧コードでログインした場合は管理者アカウント画面へ直接誘導し、新しいWindows Hello登録後に通常のパスキーセッションへ切り替える。break-glassの`recovery` scopeは管理者一覧、ダッシュボード、監査ログの読取とMFAリセットだけを許可し、管理者追加・更新・削除および通常の管理APIをサーバー側で拒否する。
...
報告への運営返信は本体Redisの会話履歴を先に保存し、送信者に確認済みの復旧用メールがある場合だけResendで通知する。通知メールには返信本文、環境別SDK Portalの該当スレッド導線、GPTへ貼る報告IDだけを載せ、メール自体を返信先・会話の正本にしない。新規報告と報告者の追記は`lib/user-report-admin-notification.ts`から管理者の共通購読先へ通知し、最終試行日時と安全な失敗理由を報告レコードへ保存する。AI下書きの承認投稿も、以前のスレッド通知状態ではなく新しいメッセージのinsert結果で管理者通知を判定する。管理画面から直近MFAと監査ログを伴う再送ができる。MCPの`get_support_thread`は、利用者が報告IDだけを入力した場合にも呼ぶ規則と、最新返信までの要約、次の対応の説明、変更・返信前の確認を`assistantPolicy`として返す。返信が必要な場合も`prepare_support_reply`で7日間の下書きだけを作り、Portalで本人が確認・修正して送信するまで会話履歴と状態を変更しない。AIの新規報告は、status指定なしの`list_support_threads`で本人の全報告を照合し、関連候補がない場合だけ全report IDを`checkedReportIds`として`prepare_support_report`へ渡す。現在の全report IDと一致しない呼出しは拒否し、同一・再発・続報の可能性がある場合は`get_support_thread`から既存スレッドへの返信へ切り替える。AI用の直接返信toolは提供しない。Portalの人間用`/support/new`は不具合報告・改善要望・要約・詳細・対象ページを入力し、「内容を確認し、報告を送信」を押した場合だけ、認証済み本人の新規スレッドを直接作る。公開問い合わせ、本体報告、公開問い合わせ追記、Portal報告追記は、送信成功まで同じrequest IDを保持する。request IDからrecord／message IDを決め、保存後に応答だけ失われた再試行を冪等化する。メール未登録・未確認・配送失敗でもPortal履歴と状態更新は保持し、管理画面の各メッセージへ配送状態を表示する。`/support?thread=<reportId>`は本人認証後に対象スレッドを展開し、未ログイン時もaccount link後のreturnToを維持する。

SDK v1は、manifest、Game→Controller→LayoutのUI三層、認証済みID・表示名をactorとして注入するserver module、閲覧者別RoomView、revision付きCommand、DB不要のMock Runtimeまでを提供する。新規online-roomゲームは`defineGameSdkOnlineRoomAppSet`へゲーム固有state・Command・Viewだけを登録し、`createGameSdkOnlineRoomModule`で既存のSDK基本セットと合成する。基本セットはRoom作成、ホスト、参加・退出、設定、revision、共通permissions、内部player IDを除いたseat形式の共通View、中断・再戦を所有する。共通module catalogは`@game-fields/game-sdk/modules`を正本とし、新規draftは`createInitialGameSdkModuleProfile()`のcanonical defaultを`system-default`由来の初期contractとして自動確定する。この初期確定に人間actorやconfirmation timestampは付けない。制作AIまたはPortal ownerがcanonical profileを変更する場合はproposalへ分離し、本人の明示承認までrequirements、prototype、packageを停止する。canonicalに同一な再適用はproposalやrevision更新を起こさない。MCPは成立済みcontractの`requiredModuleIds`に加え、各moduleの`delivery`、`packageExports`、`publicApis`、`usage`を`requiredModules`として返す。必須・解除可・任意の内部分類はSDK Portalの所有者画面へ閉じる。profileは本体の`online-room-route-factory`、`online-room-store-runtime`、`@game-fields/game-runtime`、共通UIと純粋domain部品の採用レシピであり、SDK側へ同等基盤を複製しない。SDK-devはcatalogとは別の実装レジストリで全IDを本体共通部品、SDK helper、またはPreview adapterへ解決し、必須IDに割当がない状態を自動テストで拒否する。ゲーム側のCreate/Commandへactor IDや表示名を本人情報として入れず、保存Roomをクライアントへ直接返さない。接続前互換性は`packages/game-sdk/src/handshake.ts`を正本とし、環境、Platform／package release、contract schema、必須capabilityを`/.well-known/game-fields-sdk`またはMCP `get_sdk_handshake`で合意する。MCP `initialize`、OAuth、SDK handshakeはそれぞれtransport、本人認証、互換性確認を担当し、代用しない。`npm run check:sdk`は公開SDK、内部Runtime core、実証ゲームの依存境界を検査し、`npm run lint`から必ず実行される。
...
公開SDKの開発候補は`@game-fields/game-sdk@0.2.0`で、基本契約、SDK基本セット／AppSet合成、mock/client runtime、portable server、handshakeに加え、ワード・LLMのPlatform注入契約、トランプと描画の純粋ロジック・React UIをsubpath exportとして持つ。本体もトランプ・描画を公開package経由で利用する。`npm run build:sdk-package`で独立buildし、`npm run test:sdk-package`でtarballを空の外部fixtureへinstallしてRuntime・resource・React UI・portable protocolを検査する。npmの現行安定版は`0.1.0`で、`0.2.0`はdevelop実機検証後にmainから公開する。

正式な制作clientはChatGPT WorkとClaude Codeである。ChatGPT Workの入口は`sdk/entry/START_GAME_FIELDS.md`、Claude Codeの入口は`sdk/entry/START_CLAUDE_CODE.md`で、通常チャットやその他のsurfaceへ同じファイルを再投入しない。利用可能なら、安定版は公開`koromo2010/app-games`の`sdk-starter`、development候補は`sdk-starter-dev`だけを`--depth 1 --single-branch`で取得する。参照先は`config/sdk-release-profiles.json`の環境別`starterRef`をDownloadMeとmanifestへ同時注入する。ブランチsnapshotは`npm run build:sdk-starter-repository`で生成し、`starter-manifest.json`に公式repository、ref、Platformと同じSemVerの`downloadMeVersion`、environment、starter version、SDK versionを持つ。本体の`main`／`develop`を取得する必要はない。`apps/sdk-portal/.vercel-root-placeholder`とPortal／隔離Preview Rootの`vercel.json`は、VercelがRoot Directory確認後にStarter branchを`ignoreCommand`でCANCELEDにするためだけに置く。ゲーム提出ZIPには含めない。

`sdk/starter-template`を正本とし、SDK tarball、`START_HERE.md`、ChatGPT用`AGENTS.md`、`GAME_SPEC.md`、APIリファレンス、型付きmanifest／Command／RoomView／AppSet、契約テスト、完走デモ、promotion readiness、game package builderを含む。`npm run test:sdk-starter`は入口文書、公開Git用snapshotとZIPの同一性、同梱SDK install、型検査、契約テスト、デモ完走、提出ZIPを確認する。Portal developmentの一般向け入口は`GameFieldsDownloadMe-dev-ver0.2.0.md`、プラグインは`dev-game-fields`で、`sdk-starter-dev`側の`downloadMeVersion: "0.2.0"`と一致しない場合は制作開始前に停止する。productionの入口は`GameFieldsDownloadMe-ver0.2.0.md`、プラグインは`game-fields`、Starterは`sdk-starter`である。DownloadMeは人間向け説明を持たないAI実行契約とし、状態機械、述語、禁止条件、tool呼出し中心の宣言形式で記述する。利用者向けの導入説明はPortalへ分離する。WorkではSDK toolが遅延読み込みされるため、初期一覧に`get_sdk_handshake`がない場合も、対応環境のプラグイン名と`get_sdk_handshake Game Fields SDK接続互換性`でtool検索する。`requiredCapabilities`のMCP入力schemaは固定enumにせず、将来名もserver側の`CAPABILITY_UNAVAILABLE`判定へ到達させる。既存チャットのtool schemaはプラグイン更新で差し替わらないため、更新後に作成した新しいチャットへ対応環境のDownloadMeだけを添付する。保存済み制作者環境は`list_creator_environments`で再取得する。

Portal用の別Vercel Project `app-games-sdk`は`game-fields` Team内に作成済みで、`https://sdk.game-fields.com`から取得できる。本体・devのDB、Redis、Blob、管理者秘密情報は共有していない。GitHub `koromo2010/app-games`へ接続し、Root Directoryは`apps/sdk-portal`、Production Branchは`main`、`develop`はPreview、Ignored Build Stepは`main`と`develop`だけをbuild対象とする。`@game-fields/game-sdk@0.1.0`の初回npm公開は完了している。`0.2.0`は新規プラグイン案内とinstance registry healthを含む次期候補で、mainとnpmには未公開である。外部開発者はSDKで作成したpackageを提出するだけで、`develop`、`main`、Vercel、本番データへの書き込み権限を持たない。運営の`SDK → main`採用だけが同じrevisionとhashを本番カタログへ反映し、本体コードの`dev → main`とは完全に分離する。
...
パスワード再設定と、サイト管理者メール一致によるデバッグ権限の自動付与は `email_verified_at` がある確認済みメールだけを対象にする。管理画面からのプレイヤーID別の個別付与は従来どおり残す。確認機能導入前の既存メールは、一度きりのDB migrationでアドレスを保持したまま `email_verified_at = NULL` へ移し、再確認まで復旧・自動権限付与に使わない。マイページは未登録・未確認・確認済みを区別し、保存済みアドレスをマスク表示する。未確認メールは現在のパスワードを再入力すると同じ登録先へ確認メールを再送できる。新しい確認メールの配送成功後に以前の確認トークンを無効化し、配送失敗時は新しい未配送トークンを捨てて以前の有効なリンクを維持する。
...
メール確認リンクとパスワード再設定リンクはそれぞれ1時間有効で、一度使うとRedisから削除される。確認リンクはGETだけで確定せず、リンク先の明示承認POSTで確定する。同じメールアドレスからのパスワード再設定発行は60秒に1回まで。再設定メールの配送失敗時は、その試行が作った未配送トークンと60秒制限だけを原子的に解除し、別の並行試行が作った制限は消さない。再設定APIは、登録の有無にかかわらず同じ成功応答を返す。未確認メールの再送UIは新規登録・変更フォームと分離し、マスク済み登録先、再送専用の現在パスワード欄、再送ボタンを一つの枠で表示する。Resendの送信拒否は、プロバイダー本文やメールアドレスをログへ残さず、認証設定、送信元未確認、テスト送信先制限、送信枠、レート制限、その他の安全なエラーコードへ分類する。
...
- 参加可能な部屋一覧は全件 `SMEMBERS` + 個別GETを行わず、`SSCAN` と `MGET` で取得する。serverは期限切れ・満室・開始済み・locale不一致等のfilter後に論理ページを満たすまでbounded scanし、上限時はexact `nextCursor` を返す。built-inとSDKの共通clientはterminalまでboundedに継続取得し、namespace＋immutable generation identityで蓄積・dedupeする。malformed・停滞・反復・循環cursorはfail-closedし、navigation・locale変更・unmountで旧取得を失効させる。部屋コードを指定した直接参加はページ外でも利用でき、一覧からの参加はgeneration identityを再検証する。
...
- 2026-07-25に旧選定347行を346語へ正規化し、共通DBに不足していた27語を同期したうえで、easy 119／normal 164／hard 63の保存済み分類を適用した。再dry-runは対応346語・不足0で、「度者」が審査済み集合に含まれないこともdevで確認済み。一時build hookは撤去済みだが、`LEGACY_WORD_DATABASE_URL`と旧DB読取ロールの一時権限は利用者の削除確認待ちである。外部設定の正本は`docs/ENVIRONMENT_VARIABLES.md`。
...
- `/tahoiya`
- 2人から開始可能
- 新規ルームは「全員作成・全員投票」を既定かつ通常UIで選べる唯一の方式とする。保存済みの旧ルームを継続できるよう「回答者1人」の保存形式・進行ロジックは互換用に維持するが、方式選択UIと案内文には表示しない
...
- 互換用の回答者1人ルームだけ、偽説明担当へ正解情報を見せる設定が使える。新規ルームの全員投票では絶対に見せない
...
- Preview中に限定セッションが切れた場合、Word DBとAI APIの401は`PLAYER_AUTH_REQUIRED`へ統一する。外側Shellはゲーム固有iframeを継続せず、共通Session Gateへ戻してSDK Portalでの再ログインを案内する。固定ゲーム側の素材取得エラーだけを表示して、Word DBまたはAI自体の障害に見せない。
...
- production入口は`/GameFieldsDownloadMe-ver<platformVersion>.md`、development入口は`/GameFieldsDownloadMe-dev-ver<platformVersion>.md`から配布する。正本は`/sdk/entry/START_GAME_FIELDS.md`で、Portalのbuild前に`sync:download`が`config/platform-release.json`と`config/sdk-release-profiles.json`から対象環境専用の1ファイルを生成する。旧`DownloadMe.md`、`GameFieldsDownloadMe.md`、整数版`GameFieldsDownloadMe-verN.md`、別環境のSemVer版へのWebアクセスは、そのDeploymentの現行DownloadMeへ一時redirectする。DownloadMeはAI向け実行契約、人間向け説明はPortalと明確に分離する。配布ファイルを手作業で別管理しない。
- Game Fields本体とSDK Portalの公開版は`config/platform-release.json`の`platformVersion`を共通の正本とする。DownloadMeは独立counterを持たず同じSemVer、SDK package、Runtime、Portalのpackage versionも同版へ固定し、`npm run check:versions`で不一致を拒否する。環境別の正本は`config/sdk-release-profiles.json`とし、productionは`game-fields`／`sdk-starter`、developmentは`dev-game-fields`／`sdk-starter-dev`を使う。既存ゲームは作成時のSDK契約schemaへ固定し、最新版への一斉更新はしない。互換維持と旧schema廃止条件は`docs/SDK_VERSIONING.md`を正本とする。
- 制作者ごとに`https://<SDK Portal>/<slug>`を一つ割り当てる。ゲームごとにURLを増やさず、その制作者の広場へゲームカードを追加する。
- 制作者向けrouteは責務で分ける。`/<slug>`は制作環境、`/<slug>/games/<game>?view=modules`は共通モジュール設定、`?view=preview&revision=<sha>`は固定client packageをブラウザ内だけで確認するPreview、`?revision=<sha>`はRoom API／Redis／portable server／actor別View／既存Room復帰を含む正式Roomである。Previewから正式Room状態を復元せず、正式RoomからPreviewへfallbackしない。MCPの`packagePreviewUrl`はPreview routeを返す。
- Runtime packageのrevisionはimmutableである。履歴画面は各revisionがdevelopment／mainのどちらで現在利用中かを表示し、取得と固定revision Preview／正式Roomへの導線を提供する。内容更新は既存revisionの上書きではなく新revision追加で行い、個別revision削除は監査・開始済みRoomの再現性を壊すため提供しない。
...
- ChatGPT WorkとClaude Codeの共通制作経路は`/api/mcp`のOAuth 2.1付きリモートMCPとする。DownloadMeへ秘密値を埋め込まない。protected resource metadata、authorization server discovery、DCR、authorization code + S256 PKCE、refresh token rotation、scope検証をPortalで提供する。MCP toolsはログイン中アカウントの既存制作者環境一覧、URL空き確認、本人名義の予約・確定、本人所有環境へのgame package保存だけを公開し、本体DB・管理機能・他利用者環境へは到達させない。制作開始時はまず`get_sdk_handshake`へ各client profile記載の環境・release・contract・必須capabilityを送り、`accepted=true`をaggregate verdictとして採用する。矛盾する別の実観測がない限りcanonical endpoint等を独自に再判定しない。bindingは`structuredContent.environmentBinding`から取得し、その後に`list_creator_environments`を呼ぶ。既存環境が1件なら再利用、複数なら選択、0件の場合だけ新規URLを予約する。WorkはGame Fields App、Claude Codeは同じ環境別remote MCP URLを使う。初回のApp／MCP接続とブラウザ承認は利用者操作が必要で、DownloadMe添付だけで未登録Appを自動導入できるとは説明しない。
...
- SDK Portalの`/api/health`はDB schemaと制作者URL用instance registryを必須componentとして扱う。registryは3秒上限の`PING`だけを行い、資格未設定は`SDK_INSTANCE_REGISTRY_NOT_CONFIGURED`、接続不能または不正応答は`SDK_INSTANCE_REGISTRY_UNAVAILABLE`として503にする。予約・確定とhealthは同じREST資格resolverを使い、healthのための書込みや予約キー作成は行わない。Room実行側はネットワーク例外と408／502／503／504だけを1回再試行し、401／403は`GAME_SDK_REMOTE_RUNNER_AUTH_FAILED`として停止する。正式PreviewのRoom作成調査では、本体の`event=game-sdk.preview-room`、Portalのruntime grant取得、Previewの`event=sdk.preview-runner-auth`を同じ時刻・revisionで照合する。
...
- 2026-07-26に本番隔離Preview Project `app-games-sdk-preview`と`preview.game-fields.com`を作成し、本番専用署名鍵、private package Git `koromo2010/game-fields-sdk-mocks`、Portal専用write token、Preview専用read tokenを分離して登録・再デプロイした。`app-games-sdk`と`app-games-sdk-preview`のProduction Branchは`main`へ統一済み。本番SDK専用`app-games-sdk-neon`もPortalのProductionだけへLink済みである。2026-08-02に`UPSTASH_REDIS_REST_URL`と`UPSTASH_REDIS_REST_TOKEN`を本番`app-games-sdk`のProductionだけへ登録し、Preview Projectへは追加していない。新Deploymentは未作成である。配備後は`GET https://sdk.game-fields.com/api/health`で現行schemaと`instanceRegistry.status: ok`、`namespace: production`を同時確認する。
...
- `SDK_PREVIEW_SIGNING_SECRET`はPortalではEd25519秘密鍵の導出とpackage client／server grant署名、Previewでは固定revision・asset path・期限へ限定したasset tokenのHMAC署名にだけ使う。両Projectの値は一致させない。Previewはproductionで固定したPortal公開鍵を使ってgrantをローカル検証し、汎用の`/api/preview-token/verify`を持たない。Portalの`/.well-known/sdk-preview-public-key`は段階公開と鍵ローテーション時の公開鍵取得用であり、通常のproduction Room実行は同endpointへ依存しない。Portalだけに`SDK_MOCK_GITHUB_WRITE_TOKEN`、previewだけに別の`SDK_MOCK_GITHUB_READ_TOKEN`を設定し、どちらも専用非公開repo以外へ権限を与えない。変数配置は`docs/ENVIRONMENT_VARIABLES.md`を正本とする。

### 承認済みSDKゲームの正式Runtime

develop限定の`wordwolf-sdk`を最初の承認済み対象として、
`/sdk-games/[gameId]`の正式Shellを追加した。隔離Previewや保存済みモックを
実行元にせず、静的server registry、署名済み本体Cookie、Redis CAS、
active room、一覧、revision通知を使う。AppSetが結果transitionで返す
`standardResult`を共通結果へ投影し、保存後hookから`wordwolf-sdk`専用の
戦績、rating、playbackへ冪等記録する。`reason`は機械コード、任意の`presentation.reason`は日本語／英語表示、`presentation.highlights`は共有可能な最大3件、`presentation.playLog`は参加者本人の詳細履歴最大50件とする。playbackへ渡すのはこの共通結果だけで、
役職やお題等の固有stateは渡さない。

timerは`expireAppTurn`をserver callbackとして必須化し、deadline、
...
正式Shellのゲームラウンジでは、Preview・採用済みiframe package・旧wordwolf clientの
全経路を共通`GameSdkShellHeader`へ集約する。各Shellは`surface="lounge"`を渡し、
共通ヘッダー自身がトップバーへ戻る直接操作を常設する。個別Shellへ戻るリンクを
直書きしない。採用済みゲームはメニューを開かず1クリックで`/games`へ戻り、
制作者Previewは同じ契約で制作者ページへ戻る。「ロビー」は作成済みRoomの開始前状態だけを指す。

...
- ワード候補DBと使用履歴はゲーム別に分離する。通常出題では参加者の誰か一人でも使用済みの単語を除外し、参加者全員が未使用のローカル／再利用候補を優先する。該当候補が尽きた場合だけLLM APIで新規生成し、そのゲーム専用候補DBへ追加する。デバッグ生成は候補DB・使用履歴を更新しない。
...
- ほどあいトークは2～8人の1台回覧式。0～120の目盛りを言葉で表し、並び違い数から1～4ラウンド、各0～3点を得る。状態はブラウザー内保存でネット対戦は未実装。
- 最初のプレイヤーをローカルホストとして扱い、共通 `DebugModeButton`、`RoomConfigSummary`、`RoomTimeLimitControl`、`GamePhaseTimer` を使用する。時間切れは表示のみで自動進行しない。
- ほどあいトークのラウンド数・ヒント時間・相談時間は共通 room-defaults APIで端末とRedisへ保存する。デバッグ状態は保存対象外。
- ほどあいトークの権利面の方針とルールは `docs/HODOAI_TALK_PROTOTYPE.md`、進行と独自お題は `lib/hodoai-talk.ts`、画面は `app/hodoai-talk/HodoaiTalkGame.tsx`。

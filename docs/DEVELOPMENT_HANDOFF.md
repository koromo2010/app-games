...
- デバッグのワード生成テストは `forceNew=1` を送り、既存の候補DB・ローカル候補を返さずLLMで新しい候補を生成する。候補DB内の全単語を生成NGリストに含め、成功結果だけを全員未使用の候補として保存する。AIを利用できない場合や全プロバイダー失敗時は既存候補へフォールバックせずエラーを返す。

...
- デバッグ用ダミー参加者の追加・一覧・削除UIは、ゲーム固有のロビー設定や参加者一覧へ置かず、共通 `DebugModeButton` 内の `DebugParticipantControls` に集約する。ゲーム側はダミー一覧と追加・削除Commandだけを注入し、認可・フェーズ・人数上限はサーバー側Storeで再検証する。公開オンラインゲームのワードウルフ、たほい屋、ワードスケール、ワードソナー、ワードアウト、大富豪は、個別削除とデバッグOFF時の一括整理に対応する。ダミーをactive-room索引へ登録せず、旧索引が残っている場合も削除時に解放する。共通の名前採番とロビー復帰状態の正規化は`lib/online-room-debug-participants.ts`、たほい屋固有の参加者依存状態の整理は`lib/tahoiya-debug-participants.ts`を正本とする。

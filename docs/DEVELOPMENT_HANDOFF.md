...
- `/nigoichi` は非公開アクセスキーかつログイン済みの利用者向け。3〜6人のオンライン部屋制で、部屋一覧、4文字コード、任意の合言葉、アクティブ部屋復帰に対応する。
- 各ゲームで `2N+1` 個の単語を並べ、重複なしの2番号を各人へ配る。全員の連想語がそろうまで他人の提出を隠し、一斉公開後は余り番号の予想を全員が出すまで他人の予想を隠す。結果で余り番号、全手札、正誤を公開する。

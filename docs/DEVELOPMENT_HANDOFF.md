...
- 試用期間中の入口ファイルは`/GameFieldsDownloadMe-ver6.md`から配布する。正本は`/sdk/entry/START_GAME_FIELDS.md`で、Portalのdev/build前に`sync:download`が`public/GameFieldsDownloadMe-ver6.md`へ同期する。内容を改版するときはファイル名の`verN`も上げ、仕様固定後にバージョンなしの名前へ戻す。配布ファイルを手作業で別管理しない。

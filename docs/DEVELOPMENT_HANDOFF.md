...
- 管理画面のLLM評価レビューでは、管理者のOK/NG票と正式採用を別操作として扱う。管理者票はお題品質フィードバックへ1票として反映し、正式採用は紐付くペアdraftを`active`へ昇格して共通DBの本番出題対象へ反映する。正式採用はMFA再確認と確認ダイアログを必須とし、投票内容は自動変更しない。
- 同じartifactへの評価は更新可能

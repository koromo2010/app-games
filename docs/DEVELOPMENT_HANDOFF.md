...
- デバッグ用ダミー参加者の追加・一覧・削除UIは、ゲーム固有のロビー設定や参加者一覧へ置かず、共通 `DebugModeButton` 内の `DebugParticipantControls` に集約する。ゲーム側はダミー一覧と追加・削除Commandだけを注入し、認可・フェーズ・人数上限はサーバー側Storeで再検証する。ワードウルフとたほい屋ではデバッグOFF時に残っているダミーを全員整理する。たほい屋の参加者依存状態の整理は`lib/tahoiya-debug-participants.ts`を正本とし、ダミーをactive-room索引へ登録しない。

...
- Redisを正本とし、revision付きCAS、共通TTL、1人1アクティブ部屋、Cookie由来のactor、サーバー側Command検証を使う。各入力フェーズはなし／30／60／90／120秒から時間制限を選べ、時間切れはサーバーで自動補完・精算して進行する。詳細は `docs/CODE_INTERCEPT.md`。
...
- デバッグロビーには、本番フローへ入れる前の比較用として「難易度先行審査モック」を置く。実際の未使用候補10語を説明なしでLLMへ渡し、一般成人の推定認知率を `既知 / 境界 / 一般には不明 / ほぼ誰も知らない` に分類する。秘境は `一般には不明（3〜14%）`、魔境は `ほぼ誰も知らない（0〜1%）` だけを合格にし、2%は除外する。大学名・企業名・地名は明示フラグで強制除外する。結果は共有用にコピーできるが、候補DB・出題履歴へ保存しない。

...
- デバッグロビーには、本番フローへ入れる前の比較用として「難易度先行審査モック」を置く。実際の未使用候補10語を説明なしでLLMへ渡し、一般成人の推定認知率を `既知 / 境界 / 一般には不明 / ほぼ誰も知らない` に分類する。難易度設定にかかわらず `一般には不明` だけを合格にし、大学名・企業名は明示フラグで強制除外する。結果は共有用にコピーできるが、候補DB・出題履歴へ保存しない。

...
| ほどあいトーク | `app/hodoai-talk`, `app/api/hodoai/rooms`, `lib/hodoai-room-store.ts`, `lib/hodoai-talk.ts` |
...
- `/hodoai-talk` も同じCookieをサーバー側で検証する。独自名称「ほどあいトーク」と独自お題だけを使う。
...
- ほどあいトークは2～8人のログイン必須オンライン部屋制。部屋一覧・4文字コード・任意の合言葉・アクティブ部屋復帰に対応する。
- 0～120の秘密の目盛りを各プレイヤーへ配り、全員同時にヒントを提出する。提出中のAPI応答には本人の目盛りと本人のヒントだけを含める。全員提出後、全ヒントを公開してホストが並べ替える。
- ゲーム状態はRedisを正とし、revision付きcompare-and-setで操作を保存する。全員提出または時間切れのフェーズ遷移、並び違い採点はサーバーが行う。
- 部屋設定は全員に表示し、設定変更・開始・並べ替え・採点・次ラウンドはホストだけ。共通デバッグ認証、共通時間制限、設定デフォルト保存を使用する。
- 個人利用Cookieはページと `/api/hodoai/rooms` の両方で検証する。通常のゲーム操作にはゲームロビーで作成したプレイヤーアカウントも必要。
- 権利面と詳細ルールは `docs/HODOAI_TALK_PROTOTYPE.md`、型と独自お題は `lib/hodoai-talk.ts`、Redis進行は `lib/hodoai-room-store.ts`、APIは `app/api/hodoai/rooms/route.ts`、画面は `app/hodoai-talk/HodoaiTalkGame.tsx`。

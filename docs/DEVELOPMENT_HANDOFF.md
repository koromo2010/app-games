...
- 単語は共通単語DBの `active_word_game_eligibility.game_id = 'general_word_pool'`（General Game Pool）を正本とする。難易度はそのeligibility行の `difficulty_easy`、`difficulty_normal`、`difficulty_hard` 値から判定し、旧値（`easy`、`normal`、`standard`、`hard`）と移行用の同名eligibilityタグにも対応して、開始時に必要枚数を重複なしで抽出する。旧ワードウルフ適格性からは抽出しない。簡単は簡単100%、普通は普通80%＋簡単20%、難しいは難しい50%＋普通40%＋簡単10%を各語ごとに抽選する。同じ参加者が当日（JST）このゲームで見た単語は除外し、必要数を揃えられないほど使い切った場合だけ当日履歴を全解除する。DB未設定または必要枚数を確保できない場合はローカル固定語彙へ戻さず、開始を503で失敗させる。実装は `lib/general-game-word-pool.ts` と `lib/general-game-word-history-store.ts`。
...
- 秘密単語はワードアウトと同じ `general_word_pool` 適格性タグから重複なしに抽出する。ロビーで簡単・普通・難しいを選べ、簡単は簡単100%、普通は普通80%＋簡単20%、難しいは難しい50%＋普通40%＋簡単10%を各語ごとに抽選する。旧ルームは普通として復元する。同じ参加者が当日（JST）コードインターセプトで見た単語は除外し、使い切り時だけ当日履歴を全解除する。ワードアウトの履歴とは混ぜない。
...
正解説明の長さは全体共通の管理ハイパラ `tahoiya-definition-median` で中央値を10〜50文字帯から10文字刻みで選ぶ。初期値は30文字帯で、約10字、20字、30字、40字、50字、55〜60字を `15% / 25% / 30% / 17% / 9% / 4%` の順で混在させる。30文字帯までの累積確率は70%（20文字帯までは40%）で、上限は60文字。部屋設定には出さず、変更は保存済みのお題を書き換えず新しく生成する正解文から反映する。選ばれた長さは生成と校閲の両プロンプトへ渡すが、文字数合わせの水増しはさせない。長さより辞書語釈としての自然さを優先し、必要な語義が少なければ目安より短くする。新しく生成する文は、敬体、知名度・普及、語の直訳、歴史や文献への登場などで水増しした場合に保存前に棄却する。保存済みのお題はこの検査で再判定せず、そのまま再利用する。

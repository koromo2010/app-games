...
- `lib/tahoiya-seed-topics.ts` に、オープンな専門語彙源から人手確認した初期シードを保持する。各レコードは `easy | standard | extreme` のLLM難易度判定、ジャンル、参照ライブラリ、URL、判定理由を持つ。API初回利用時に `HSETNX` でゲーム専用Redis候補DBへ登録するため、既存候補は上書きしない。
...
- ほどあいトークは2～8人の1台回覧式。0～120の目盛りを言葉で表し、並び違い数から3ラウンド合計0～9点を得る。状態はブラウザー内保存でネット対戦は未実装。

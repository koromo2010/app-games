...
- 試用期間中の入口ファイルは`/GameFieldsDownloadMe-ver1.md`から配布する。正本は`/sdk/entry/START_GAME_FIELDS.md`で、Portalのdev/build前に`sync:download`が`public/GameFieldsDownloadMe-ver1.md`へ同期する。内容を改版するときはファイル名の`verN`も上げ、仕様固定後にバージョンなしの名前へ戻す。配布ファイルを手作業で別管理しない。
...
- 表のGame FieldsアカウントをSDK所有権の正本として使う。Portalの`/api/account-link/start`は本体`/api/sdk-account-link`へ遷移し、本体の署名済みプレイヤーCookieで本人確認した後、60秒の署名コードをSDKへ返す。Portalはstateを照合して30日のSDK専用HttpOnly Cookieへ交換し、新規制作者の`owner_player_id`へ紐づける。表サイトのパスワードとCookieをSDKへ渡さない。環境別の`SDK_ACCOUNT_LINK_SECRET`を本体とPortalだけで共有し、`GAME_FIELDS_APP_BASE_URL`で接続する本体を固定する。
- ChatGPTの別端末・別チャットで接続を保持する最終経路はApps SDK準拠のOAuth 2.1付きMCP Appとする。DownloadMeへ秘密値を埋め込まない。MCPのprotected resource metadata、OAuth discovery、PKCE、access tokenのaudience/scope検証、SDK操作tool、ChatGPT側のApp登録は次段階であり、現時点のブラウザSSOだけをChatGPTアカウント連携済みとは扱わない。
- 初期プレビューは制作者slugごとにブラウザ保存を分離し、広場、入室前、部屋ロビー、ゲーム固有領域、結果、視点・フェーズ・ダミー・中断のデバッグUIを確認できる。複数端末で共有する正式Room永続化は次段階である。
- 実際の`mock/`は管理トークン付き`PUT /api/instances/<slug>/games/<game-id>/mock`へ送る。Portalはファイル名、拡張子、件数、単体・合計容量、必須`index.html` / `styles.css` / `mock.js`を検査し、専用の非公開Gitリポジトリの`previews/<slug>/<game-id>/mock`だけへ自動commitする。制作者やAIへGit・Vercelの直接権限を渡さず、更新のたびに同じ共有URLを使う。
- クライアントへ案内するURLは`https://<SDK Portal>/<slug>/mock/<game-id>`。PortalはDBの確定commit SHAへ10分のHMAC grantを発行し、内部iframeだけを`preview-dev.game-fields.com`または`preview.game-fields.com`へ接続する。

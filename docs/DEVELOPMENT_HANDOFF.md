...
| ことば潜伏戦 | `app/kotoba-senpuku`, `app/api/kotoba-senpuku/rooms`, `lib/kotoba-senpuku-room-store.ts`, `lib/kotoba-senpuku.ts` |
...
- `/kotoba-de-kazu-narabe` も同じCookieをサーバー側で検証する。旧 `/hodoai-talk` はここへ転送する。
- `/kotoba-senpuku` も同じCookieをサーバー側で検証する。
...
- ことばで数ならべは2人以上のログイン必須オンライン部屋制。通常画面に上限は表示せず、同期負荷を守る技術的安全上限だけ50人とする。0～120の異なる目盛りのため理論上限は121人、快適な目安は20～30人。
- 部屋一覧・4文字コード・任意の合言葉・アクティブ部屋復帰に対応する。Redisを正とし、revision付きcompare-and-setでヒント提出、時間切れ遷移、並べ替え、採点を保存する。
- 提出中のAPI応答は本人の目盛りと本人のヒントだけを含める。全員提出後、全ヒントを公開してホストが並べ替える。
...
- 権利面・人数・詳細ルールは `docs/KOTOBA_DE_KAZU_NARABE.md`、型と独自お題は `lib/hodoai-talk.ts`、Redis進行は `lib/hodoai-room-store.ts`、APIは `app/api/hodoai/rooms/route.ts`、画面は `app/hodoai-talk/HodoaiTalkGame.tsx`。
- ことば潜伏戦は2〜8人の独自オンライン文字推理ゲーム。文字スキャンと直接推理で信号点を集め、脱落せず全員が最後まで手番を持つ。秘密語は本人だけへ返し、公開マスク・得点・手番は全員同期する。
- ことば潜伏戦はラウンド数、秘密語入力時間、手番時間をプレイヤー別デフォルトとして保存する。デバッグONホストはダミー追加、秘密語補完、全秘密語表示、手番代行・自動実行ができる。詳細は `docs/KOTOBA_SENPUKU.md`。

...
- SDK Portalは`apps/sdk-portal`を正本とし、`sdk.game-fields.com`で公開する。
...
- 制作者ごとに`https://sdk.game-fields.com/<slug>`を一つ割り当てる。ゲームごとにURLを増やさず、その制作者の広場へゲームカードを追加する。
- URL名は3〜32文字の小文字英数字とハイフン。AIは制作開始時に希望名を聞き、`/api/instances/check`で重複確認後、`/api/instances/reserve`で7日間仮予約する。予約はRedisの`SET NX`で一意にする。
- 必須環境変数はSDK Portal専用の`SDK_REDIS_REST_URL`と`SDK_REDIS_REST_TOKEN`。互換読取としてVercel/Upstash標準名も受け付けるが、SDK専用名を優先する。未設定時は空きと推測せず503を返す。

...
- 隔離previewは各HTMLへ`apps/sdk-preview/lib/preset-runtime.ts`の`GameFieldsPreset`を自動注入する。参加者、ダミー、デバッグ、視点、フェーズ、開始、中断、再戦、自動進行は共通CommandとしてPreview側で状態管理し、ゲーム固有JavaScriptは`registerGame`で固有状態の処理だけを接続する。利用属性とAPIは`sdk/starter-template/SDK_MODULE_CATALOG.md`と`SDK_API.md`を正本とする。本体統合時の認証・認可・永続化を、このブラウザPreview状態で代用してはならない。

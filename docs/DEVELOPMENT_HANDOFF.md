...
管理画面は改善要望・バグ報告と公開問い合わせを別タブにせず、`AdminSupportInboxPanel`の一つの「問い合わせ・報告」受信箱へ作成日時順で混在表示する。状態フィルター、会話、返信、管理者通知の送信状態・安全な失敗理由・再送は共通操作とする。管理者アカウントの既存`receive_contacts`列は「問い合わせ・報告を受け取る」購読を表し、公開問い合わせだけでなく、本体ゲーム・SDK Portalからの新規報告と問い合わせ者・報告者の追記も同じ宛先へ送る。

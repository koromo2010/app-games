...
- ワードウルフは1ゲーム、たほい屋は1ラウンド、ことばで数ならべは同じカードへの全ことば提出と最終並べ替えを1戦として記録する。
...
- ことばで数ならべは2人以上のログイン必須オンライン部屋制。各自へ0～120の数字カードを1～5枚（初期値1枚）最初に1度だけ配り、同じカードへテーマを変えて1～4回ことばを出した後、全カードを1度だけ昇順へ並べる。通常画面に人数上限は表示せず技術的安全上限は50人、カード総数は121枚以下とする。

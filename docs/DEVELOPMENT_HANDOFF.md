...
2026-07-19に、一般ゲーム向けの安全側語彙評価をマスターとは別の `word_pool_evaluations` に追加した。2026-07-20の `standard-game-ichi1-safe-v3` はJMdictの `ichi1` 普通名詞を基礎に、Zipf下限・文字数上限を使わず、WordNet階層と可逆フラグで抽象語・上位概念・人物だけの語・呼称・古語・形式分類語を除外する。ローカルDBでは2,247語中347語が候補で、`general_game_pool` とeasy 119語・normal 165語・hard 63語の難易度フラグを保持する。一般347語とJMdict四字熟語2,168語は、本番の既存カタログを置換しない差分同期で `shared_word_pool_evaluations` へ反映済みである。
...
ゲームの公開／非公開は `config/game-registry.json` の `private` を正本とする。ページは `gamePageAccessAllowed`、部屋APIは `gameApiAccessDeniedResponse` を通し、非公開ゲームだけ共通Cookieを要求する。ことばソナーとワードスケールは公開ゲームのためアクセスキー不要だが、ログインと部屋内の操作権限は引き続き必要。
...
- 各回の提出中は本人が持つカードの数字と本人が過去回を含めて提出したことばだけを返す。設定回数ぶんの全カード提出後、全員のことばを公開する。ホストがロビーまたは並べ替え中に指定した並べ替え役1人だけがカードを動かし、ホストが最終順を確定して数字を公開する。担当者以外の `reorder` はサーバー側で拒否する。
...
- ローカルJMdictは、`word_source_entries.entry_payload`へ`ent_seq`単位の原本フィールドをJSONB保存し、`word_source_entry_links`で共通単語へ関連付ける。2026-07-19時点で全217,946エントリー、301,447表記を処理し、103,959表記は既存IDへ統合、197,488表記は新規追番、全307,169リンクを作成済み。未解決表記・未リンクエントリーは0件である。JMdict全件と原本フィールドは本番へ送らず、2026-07-20に安全・表記品質条件を通った四字熟語2,168語の最小投影と所属フラグだけを本番へ同期した。全件取込は`scripts/import-jmdict-all.py`、選定プール同期は`scripts/publish-selected-word-pools.py`を使う。
...
- 本番の主フィールドは `word_master_id`、`surface`、`reading`、`zipf_frequency`、`active`。ゲーム別難易度はZipfから導出する。
- `word_master_id` はローカルIDを永久利用し、物理削除・番号再利用は禁止。停止は `active = false`。
- センシティブ語は `word-content-safety-v1` の完全一致で事前除外し、`words.content_safety_status` と理由フラグへ保存する。既知語は `scripts/apply-word-content-safety.py` で3ゲームとも使用不可にし、本番同期から外す。未知語はワードウルフ相方生成と同じLLMリクエストで最初の単語を判定し、却下時だけ単語IDへ `llm-*` 判定を蓄積する。相方は安全な語だけを生成させ、却下相方は保存しない。
- 差分同期は `scripts/upload-shared-word-catalog.py`。対象条件と運用詳細は `docs/WORD_MASTER_DATABASE.md`。
- 辞書原本、正規化CSV、Wikidataキャッシュ、DBダンプはGit・Vercel・本番DBへ送らない。`.vercelignore` のローカル辞書除外を維持する。
- VercelのSensitive DB変数を表示・ログ出力しない。初回同期に使った署名付き一時Functionとプレビューは削除済みで、恒久的な外部投入APIは存在しない。
- ワードウルフの意味近傍候補は、まずローカル固定のchiVe `v1.3 mc90`で試作する。`scripts/prepare-chive.py` が取得・SHA-256検証・収録率集計、`scripts/generate-chive-wordwolf-pairs.py` が未審査サンプル生成を担当する。初回収録率は共通候補197,040件中128,183件（65.05%）。モデルと生成JSONは `.word-master-local/` のみで保持し、Sudachi同義語辞書と日本語WordNetの後段審査前には本番へ登録しない。
- Sudachi同義語辞書もCoreと同じ `20260428` に固定する。`scripts/prepare-sudachi-synonyms.py` が取得・SHA-256検証・索引化、`scripts/filter-wordwolf-pairs-sudachi-synonyms.py` がchiVe候補を除外する。固定版は70,661項目・26,152グループ、共通候補との一致35,890件（18.21%）。初回100組から同義語3組を除外したが、未収録同義語もあるため日本語WordNetと最終審査を続ける。
- 日本語WordNetは公式SQLite版 `1.1` に固定する。`scripts/prepare-japanese-wordnet.py` が取得済み圧縮ファイルのSHA-256検証・展開・整合性検査・収録率集計、`scripts/analyze-wordwolf-pairs-japanese-wordnet.py` が同一概念・類似・直接上下・兄弟関係を判定する。共通候補197,040件中42,970件（21.81%）が一致し、Sudachi除外後97組から同一synset 2組を除外、兄弟語3組を含む95組を残した。83組は片方以上が未収録なので補助判定に限定する。原本・展開DB・判定JSONは `.word-master-local/` のみで保持し、公開利用時は固定マニフェストの帰属リンクを表示する。
- 既定条件の全件収容量調査では、chiVe一致44,239語から重複なし16,025組・32,050語を生成し、Sudachiで410組、WordNetで298組を除外した。辞書工程通過は15,317組・30,634語、うちWordNet兄弟概念は167組・334語。残る15,150組はLLMまたは人の最終審査待ちで、除外語の再ペアリングは未実装である。

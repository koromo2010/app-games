...
- 最終結果ではホスト以外も共通 `GameResultShareButton` からプレイログを共有できるようにする。共有文は得点と匿名化した見どころに限定し、参加者名、秘密情報、投稿本文、認証付きURLを含めない。
...
- ことばで数ならべは2人以上のログイン必須オンライン部屋制。各自へ0～120の数字カードを1～5枚（初期値1枚）配り、指定テーマに沿ったことばをカードごとに出して、全カードを昇順へ並べる。通常画面に人数上限は表示せず技術的安全上限は50人、カード総数は121枚以下とする。
- 部屋一覧・4文字コード・任意の合言葉・アクティブ部屋復帰に対応する。Redisを正とし、revision付きcompare-and-setでヒント提出、時間切れ遷移、並べ替え、採点を保存する。
- 提出中のAPI応答は本人が持つカードの数字と本人が提出したことばだけを含める。全カードの提出後、全員のことばを公開してホストがカード単位で並べ替える。

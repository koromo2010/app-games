...
- 制作者URLの表画面はPortal独自ロビーを持たず、Game Fields本体の`/sdk-preview/<creator>`を全画面利用する。本体の`GameLobby`へSDKゲーム定義を追加し、カード固定外枠、ログイン、共通UIを既存ゲームと共有する。ゲームURLは`/<creator>/games/<game-id>`とし、`/mock`を利用者向けURLにしない。

...
`sdk/starter-template`を正本とし、SDK tarball、`START_HERE.md`、ChatGPT用`AGENTS.md`、`GAME_SPEC.md`、最小APIリファレンス、型付きmanifest／Command／RoomView、契約テスト、ダミー2人の完走デモを含む。従来の試用ZIPは`npm run build:sdk-starter`で生成できる。スターター内の`npm run package`は`node_modules`、`dist`、`.git`を除外した`submission/game-fields-submission.zip`を作る。`npm run test:sdk-starter`は入口文書、公開Git用snapshotとZIPの同一性、同梱SDK install、型検査、契約テスト、デモ完走、提出ZIPを確認する。Portalの一般向け入口は`GameFieldsDownloadMe-ver7.md`で、スターター側の`downloadMeVersion: 7`と一致しない場合は制作開始前に停止する。
...
- 試用期間中の入口ファイルは`/GameFieldsDownloadMe-ver7.md`から配布する。正本は`/sdk/entry/START_GAME_FIELDS.md`で、Portalのdev/build前に`sync:download`が`public/GameFieldsDownloadMe-ver7.md`へ同期する。内容を改版するときはファイル名の`verN`とstarter manifestの`downloadMeVersion`を同時に上げ、仕様固定後にバージョンなしの名前へ戻す。配布ファイルを手作業で別管理しない。

...
- `/games/code-intercept` は非公開アクセスキーかつログイン済みの利用者向け。旧 `/code-intercept` は正式URLへ転送する。4〜12人を赤・青へ分け、各チーム2人以上・人数差1人以内で開始する。チーム編成は手動または開始時ランダムを選べ、ランダム時は人数を均等に振り分けてチーム内の出題順もシャッフルする。
...
- 「秘境」と「魔境」のお題難易度がある。共通DBの `0 <= 実質Zipf < 3` を未判定母集団とし、LLM推定認知率が `1%超〜14%` なら秘境、`0〜1%` なら魔境とする

...
- 最初の候補packageでは進行・共通moduleを原則必須とするsystem-default初期contractを使う。この初期確定を人間確認済みとは扱わない。AIまたは制作者は不要な進行部品の削除を提案できるが、active profileから外すのはPortalでの人間確認後だけとする。
...
- `online-room`は制限時間を1項目だけ必須とし、ゲーム側がその初期値と選択肢を宣言する。0秒を選択肢へ含める場合は制限なしとして扱う。
- ゲーム固有slotは共通設定UIを複製せず、共通Room設定として同期された値を参照する。
- ルールには「何をするか」「準備」「1回の流れ」「得点」「勝ち・終了条件」「時間切れ」を含める。
- ゲーム固有slotは勝敗と得点を表示し、「同じ部屋へ戻る」「広場へ戻る」「退出」は外側Shellに任せる。
- Platformが所有する非公開のShell policyをゲーム固有packageから表示・変更・再実装しない。
...
- デバッグモードには最低限、ダミー参加者追加、閲覧プレイヤー視点切替、安全な主要状態進行、待機・時間切れ・切断・入力エラーの再現を用意する。ゲーム固有stateのphase文字列を直接書き換える操作は作らない。
...
- 終了理由は内部コードをそのまま画面へ出さず、`standardResult.presentation.reason`へ日本語・英語を記録する。
- `standardResult.presentation.highlights`は共有可能な見どころを最大3件、`standardResult.presentation.playLog`は参加者向けの詳しい時系列を最大50件とする。共有用と本人履歴用を同じ配列で代用しない。
- 詳細ログは結果時点で全参加者に公開済みの情報だけから組み立てる。秘密語・正解等はゲームの結果画面で正式に公開する仕様の場合だけ含める。
...
- 秘密情報がある場合の本人表示と非表示
- デバッグ権限あり／なしの表示差
- ダミー参加者、閲覧視点、安全な主要状態進行、時間切れ・切断・入力拒否、自動進行、中断

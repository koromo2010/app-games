...
最初のモックをSDKへ保存すると、共通モジュール38件がすべて必須になります。ChatGPTや管理トークンからは変更できません。利用者がモックを承認した後、ChatGPTは`get_game_module_requirements`から返る確定済みの`requiredModuleIds`と各moduleの`delivery`・公開import・API契約を正本として本実装します。

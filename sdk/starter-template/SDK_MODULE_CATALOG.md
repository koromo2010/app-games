...
| ゲーム共通ヘッダー | 本体統合時に利用 | 広場へ戻る、メニュー、manifestのルール、プレイヤーメニュー。ルールは全phaseでトップバナーから開く |
| 部屋共通UI | 本体統合時に利用 | 参加者一覧、ロビーだけに表示する部屋設定、ホストだけの設定変更、時間制限。対戦中は共通サイド欄を隠してゲームを全幅表示し、中断だけをトップバナーへ置く。timerの正本は共通module、表示位置はゲーム固有クライアント |
| デバッグUI | SDK標準／本体統合時に利用 | 権限表示、ダミー参加者、閲覧視点、安全な主要状態進行、時間切れ・切断・入力拒否の再現、参加者を維持した進行中断 |
| 結果UI | SDK標準／本体統合時に利用 | `standardResult.presentation`の日本語・英語終了理由、共有用見どころ最大3件、本人履歴用プレイログ最大50件。同じ部屋へ戻る、広場へ戻る、匿名共有、履歴、AI生成物フィードバック、再戦導線 |

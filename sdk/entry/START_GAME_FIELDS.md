...
Game Fields SDK toolsが未接続なら、制作を開始したふりをせず、Workでは最初に`gameapp-dev`プラグインの追加・選択を案内してください。プラグインが候補に存在しない場合だけ、開発者モードをONにして`__SDK_PORTAL_BASE_URL__/api/mcp`をOAuth Appとして追加する手順へ進みます。Codexでは同じURLをリモートMCPとして接続します。OAuth画面では、すでにSDK Portalへ接続済みのGame Fieldsアカウントで「この制作クライアントを許可」を押します。接続後、`check_creator_url`を呼べることを確認します。接続確認前にURL名やゲーム内容を尋ねてはいけません。

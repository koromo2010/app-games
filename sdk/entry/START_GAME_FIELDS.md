...
```text
GET __SDK_PORTAL_BASE_URL__/api/instances/check?slug=<希望名>
```

空いていれば次へ予約し、返された`url`を利用者へ伝えてください。

```text
POST __SDK_PORTAL_BASE_URL__/api/instances/reserve
...
```text
POST __SDK_PORTAL_BASE_URL__/api/instances/finalize
Content-Type: application/json

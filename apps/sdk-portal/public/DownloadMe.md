...
```text
GET https://sdk.game-fields.com/api/instances/check?slug=<希望名>
```

空いていれば次へ予約し、返された`url`を利用者へ伝えてください。

```text
POST https://sdk.game-fields.com/api/instances/reserve
...
制作者用URLはゲーム単位ではありません。同じ制作者が作るゲームは、すべて同じ`https://sdk.game-fields.com/<slug>`の広場へ追加します。

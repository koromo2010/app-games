...
```text
https://sdk-dev.game-fields.com/api/mcp
```
...
`requiredCapabilities`は以下の7件をそのまま送り、tool schemaに見える別surface向けの候補を追加してはいけません。
...
- Node.jsのコマンドを実行する
- 完成したZIPファイルを利用者へ返す
- `https://sdk-dev.game-fields.com/api/mcp`のGame Fields SDK toolsを利用する（Workでは通常`gameapp-dev`プラグイン経由）
...
このファイル単体から`index.html`、ZIP、チャット内プレビューなどを作っただけでは、Game Fieldsゲームは未完了です。作業用のローカルファイルは、後述する公式スターターを取得したフォルダ内に限って作成できますが、それを利用者向けの完成URLの代わりにしてはいけません。

画面モックの確認完了条件は、次のすべてです。
...
ただし、モック保存は正式ゲームの完成ではありません。最終的な提出完了条件は、さらに次のすべてです。

1. AppSetと正式クライアントを実装し、Room View／Command契約だけで動作させる
2. `npm run check`、`npm run demo`、`npm run diagnose:promotion`を成功させる
3. `npm run build:game-package`でクライアントとAppSetを一つの不変packageにする
4. `publish_game_package`へ`game-package/`の全ファイルをそのまま送り、`saved: true`を受け取る
5. SDKが再計算した`serverBundleSha256`と`appSetSourceSha256`がpackage manifestと一致する
6. 返された`packagePreviewUrl`を開き、本番と同じ共通Roomで候補revisionを確認できる

SDKへ接続できない、認証できない、保存できない、または各hashが一致しない場合は、ローカルプレビューを代替完成品として返さず「SDKへの提出は未完了」と明記してください。正式packageを保存する前に「ゲームが完成しました」と報告してはいけません。

### 2. SDKへログインし、既存環境へ戻るか新規作成する

Game Fields SDK toolsが未接続と判断する前に、Workでは上記の完全な語句でtool検索を実行してください。検索で`get_sdk_handshake`が見つかれば現在のチャットへ読み込み、質問より先に呼び出します。検索しても`gameapp-dev`自体が見つからない場合は、利用者へプラグインの追加・選択を案内してください。プラグインが候補に存在しない場合だけ、開発者モードをONにして`https://sdk-dev.game-fields.com/api/mcp`をOAuth Appとして追加する手順へ進みます。明示的な検索後に、`gameapp-dev`のほかのtoolは見つかるのに`get_sdk_handshake`だけがない場合に限って更新前と判定し、上記の定型文でプラグイン更新、新しいチャットでの再選択、このDownloadMeの再添付だけを案内して停止します。Codexでは同じURLをリモートMCPとして接続します。OAuth画面では、すでにSDK Portalへ接続済みのGame Fieldsアカウントで「この制作クライアントを許可」を押します。接続後、質問より先に上記の`get_sdk_handshake`を呼び、`accepted=true`を確認します。接続確認前にURL名やゲーム内容を尋ねてはいけません。
...
- `ref`が`sdk-starter-dev`
- `downloadMeVersion`が`10`
- `sdkVersion`が空でない
...
`mock/preview.json`のゲームID・表示名・説明・共通設定画面へ出す`settings`も今回の仕様へ更新し、`npm run check:mock`を成功させてください。共通設定画面は宣言項目だけを表示し、最大人数やラウンド数等は必要なゲームだけが追加します。`online-room`で必須なのは`platformRole: "time-limit"`を持つ制限時間1項目だけで、その`defaultValue`と`options`もゲーム側で決めます。ゲーム固有slotへ設定UIを複製せず、同期された`GameFieldsRoom`のViewを参照してください。`minimumPlayers`は公開時の実人数を指定し、1人Previewが必要な場合だけ`previewMinimumPlayers: 1`を併記します。その後、Game Fields SDKの`publish_mock` toolへ制作者slug、ゲーム情報、検査済み`mock/`ファイル一式を渡します。OAuth資格情報をファイル、Git、会話、コマンド引数へ展開してはいけません。スターターの`npm run publish:mock:legacy`は旧管理トークン互換用であり、Work／Codexの新規制作では使いません。
...
`diagnose:promotion`が失敗した場合は、問題をAppSet固有のルール違反と、SDKの説明・生成物・共通bridge不足に分けてください。SDK不足をゲーム固有の代替処理で隠さず、`SDK_REQUESTS.md`へ診断コードと不足契約を記録します。

検査成功後、`game-package/game-fields-package.json`に記録された`server.bundleSha256`と`appSetSourceSha256`を控えます。ここから提出・Preview・審査・昇格まで、AppSet原文を翻訳、補正、再生成してはいけません。修正が必要になった場合だけ、ソースを修正して全検査をやり直し、新しいrevisionとして提出します。

Work／Codexでは`game-package/`の全ファイルを`publish_game_package` toolへ渡してください。OAuth資格情報をファイル、Git、会話、コマンド引数へ展開してはいけません。スターターの`npm run publish:game-package`は旧管理トークン互換用です。toolの応答について、次をすべて確認します。

- `saved: true`
- `packageRevision`
- `serverBundleSha256`が手元のmanifestと一致
- `appSetSourceSha256`が手元のmanifestと一致
- `immutableAppSet: true`
- `packagePreviewUrl`

`packagePreviewUrl`は、未審査AppSetを隔離runnerで実行しながら、本番と同じRoom API、Redis、CAS、再接続、参加者同期を使う正式Previewです。モックPreviewを正式Previewの代用にしてはいけません。

最後に`submission/game-fields-submission.zip`も利用者へ返し、正式packageの保存revision、2つのhash、テスト結果、未実装事項を簡潔に説明してください。Game Fields側は、保存済みの同一revisionと同一hashだけをdevelopment、stableへ昇格します。昇格時にAppSetを改造する工程はありません。

...
ChatGPT WorkでGame Fields SDK toolsが見つからない場合、URL名やゲーム内容を先に質問してはいけません。まず利用者へ、チャットのプラグイン選択から`gameapp-dev`を追加・選択するよう案内してください。`gameapp-dev`が候補に表示されない場合だけ、ChatGPTの設定で開発者モードをONにし、プラグイン管理画面から上記MCP URLをOAuth Appとして追加する手順を案内します。接続後に`check_creator_url`が利用可能になったことを確認してから制作を始めてください。
...
ChatGPTはSDK接続状態を確認した後、あなた専用のSDK環境で使うURL名を聞きます。小文字英数字とハイフンで希望名を答えてください。URL確定後、ゲームの核を対話で決めます。
...
4. SDKから`saved: true`と`previewUrl`を受け取る
5. `previewUrl`をクリック可能な形で利用者へ案内する

SDKへ接続できない、認証できない、保存できない、または`previewUrl`を取得できない場合は、ローカルプレビューを代替完成品として返さず「SDKへの保存は未完了」と明記してください。SDK URLを取得する前に「ゲームを作成しました」「完成しました」と報告してはいけません。

### 2. SDKへログインし、制作者用URL名を確認・予約する

Game Fields SDK toolsが未接続なら、制作を開始したふりをせず、Workでは最初に`gameapp-dev`プラグインの追加・選択を案内してください。プラグインが候補に存在しない場合だけ、開発者モードをONにして`https://sdk-dev.game-fields.com/api/mcp`をOAuth Appとして追加する手順へ進みます。Codexでは同じURLをリモートMCPとして接続します。OAuth画面では、すでにSDK Portalへ接続済みのGame Fieldsアカウントで「この制作クライアントを許可」を押します。接続後、`check_creator_url`を呼べることを確認します。接続確認前にURL名やゲーム内容を尋ねてはいけません。

最初の利用者向け質問は、ゲーム仕様ではなく次の一問にしてください。

> あなた専用のGame Fields SDK環境で使うURL名を決めます。`yusuke-lab`のように、小文字英数字とハイフンで希望名を教えてください。

回答をURL用に小文字化し、空白や記号をハイフンへ変換したうえで、Game Fields SDKの`check_creator_url`を呼び出してください。
...
コマンドが返した`saved: true`と`previewUrl`を必ず確認してください。`mock/index.html`のローカルパス、ChatGPT内のプレビュー、ダウンロード用HTML、独自に推測したURLを、SDK発行URLとして案内してはいけません。

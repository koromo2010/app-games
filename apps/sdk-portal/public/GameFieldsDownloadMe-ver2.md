...
- Node.jsのコマンドを実行する
- 完成したZIPファイルを利用者へ返す
- `https://sdk-dev.game-fields.com/api/mcp`のGame Fields SDK toolsを利用する
...
Game Fields SDK toolsが未接続なら、制作を開始したふりをせず、WorkまたはCodexの接続画面から`https://sdk-dev.game-fields.com/api/mcp`を追加するよう案内してください。OAuth画面では、すでにSDK Portalへ接続済みのGame Fieldsアカウントで「この制作クライアントを許可」を押します。接続後、`check_creator_url`を呼べることを確認します。

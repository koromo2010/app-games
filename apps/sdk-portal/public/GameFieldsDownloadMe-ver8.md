...
ChatGPT WorkでGame Fields SDK toolsが見つからない場合、URL名やゲーム内容を先に質問してはいけません。まず利用者へ、チャットのプラグイン選択から`gameapp-dev`を追加・選択するよう案内してください。`gameapp-dev`が候補に表示されない場合だけ、ChatGPTの設定で開発者モードをONにし、プラグイン管理画面から上記MCP URLをOAuth Appとして追加する手順を案内します。

`gameapp-dev`のtoolが1件以上見えているのに`get_sdk_handshake`だけが見つからない場合、プラグインは更新前です。URL名やゲーム内容を質問せず、次の案内だけを表示して作業を止めてください。ほかのtoolで制作を続けたり、開発者モードで別接続を追加したりしてはいけません。
...
Game Fields SDK toolsが未接続なら、制作を開始したふりをせず、Workでは最初に`gameapp-dev`プラグインの追加・選択を案内してください。プラグインが候補に存在しない場合だけ、開発者モードをONにして`https://sdk-dev.game-fields.com/api/mcp`をOAuth Appとして追加する手順へ進みます。`gameapp-dev`のほかのtoolは見えるのに`get_sdk_handshake`だけがない場合は更新前と判定し、上記の定型文でプラグイン更新、新しいチャットでの再選択、このDownloadMeの再添付だけを案内して停止します。Codexでは同じURLをリモートMCPとして接続します。OAuth画面では、すでにSDK Portalへ接続済みのGame Fieldsアカウントで「この制作クライアントを許可」を押します。接続後、質問より先に上記の`get_sdk_handshake`を呼び、`accepted=true`を確認します。接続確認前にURL名やゲーム内容を尋ねてはいけません。
...
利用者がモックを承認したら、実装開始前に`get_game_module_requirements`を今回のslugとgameIdで呼び出してください。応答の`editableByAi`が`false`であることを確認し、`requiredModuleIds`に返されたmoduleをすべて使用します。AIはSDK-dev内部の分類や変更可能性を推測せず、この必須一覧だけを実装判断の正本にしてください。一覧を取得できない場合は本実装を始めず、SDK profileを確認できないことを利用者へ伝えてください。

...
import { NewSupportReportForm } from "./NewSupportReportForm";

export const dynamic = "force-dynamic";

export default async function NewSupportReportPage() {
  const account = await getSdkAccountSession().catch(() => null);
  if (!account) {
    redirect(
      "/api/account-link/start?returnTo=%2Fsupport%2Fnew",
    );
  }

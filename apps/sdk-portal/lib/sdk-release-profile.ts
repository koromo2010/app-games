...
export function sdkPortalMcpInstructions(origin?: string) {
  ...
  return `${profile.toolDescriptionPrefix} ${profile.connectorDisplayName}。最初にget_sdk_handshakeを呼び、添付された最新版プロファイルに記載されたrequiredCapabilitiesだけをそのまま送り、accepted=trueとenvironmentBindingを確認してください。`
    ...
    + "その後、Game Fieldsアカウント本人のSDK制作環境だけを操作します。accepted handshakeまたはread responseのaccountContext.accountRef・environmentを実際のMCPアカウントとして使い、ユーザーの発言、slug、表示名、Portal URLからアカウントを推測してはいけません。owner-bound writeの前に実際のMCPアカウントとtarget creatorを一度示し、同じaccountRefをexpectedAccountRefとして渡してください。新規draftのsystem-default初期module contractは人間確認を偽装せずそのまま利用し、構成を変更したい場合だけprepare_module_profile_updateでproposalを保存します。返されたreviewUrlで本人が承認するまでactive profileを変更したりprototypeへ進んだりしてはいけません。保存後はsavedとcreatorUrlを確認し、制作者トップを最初の案内リンクにしてください。";
}

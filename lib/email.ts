...
} from "@/lib/support-email-content";
...
export async function sendPasswordResetEmail(input: {
  ...
  const safeName = escapeEmailHtml(input.playerName);
...
export async function sendRecoveryEmailVerificationEmail(input: {
  ...
  const from = process.env.EMAIL_FROM?.trim() || "Game Fields <noreply@game-fields.com>";
  const safeName = escapeEmailHtml(input.playerName);
  const safeUrl = escapeEmailHtml(input.verificationUrl);
  const { error } = await resend.emails.send({
    from,
    to: input.email,
...
export async function sendOperationsAlertEmail(input: {
  ...
  const resend = new Resend(apiKey);
  const from = process.env.EMAIL_FROM?.trim() || "Game Fields <noreply@game-fields.com>";
  const content = buildOperationsEmailContent(input);
...
export async function sendSupportAdminNotificationEmail(input: {
  ...
  return sendOperationsAlertEmail({
    audience: "contacts",
    replyTo: input.replyTo,
    subject: supportEmailSubject(input.reference, input.title),
    lines: input.lines,
    idempotencyKey: input.idempotencyKey,
    supportReference: input.reference,
  });
}
...
export async function sendSupportReplyEmail(input: {
  ...
  const content = buildContactReplyEmailContent(input);
  const { error } = await resend.emails.send({
    from,
    to: input.to,
    subject: content.subject,
...
export async function sendCreatorSupportReplyEmail(input: {
  ...
  const from = process.env.EMAIL_FROM?.trim()
    || "Game Fields <noreply@game-fields.com>";
  const content = buildCreatorSupportReplyEmailContent(input);
  const { error } = await resend.emails.send({
    from,
    to: input.to,
    subject: content.subject,
...
export async function sendContactReceiptEmail(input: {
  ...
  const from = process.env.EMAIL_FROM?.trim()
    || "Game Fields <noreply@game-fields.com>";
  const content = buildContactReceiptEmailContent(input);
  const { error } = await resend.emails.send({
    from,
    to: input.to,
    subject: content.subject,

...
export type CompletionDiagnosticProjection =
  ...
  | { consumed: true; phase: "result"; httpStatus: number; diagnostic: CompletionDiagnostic }
  | { consumed: true; phase: "failure"; httpStatus: number | null; code: ProductionOwnerRestorationDiagnosticFailureCode };
...
export function projectCompletionDiagnosticResponse(status: number, payload: unknown): CompletionDiagnosticProjection {
  if (status < 200 || status >= 300) return { consumed: true, phase: "failure", httpStatus: status, code: responseFailure(payload) };
  const diagnostic = parseCompletionDiagnostic(payload);
  return diagnostic
    ? { consumed: true, phase: "result", httpStatus: status, diagnostic }
    : { consumed: true, phase: "failure", httpStatus: status, code: "OWNER_RESTORATION_DIAGNOSTIC_UPSTREAM_RESPONSE_INVALID" };
}

export function projectCompletionDiagnosticTransportFailure(): CompletionDiagnosticProjection {
  return { consumed: true, phase: "failure", httpStatus: null, code: "OWNER_RESTORATION_DIAGNOSTIC_UPSTREAM_UNAVAILABLE" };
}

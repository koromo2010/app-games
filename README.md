...
Every game with a debug mode must place `app/components/DebugModeButton.tsx` in its top bar for the host. Debug controls are available only when the player's registered recovery email matches an account registered in the site administration screen. The shared `/api/debug-auth` check and mutation APIs enforce the same rule; players cannot grant the permission to themselves. While a debug game is active, the shared control can abort it back to the same room's pre-game state without removing participants.

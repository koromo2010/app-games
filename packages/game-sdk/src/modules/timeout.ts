...
/**
 * DEBUG dummies are platform-owned participants whose IDs are generated by the
 * shared room lifecycle as `debug:<requestId>`. One operator controls several
 * such seats, so normal consecutive-timeout penalties must never apply to them.
 */
export function gameSdkPlayerTimeoutIsExempt(participantId: string): boolean {
  return participantId.startsWith("debug:");
}
...
export function recordGameSdkPlayerTimeout<TId extends string>(
  ...
  if (gameSdkPlayerTimeoutIsExempt(participantId)) return state;
  const current = state.statuses[participantId]
    ?? { consecutiveTimeouts: 0, reducedTime: false };
...
export function gameSdkPlayerTimeLimitSeconds<TId extends string>(
  ...
  if (
    !participantId
    || gameSdkPlayerTimeoutIsExempt(participantId)
    || !state.statuses[participantId]?.reducedTime
  ) {
    return baseSeconds;
  }

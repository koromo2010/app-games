...
export function useHodoaiRoomActions(params: Params) {
  ...
  const createRoom = async () => {
    ...
    try {
      ...
      const nextRoom: HodoaiRoom = { code: makeRoomCode(), revision: 0, hostId: session.id, sorterId: session.id, ownerId, passphrase: passphrase.trim(), phase: "lobby", players: [host], ...defaults, ...defaultHodoaiScoring, playerTimeouts: { [session.id]: { consecutiveTimeouts: 0, reducedTime: false } }, playerTimeoutNotice: null, debugMode: false, debugReplayEnabled: false, debugLog: [], gameNumber: 1, gameStartedAt: null, round: 1, theme: null, cards: [], values: {}, clues: {}, clueHistory: [], order: [], totalPoints: 0, history: [], phaseStartedAt: null, createdAt: now, updatedAt: now };

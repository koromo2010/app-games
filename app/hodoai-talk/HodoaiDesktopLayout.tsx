...
export function HodoaiDesktopLayout({ controller }: { controller: HodoaiController }) {
  ...
  const { runAction, createRoom, listRooms, joinRoom, dissolveRoom, leaveRoom, updateConfig, returnToRoom, setPassphrase, setJoinCode, setRulesOpen } = actions;
  ...
  return (
    <main className={`min-h-screen bg-[radial-gradient(circle_at_top,#0e7490_0%,#172033_35%,#020617_75%)] text-white ${gameTopBannerOffsetClass}`}>
      ...
      {room.phase === "lobby" && <div className="mx-auto w-full px-4 pt-4"><GameLoungeVisual gameId="hodoai" /></div>}
      <div className="mx-auto grid w-full gap-4 px-4 py-5 lg:grid-cols-[280px_minmax(0,1fr)]">

...
export function HodoaiTalkGame() {
  ...
  const createRoom = async () => {
    ...
    try {
      ...
      const nextRoom: HodoaiRoom = {
        ...
        ...defaults, debugMode: false, debugReplayEnabled: false, debugLog: [], gameNumber: 1, round: 1, theme: null, cards: [], values: {}, clues: {}, order: [], totalPoints: 0, history: [], phaseStartedAt: null, createdAt: now, updatedAt: now,
      };
      ...
      <main className={`min-h-screen bg-[radial-gradient(circle_at_top,#164e63_0%,#1e293b_42%,#020617_82%)] px-4 pb-8 text-white ${gameTopBannerOffsetClass}`}>
        ...
        <div className="mx-auto max-w-4xl">
          <section className="mt-5 overflow-hidden rounded-3xl border border-white/10 bg-slate-950/80 shadow-2xl">
            <div className="bg-gradient-to-r from-sky-400 via-amber-300 to-fuchsia-400 px-6 py-8 text-slate-950"><p className="text-xs font-black uppercase tracking-[0.28em]">Online room game</p><h1 className="mt-2 text-4xl font-black sm:text-6xl">ことばで数ならべ</h1><p className="mt-3 font-bold">配られた数字カードを指定テーマのことばで表し、全カードを小さい順に並べる協力ゲーム。</p></div>
            ...
            {showChoices && <div className="border-t border-white/10 p-6"><h2 className="font-black">参加できる部屋</h2><div className="mt-3 grid gap-2 sm:grid-cols-2">{choices.map((choice) => <button key={choice.code} type="button" onClick={() => { setJoinCode(choice.code); void joinRoom(choice.code); }} className="rounded-xl border border-white/10 bg-white/[0.05] p-4 text-left"><span className="font-mono text-lg font-black text-cyan-300">{choice.code}</span><span className="ml-3 font-bold">{choice.hostName}</span><span className="mt-1 block text-xs text-slate-400">{choice.playerCount}人・1人{choice.cardsPerPlayer}枚・{choice.roundsTotal}ラウンド・合言葉{choice.hasPassphrase ? "あり" : "なし"}</span></button>)}</div></div>}
        ...
        <GameRulesDialog open={rulesOpen} title="ことばで数ならべのルール" onClose={() => setRulesOpen(false)}>
          ...
          <ol className="mt-3 list-decimal space-y-2 pl-5"><li>各自に設定枚数の秘密の数字カードが配られます。</li><li>カードごとに、指定テーマに沿った短いことばを1つ提出します。数字や数の直接説明は禁止です。</li><li>全員のことばが公開されたら相談し、ホストが全カードを小さい順へ並べます。</li><li>答え合わせで数字を公開し、並び違いの数から得点します。</li></ol>
  ...
  return (
    <main className={`min-h-screen bg-[radial-gradient(circle_at_top,#0e7490_0%,#172033_35%,#020617_75%)] text-white ${gameTopBannerOffsetClass}`}>
      ...
      <GameRulesDialog open={rulesOpen} title="ことばで数ならべのルール" onClose={() => setRulesOpen(false)}><p>各自に配られた数字カードを、数字を直接説明せず、指定テーマに沿ったことばだけで伝え、全カードを小さい順に並べる協力ゲームです。</p><ol className="mt-3 list-decimal space-y-2 pl-5"><li>各自に設定枚数のカードが配られます。</li><li>カードごとにテーマに沿った短いことばを1つ提出します。</li><li>相談してホストが全カードを小さい順へ並べ、数字を公開して採点します。</li></ol><p className="mt-3 text-amber-200">数字や数の直接説明は禁止です。未提出は時間切れでパス、並べ替え時間切れでは現在順を自動採点します。</p></GameRulesDialog>
      <div className="mx-auto grid max-w-6xl gap-4 px-4 py-5 lg:grid-cols-[280px_minmax(0,1fr)]">
        ...
        <div className="space-y-4">
          ...
          {room.phase === "clue" && <section className="rounded-2xl border border-white/10 bg-slate-950/80 p-6"><div className="flex flex-wrap items-center justify-between gap-3"><div><h2 className="text-xl font-black">数字カードをことばに変える</h2><p className="mt-1 text-sm text-slate-400">全カードの提出 {submittedCount}/{room.cards.length}枚</p></div>{room.phaseStartedAt && <GamePhaseTimer key={room.phaseStartedAt} durationSeconds={room.clueTimeLimitSeconds} startedAt={room.phaseStartedAt} label="提出時間" />}</div><p className="mt-4 rounded-xl border border-amber-300/25 bg-amber-300/10 p-3 text-sm font-bold text-amber-100">お題「{room.theme?.title}」に沿った短いことばだけで伝えてください。数字や「半分くらい」など数の直接説明は禁止です。</p><div className="mt-4 grid gap-4 sm:grid-cols-2">{ownCards.map((card) => <div key={card.id} className="rounded-2xl border border-white/10 bg-white/[0.05] p-4"><div className="rounded-xl border border-amber-300/40 bg-amber-300/10 p-4 text-center"><p className="text-xs font-bold text-amber-100">あなたのカード {card.cardNumber}</p><p className="mt-1 text-6xl font-black text-amber-300">{room.values[card.id]}</p></div>{room.clues[card.id] ? <div className="mt-3 rounded-xl border border-emerald-300/30 bg-emerald-300/10 p-3"><p className="font-black text-emerald-100">提出済み：{room.clues[card.id]}</p></div> : <div className="mt-3"><label className="block text-sm font-bold">このカードを表すことば<input value={clueDrafts[card.id] ?? ""} maxLength={40} onChange={(event) => setClueDrafts((current) => ({ ...current, [card.id]: event.target.value }))} onKeyDown={(event) => { if (event.key === "Enter") submitClue(card.id); }} className="mt-2 w-full rounded-xl border border-white/15 bg-white/10 px-4 py-3 text-lg text-white outline-none focus:border-amber-300" /></label><button type="button" disabled={isSaving} onClick={() => submitClue(card.id)} className="mt-3 w-full rounded-xl bg-amber-300 px-4 py-3 font-black text-slate-950 disabled:opacity-50">このことばを提出</button></div>}</div>)}</div><p className="mt-4 text-center text-sm text-slate-300">自分の全カードを提出したら、ほかのプレイヤーを待ちます。提出中は他人のことばは見えません。</p>{isHost && room.debugMode && <button type="button" onClick={() => void runAction({ type: "debug-fill-clues", actorId: playerId, round: room.round })} className="mt-4 w-full rounded-xl border border-cyan-300/30 bg-cyan-300/10 px-4 py-3 text-sm font-black text-cyan-100">デバッグ：未提出のことばを自動入力</button>}</section>}

          {room.phase === "arrange" && <section className="rounded-2xl border border-white/10 bg-slate-950/80 p-6"><div className="flex flex-wrap items-center justify-between gap-3"><div><h2 className="text-xl font-black">全カードを小さい順に並べる</h2><p className="mt-1 text-sm text-slate-400">公開されたことばを手がかりに相談し、ホストが順番を操作します。</p></div>{room.phaseStartedAt && <GamePhaseTimer key={room.phaseStartedAt} durationSeconds={room.arrangeTimeLimitSeconds} startedAt={room.phaseStartedAt} label="相談時間" />}</div>{isHost && room.debugMode && <button type="button" onClick={() => void runAction({ type: "debug-sort", actorId: playerId, round: room.round })} className="mt-4 w-full rounded-xl border border-cyan-300/30 bg-cyan-300/10 px-4 py-3 text-sm font-black text-cyan-100">デバッグ：正解順に並べる</button>}<ol className="mt-4 space-y-2">{orderedCards.map(({ card, player }, index) => <li key={card.id} className="flex items-center gap-3 rounded-xl border border-white/10 bg-white/[0.05] p-3"><span className="w-7 text-center font-black text-cyan-300">{index + 1}</span><div className="min-w-0 flex-1"><p className="font-black">{room.clues[card.id]}</p><p className="text-xs text-slate-400">{player.name}・カード{card.cardNumber}{room.debugMode && isHost && typeof room.values[card.id] === "number" ? `・${room.values[card.id]}` : ""}</p></div>{isHost && <div className="flex gap-1"><button type="button" disabled={index === 0 || isSaving} onClick={() => moveClue(index, -1)} aria-label={`${player.name}のカード${card.cardNumber}を上へ`} className="rounded-lg border border-white/15 px-3 py-2 disabled:opacity-30">↑</button><button type="button" disabled={index === orderedCards.length - 1 || isSaving} onClick={() => moveClue(index, 1)} aria-label={`${player.name}のカード${card.cardNumber}を下へ`} className="rounded-lg border border-white/15 px-3 py-2 disabled:opacity-30">↓</button></div>}</li>)}</ol>{isHost ? <button type="button" disabled={isSaving} onClick={() => void runAction({ type: "score-round", actorId: playerId, round: room.round })} className="mt-5 w-full rounded-xl bg-fuchsia-400 px-4 py-3 font-black text-fuchsia-950 disabled:opacity-50">この順番で数字を公開する</button> : <p className="mt-4 rounded-xl bg-white/[0.05] p-3 text-center text-sm font-bold text-slate-300">ホストが順番を確定するまでお待ちください。</p>}</section>}

          {room.phase === "result" && latestResult && <section className="rounded-2xl border border-white/10 bg-slate-950/80 p-6"><h2 className="text-2xl font-black">答え合わせ</h2><div className="mt-4 space-y-2">{latestResult.order.map((id, index) => { const card = latestResult.cards.find((item) => item.id === id); const player = room.players.find((item) => item.id === card?.ownerId); return <div key={id} className="grid grid-cols-[2rem_1fr_auto] items-center gap-3 rounded-xl border border-white/10 bg-white/[0.05] p-3"><span className="text-center font-black text-cyan-300">{index + 1}</span><div><p className="font-black">{latestResult.clues[id]}</p><p className="text-xs text-slate-400">{player?.name}・カード{card?.cardNumber}</p></div><span className="text-2xl font-black text-amber-300">{latestResult.values[id]}</span></div>; })}</div><div className="mt-5 rounded-2xl bg-gradient-to-r from-cyan-400 to-amber-300 p-5 text-center text-slate-950"><p className="font-black">このラウンド +{latestResult.points}点</p><p className="mt-1 text-sm font-bold">並び違い {latestResult.inversions}組・合計 {room.totalPoints}点</p></div>{room.round >= room.roundsTotal && <p className="mt-5 text-center text-lg font-black">{hodoaiFinalMessage(room.totalPoints, room.roundsTotal * 3)}</p>}{isHost ? room.round >= room.roundsTotal ? <RoomResultActions disabled={isSaving} onPlayAgain={() => void runAction({ type: "reset-game", actorId: playerId })} onDissolve={() => void dissolveRoom()} /> : <button type="button" disabled={isSaving} onClick={() => void runAction({ type: "next-round", actorId: playerId, round: room.round })} className="mt-5 w-full rounded-xl bg-amber-300 px-4 py-3 font-black text-slate-950 disabled:opacity-50">次のラウンドへ</button> : <p className="mt-4 text-center text-sm font-bold text-slate-300">ホストの操作を待っています。</p>}</section>}

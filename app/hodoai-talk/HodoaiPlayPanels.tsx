...
export function HodoaiPlayPanels({ room, playerId, isHost, isSaving, runAction, updateConfig }: Props) {
  ...
  const phaseDurationSeconds = room.phase === "clue"
    ? room.clueTimeLimitSeconds
    : room.phase === "arrange"
      ? room.arrangeTimeLimitSeconds
      : 0;
  ...
  const actionWindow = useGameplayActionWindow({
    plan: {
      scope: { roomCode: room.code, generation: `${room.gameNumber}:${room.round}:${room.phaseStartedAt ?? 0}`, phase: room.phase },
  ...
  const dispatchClues = useCallback(async (action: HodoaiRoomAction) => {
    const result = await dispatchManual({
      actionKey: `submit-clues:${playerId}`,
      execute: async () => {
        const saved = await runAction(action);
        if (!saved) throw new Error("HODOAI_CLUE_SUBMISSION_NOT_SAVED");
        return saved;
      },
      classifyError: () => "ambiguous",
    });
    return result.kind === "accepted" ? result.value : null;
  }, [dispatchManual, playerId, runAction]);
  const submitClues = () => {
    ...
    setClueError("");
    void dispatchClues({ type: "submit-clues", actorId: playerId, round: room.round, clues }).then((saved) => {
  ...
  useEffect(() => {
    if (room.phase !== "clue" || !room.phaseStartedAt || room.clueTimeLimitSeconds <= 0 || actionWindow.remainingSeconds !== 0) return;
    const missingCards = ownCards.filter((card) => !room.clues[card.id]);
    const clues = Object.fromEntries(missingCards.map((card) => [card.id, normalizeHodoaiClue(clueDrafts[card.id] ?? "")]));
    ...
    void dispatchClues({ type: "submit-timeout-clues", actorId: playerId, round: room.round, clues }).then((saved) => {
      if (!saved) return;
      setClueDrafts((current) => Object.fromEntries(Object.entries(current).filter(([cardId]) => !saved.clues[cardId])));
    });
  }, [actionWindow.remainingSeconds, clueDrafts, dispatchClues, ownCards, playerId, room]);
  return <>

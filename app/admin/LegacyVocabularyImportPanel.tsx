...
function importErrorMessage(code: string | undefined) {
  if (code === "LEGACY_VOCABULARY_CATALOG_NOT_FOUND") return "開発DBに旧単語カタログが見つかりません。取込元の接続設定を確認してください。";
...
export function LegacyVocabularyImportPanel({ onAuthExpired }: { onAuthExpired: () => void }) {
  ...
  return <section className="mb-8 rounded-2xl border border-amber-300/20 bg-amber-300/[0.06] p-5">
    <div className="flex flex-wrap items-start justify-between gap-4"><div><p className="text-xs font-bold uppercase tracking-wide text-amber-300">Preview限定・初回移行</p><h2 className="mt-1 text-xl font-black">旧単語カタログを共通DBへ取り込む</h2><p className="mt-2 max-w-3xl text-sm leading-6 text-slate-300">開発DBに残っている審査済み単語だけを、共通単語DBのワードウルフ候補へ追加します。本番アプリDBは変更しません。途中で止まっても同じボタンから再開できます。</p></div><button type="button" onClick={() => void loadStatus()} disabled={loading || importing} className="rounded-lg border border-white/15 px-3 py-2 text-sm font-bold disabled:opacity-40">件数を再確認</button></div>
    {status && <dl className="mt-4 grid gap-3 text-sm sm:grid-cols-3"><div className="rounded-xl bg-black/20 p-3"><dt className="text-slate-400">旧DBの対象</dt><dd className="mt-1 text-lg font-black">{status.sourceActiveCount.toLocaleString("ja-JP")}語</dd></div><div className="rounded-xl bg-black/20 p-3"><dt className="text-slate-400">共通DBの有効単語</dt><dd className="mt-1 text-lg font-black">{status.targetActiveWordCount.toLocaleString("ja-JP")}語</dd></div><div className="rounded-xl bg-black/20 p-3"><dt className="text-slate-400">ワードウルフ候補</dt><dd className="mt-1 text-lg font-black">{status.targetWordwolfEligibleCount.toLocaleString("ja-JP")}語</dd></div></dl>}

...
function importErrorMessage(code: string | undefined) {
  if (code === "LEGACY_VOCABULARY_CATALOG_NOT_FOUND") return "現在の開発DBに旧単語カタログがありません。app-games-devへ旧カタログ読取URLを設定してください。";
  if (code === "LEGACY_VOCABULARY_IMPORT_PREVIEW_ONLY") return "この取込はdevelop開発環境だけで実行できます。";
...
export function LegacyVocabularyImportPanel({ onAuthExpired }: { onAuthExpired: () => void }) {
  ...
  return <section className="mb-8 rounded-2xl border border-amber-300/20 bg-amber-300/[0.06] p-5">
    <div className="flex flex-wrap items-start justify-between gap-4"><div><p className="text-xs font-bold uppercase tracking-wide text-amber-300">develop限定・初回移行</p><h2 className="mt-1 text-xl font-black">旧単語カタログを共通DBへ取り込む</h2><p className="mt-2 max-w-3xl text-sm leading-6 text-slate-300">旧DBの審査済み単語だけを読み取り、共通単語DBのワードウルフ候補へ追加します。旧DBの内容は変更しません。途中で止まっても同じボタンから再開できます。</p></div><button type="button" onClick={() => void loadStatus()} disabled={loading || importing} className="rounded-lg border border-white/15 px-3 py-2 text-sm font-bold disabled:opacity-40">件数を再確認</button></div>

...
export function SiteAdminPanel({ showPreviewVocabularyMigrations, releaseManagementMode, showOriginalDataPreservation, showProductionPrivateWorkspaceImport }: {
  ...
  return (
    <main className="min-h-screen bg-slate-950 text-white">
      <header className="border-b border-white/10 bg-slate-900/90"><div className="mx-auto flex max-w-6xl items-center justify-between gap-4 px-4 py-4"><div><p className="text-xs font-bold uppercase tracking-[0.2em] text-cyan-300">Game Fields Admin</p><h1 className="text-2xl font-black">サイト管理</h1></div><div className="flex gap-2">{showProductionPrivateWorkspaceImport && session?.scope === "full" && <Link href="/site-admin/runtime-operations/production-private-workspace-import/moi-lab2" className="rounded-lg border border-rose-300/30 px-3 py-2 text-sm font-bold text-rose-100 hover:bg-rose-300/10">Production import</Link>}<Link href="/games" className="rounded-lg border border-white/15 px-3 py-2 text-sm font-bold hover:bg-white/10">サイトを見る</Link><button type="button" onClick={() => void logout()} className="rounded-lg border border-white/15 px-3 py-2 text-sm font-bold text-slate-300 hover:bg-white/10">ログアウト</button></div></div></header>

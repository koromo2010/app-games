...
import { AdminSupportInboxPanel } from "./AdminSupportInboxPanel";
import { GameOperationsPanel } from "./GameOperationsPanel";
import { VocabularyDraftsPanel } from "./VocabularyDraftsPanel";
import { ReleaseManagementPanel } from "./ReleaseManagementPanel";
import { OriginalDataPreservationPanel } from "./OriginalDataPreservationPanel";

type ScreenState = "checking" | "login" | "mfa" | "recovery-codes" | "settings";
type LoginMethod = "account" | "master";
type AdminSection = "dashboard" | "support" | "releases" | "original-data-preservation" | "site-settings" | "games" | "vocabulary" | "hyperparameters" | "accounts" | "audit";
...
export function SiteAdminPanel({ showPreviewVocabularyMigrations, releaseManagementMode, showOriginalDataPreservation, showDevelopmentMigration011Operator, showDevelopmentPrivateWorkspaceImport, showProductionPrivateWorkspaceImport, initialLogoutResult, showInlineLocaleSwitcher = true }: {
  ...
  showDevelopmentPrivateWorkspaceImport: boolean;
  showProductionPrivateWorkspaceImport: boolean;
  ...
  useEffect(() => {
    ...
    void fetch("/api/admin/site-settings", { cache: "no-store", signal: controller.signal }).then(async (response) => {
      ...
      if (response.ok && data?.settings && data.session) { setSettings(data.settings); setSession(data.session); setSection(data.session.scope === "recovery" || data.session.method === "recovery-code" ? "accounts" : "dashboard"); setScreen("settings"); return; }
      setScreen("login");
  ...
  const loadAuthenticatedSettings = async () => {
    ...
    setSettings(data.settings); setSession(data.session); setEmail(""); setRecoveryCode(""); setSection(data.session.method === "recovery-code" ? "accounts" : "dashboard"); setScreen("settings");
  };
  ...
  return (
    <main className="min-h-screen bg-slate-950 text-white">
      <header className="border-b border-white/10 bg-slate-900/90"><div className="mx-auto flex max-w-6xl flex-col gap-4 px-4 py-4 sm:flex-row sm:items-center sm:justify-between"><div className="shrink-0"><p className="text-xs font-bold uppercase tracking-[0.2em] text-cyan-300">Game Fields Admin</p><h1 className="text-2xl font-black">サイト管理</h1></div><div className="flex w-full flex-wrap items-center gap-2 sm:w-auto sm:justify-end" data-site-admin-header-actions>{showInlineLocaleSwitcher && <LocaleSwitcher className="shrink-0" />}{showProductionPrivateWorkspaceImport && session?.scope === "full" && <Link href="/site-admin/runtime-operations/production-private-workspace-import/moi-lab2" className="rounded-lg border border-rose-300/30 px-3 py-2 text-sm font-bold text-rose-100 hover:bg-rose-300/10">Production import</Link>}{showDevelopmentPrivateWorkspaceImport && session?.scope === "full" && <Link href="/site-admin/runtime-operations/development-private-workspace-import" className="rounded-lg border border-cyan-300/30 px-3 py-2 text-sm font-bold text-cyan-100 hover:bg-cyan-300/10">Private import</Link>}{showDevelopmentMigration011Operator && session?.scope === "full" && <Link href="/site-admin/runtime-operations/sdk-migration-011" className="rounded-lg border border-amber-300/30 px-3 py-2 text-sm font-bold text-amber-100 hover:bg-amber-300/10">Migration 011</Link>}<Link href="/games" className="rounded-lg border border-white/15 px-3 py-2 text-sm font-bold hover:bg-white/10">サイトを見る</Link><form method="post" action={siteAdminLogoutPath}><button type="submit" className="rounded-lg border border-white/15 px-3 py-2 text-sm font-bold text-slate-300 hover:bg-white/10">ログアウト</button></form></div></div></header>
      ...
      <nav className="border-b border-white/10 bg-slate-900/60" aria-label="管理画面メニュー"><div className="mx-auto flex max-w-6xl gap-1 overflow-x-auto px-4 py-2">{([['dashboard', 'ダッシュボード'], ['support', '問い合わせ・報告'], ...(releaseManagementMode ? [['releases', '昇格管理'] as const] : []), ...(showOriginalDataPreservation ? [['original-data-preservation', '元データ確保'] as const] : []), ['site-settings', 'サイト設定'], ['games', 'ゲーム公開管理'], ['vocabulary', '単語候補'], ['hyperparameters', 'ハイパラ管理'], ['accounts', '管理者アカウント'], ['audit', '監査ログ']] as const).filter(([value]) => session?.scope === "full" || value === "dashboard" || value === "accounts" || value === "audit").map(([value, label]) => <button key={value} type="button" aria-current={section === value ? "page" : undefined} onClick={() => setSection(value)} className={`whitespace-nowrap rounded-lg px-4 py-2 text-sm font-bold transition ${section === value ? "bg-cyan-300 text-slate-950" : "text-slate-300 hover:bg-white/10 hover:text-white"}`}>{label}</button>)}</div></nav>
      {section === "dashboard" && <AdminDashboard onAuthExpired={authExpired} />}
      {section === "support" && <AdminSupportInboxPanel onAuthExpired={authExpired} />}
      {section === "releases" && releaseManagementMode && <ReleaseManagementPanel mode={releaseManagementMode} onAuthExpired={authExpired} />}
      {section === "original-data-preservation" && showOriginalDataPreservation && <OriginalDataPreservationPanel onAuthExpired={authExpired} />}

...
export function VocabularyEvaluationsPanel({ onAuthExpired }: { onAuthExpired: () => void }) {
  ...
  return <section className="mt-10 border-t border-white/10 pt-8">
    <div className="flex flex-wrap items-end justify-between gap-3">
      <div><h2 className="text-2xl font-black">LLM評価レビュー</h2><p className="mt-1 text-sm leading-6 text-slate-400">直近100件のOK・rejectを表示します。管理者票はLLM判定を残したまま、本番RAGの「お題評価」にも1票として反映します。「距離評価」には混ぜず、正式採用は下の候補レビューで行います。</p></div>
    ...
    {loading ? <p className="mt-6 text-sm text-slate-400">読み込み中…</p> : visibleEvaluations.length === 0 ? <p className="mt-6 rounded-xl border border-white/10 bg-white/[0.04] p-5 text-sm text-slate-400">該当するLLM評価はありません。</p> : <div className="mt-6 grid gap-4 lg:grid-cols-2">{visibleEvaluations.map((evaluation) => <article key={evaluation.id} className="rounded-2xl border border-white/10 bg-white/[0.05] p-5">
      ...
      <div className="mt-4 flex gap-2"><button type="button" disabled={!votingEnabled || Boolean(voting)} aria-pressed={evaluation.myVote === "accept"} onClick={() => void vote(evaluation, "accept")} className={`rounded-lg px-4 py-2 text-sm font-black disabled:opacity-30 ${evaluation.myVote === "accept" ? "bg-emerald-200 text-slate-950" : "border border-emerald-300/30 text-emerald-200"}`}>OKに投票</button><button type="button" disabled={!votingEnabled || Boolean(voting)} aria-pressed={evaluation.myVote === "reject"} onClick={() => void vote(evaluation, "reject")} className={`rounded-lg px-4 py-2 text-sm font-bold disabled:opacity-30 ${evaluation.myVote === "reject" ? "bg-rose-200 text-slate-950" : "border border-rose-300/30 text-rose-200"}`}>NGに投票</button></div>

...
export function TahoiyaCatalogMigrationPanel({ onAuthExpired }: { onAuthExpired: () => void }) {
  ...
  return <section className="mb-8 rounded-2xl border border-violet-300/20 bg-violet-300/[0.06] p-5">
    ...
    {status && <dl className="mt-4 grid gap-3 text-sm sm:grid-cols-3"><div className="rounded-xl bg-black/20 p-3"><dt className="text-slate-400">旧Redisの有効お題</dt><dd className="mt-1 text-lg font-black">{status.sourceValidCount.toLocaleString("ja-JP")}件</dd></div><div className="rounded-xl bg-black/20 p-3"><dt className="text-slate-400">共通DBのお題</dt><dd className="mt-1 text-lg font-black">{status.targetTopicCount.toLocaleString("ja-JP")}件</dd></div><div className="rounded-xl bg-black/20 p-3"><dt className="text-slate-400">移行状態</dt><dd className="mt-1 text-lg font-black">{status.complete ? "完了" : "未完了"}</dd></div></dl>}
    ...
    <button type="button" onClick={() => void migrate()} disabled={loading || migrating || !status || status.sourceValidCount === 0 || status.complete} className="mt-4 rounded-xl bg-violet-300 px-5 py-3 text-sm font-black text-slate-950 disabled:opacity-40">{migrating ? "移行中…" : status?.complete ? "移行完了" : "移行を開始"}</button>
  </section>;
}

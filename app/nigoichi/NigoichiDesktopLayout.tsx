...
export function NigoichiDesktopLayout({ controller }: { controller: NigoichiController }) {
  ...
  const {
    setPassphrase, setJoinCode, setNewPlayerCapacity,
    ...
    <main className={`min-h-screen bg-[radial-gradient(circle_at_top,#4338ca_0%,#172033_38%,#020617_78%)] text-white ${gameTopBannerOffsetClass}`}>
      <GameTopBanner eyebrow="WORD OUT" title={<>ワードアウト <span className="font-mono text-base text-amber-300">#{room.code}</span></>}>
        ...
        <GameTopMenu>
          ...
          <OnlineRoomSpectatorLink game="nigoichi" code={room.code} />
      ...
      {room.phase === "lobby" && <div className="mx-auto w-full px-4 pt-4"><GameLoungeVisual gameId="nigoichi" /></div>}

...
import { playerGameResultLabel } from "@/lib/player-result-presentation";

type Option = { value: PlayerStatsGameFilter; label: string };
type Props = { stats: PlayerStatsResponse | null; options: readonly Option[]; gameDisplayMetadata: GameDisplayMetadataSnapshot; selectedGame: PlayerStatsGameFilter; isLoading: boolean; onGameChange: (game: PlayerStatsGameFilter) => void; onRefresh: () => void };

export function LobbyStatsPanel({ stats, options, gameDisplayMetadata, selectedGame, isLoading, onGameChange, onRefresh }: Props) {
  ...
  return <div className="rounded-lg border border-white/10 bg-white/[0.96] p-4 shadow-[0_18px_50px_rgba(15,23,42,0.18)]">
    ...
    <div className="mt-4"><p className="text-xs font-semibold uppercase text-slate-500">Recent</p>{isLoading ? <p className="mt-2 text-sm text-slate-500">{t("stats.loading")}</p> : stats?.recent.length ? <div className="mt-2 space-y-2">{stats.recent.slice(0, 5).map((result) => <div key={result.id} className="flex items-center justify-between rounded-lg bg-slate-50 px-3 py-2 text-sm"><div><p className="font-semibold text-slate-800">{playerGameResultLabel(result, locale)}</p><p className="text-xs text-slate-500">{formatDate(result.finishedAt)} / {resolveGameDisplayMetadata(gameDisplayMetadata, result.gameType, locale).displayName}</p></div><span className={`rounded-md px-2 py-1 text-xs font-bold ${result.draw ? "bg-amber-100 text-amber-700" : result.won ? "bg-cyan-100 text-cyan-700" : "bg-rose-100 text-rose-700"}`}>{result.draw ? "DRAW" : result.won ? "WIN" : "LOSE"}</span></div>)}</div> : <p className="mt-2 text-sm text-slate-500">{t("stats.empty")}</p>}</div>
  </div>;
}

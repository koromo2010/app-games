...
function LobbyGameCard({ game, operation, activeRoom, isLoggedIn, onLoginRequired, onRememberWordWolf }: { game: GameCatalogEntry; operation: GameOperation; activeRoom?: ActiveRoom; isLoggedIn: boolean; onLoginRequired: () => void; onRememberWordWolf: () => void }) {
  ...
  const card = <article className={`group h-full rounded-lg border p-3 shadow-[0_14px_38px_rgba(15,23,42,0.18)] transition hover:-translate-y-0.5 hover:shadow-[0_20px_48px_rgba(15,23,42,0.28)] ${active ? "border-cyan-300 bg-gradient-to-br from-cyan-950 via-slate-900 to-fuchsia-950 ring-2 ring-cyan-300/60" : "border-white/10 bg-white/[0.96]"}`}>
    <div className={`relative h-20 overflow-hidden rounded-md bg-gradient-to-br ${game.accent} ${active ? "ring-2 ring-white/50" : ""}`}><span aria-hidden="true" className="absolute inset-x-5 bottom-0 h-14 rounded-full bg-white/50 blur-2xl" /><Image src={game.icon} alt="" width={104} height={104} unoptimized className="absolute -bottom-4 left-1/2 h-[104px] w-[104px] -translate-x-1/2 -rotate-3 rounded-[27px] drop-shadow-[0_12px_14px_rgba(15,23,42,0.38)] transition duration-300 group-hover:-rotate-1 group-hover:scale-105" /></div>

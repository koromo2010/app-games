...
export async function POST(request: Request, context: { params: Promise<{ target: string }> }) {
  try {
    ...
    if (body.length > 512) return Response.json({ error: "CREATOR_RECOVERY_INPUT_INVALID" }, { status: 400, headers });
    const url = new URL(`/api/internal/recovery/creator-quarantine/${encodeURIComponent(target)}/execute`, sdkPromotionInternalBaseUrl()).toString();
    const response = await fetch(url, {
      method: "POST",
      headers: { ...sdkServiceHeaders("POST", url, { environment: "production" }), "Content-Type": "application/json" },
      body,
      cache: "no-store",
    });

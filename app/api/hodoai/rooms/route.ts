...
export async function GET(request: Request) {
  ...
  try {
    if (code) {
      ...
      return Response.json({ room: sanitizeHodoaiRoom(room, playerId) });
    }
    if (playerId) {
      if (!(await loadStoredPlayerSession(playerId))) return Response.json({ error: "Login required" }, { status: 401 });
      const room = await loadHodoaiPlayerActiveRoom(playerId);
      return Response.json({ room: room ? sanitizeHodoaiRoom(room, playerId) : null });
    }
...
export async function PATCH(request: Request) {
  ...
  try {
    ...
    let action = body.action as HodoaiRoomAction;
    const session = await loadStoredPlayerSession(action.actorId);
...
export async function DELETE(request: Request) {
  ...
  try {
    if (code) {
      if (!actorId) return Response.json({ error: "actorId is required" }, { status: 400 });
      if (!(await loadStoredPlayerSession(actorId))) return Response.json({ error: "Login required" }, { status: 401 });
      await deleteStoredHodoaiRoom(code, actorId);
      return Response.json({ ok: true });
    }
    if (ownerId) {
      if (!fallbackHostId || !(await loadStoredPlayerSession(fallbackHostId))) return Response.json({ error: "Login required" }, { status: 401 });
      return Response.json({ ok: true, deleted: await deleteHostedHodoaiRooms(ownerId, fallbackHostId) });
    }

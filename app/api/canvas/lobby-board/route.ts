import { appendCanvasLobbyStroke, loadCanvasLobbyBoard, undoCanvasLobbyStroke } from "@/lib/canvas-lobby-board-store";
...
export async function GET() { const denied = await gameApiAccessDeniedResponse("canvas"); if (denied) return denied; try { await requireAuthenticatedPlayer(); return Response.json({ board: await loadCanvasLobbyBoard() }); } catch (error) { return fail(error); } }
export async function PATCH(request: Request) { const denied = await gameApiAccessDeniedResponse("canvas"); if (denied) return denied; try { const session = await requireAuthenticatedPlayer(); const limited = await rateLimitResponseFor(request, rateLimitPolicies.roomMutation, { playerId: session.id }); if (limited) return limited; const body = await request.json() as { action?: "stroke" | "undo"; stroke?: DrawingStroke }; const board = body.action === "undo" ? await undoCanvasLobbyStroke(session.id) : body.action === "stroke" && body.stroke ? await appendCanvasLobbyStroke(session.id, body.stroke) : null; return board ? Response.json({ board }) : Response.json({ error: "Invalid request" }, { status: 400 }); } catch (error) { return fail(error); } }

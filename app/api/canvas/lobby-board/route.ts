...
export async function GET(request: Request) { const denied = await gameApiAccessDeniedResponse("canvas"); if (denied) return denied; try { await requireAuthenticatedPlayerId(); const board = await loadCanvasLobbyBoard(); return conditionalVersionedJsonResponse(request, `canvas-lobby:${board.revision}`, () => ({ board })); } catch (error) { return fail(error); } }

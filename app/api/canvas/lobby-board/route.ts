...
export async function GET(request: Request) { const denied = await gameApiAccessDeniedResponse("canvas"); if (denied) return denied; try { await requireAuthenticatedPlayer(); return conditionalJsonResponse(request, { board: await loadCanvasLobbyBoard() }); } catch (error) { return fail(error); } }

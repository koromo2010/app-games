...
import type { CanvasRoomAction } from "@/lib/canvas-room";
...
export async function GET(request: Request) { const denied = await gameApiAccessDeniedResponse("canvas"); if (denied) return denied; try { const session = await requireAuthenticatedPlayer(); const room = await loadCanvasRoom(new URL(request.url).searchParams.get("code") || ""); if (!room) throw new Error("CANVAS_ROOM_NOT_FOUND"); if (!room.players.some((item) => item.id === session.id)) throw new Error("CANVAS_ROOM_FORBIDDEN"); return Response.json({ room: publicCanvasRoom(room) }); } catch (error) { return fail(error); } }
export async function POST(request: Request) { const denied = await gameApiAccessDeniedResponse("canvas"); if (denied) return denied; try { const session = await requireAuthenticatedPlayer(); const limited = await rateLimitResponseFor(request, rateLimitPolicies.roomMutation, { playerId: session.id }); if (limited) return limited; const body = await request.json() as { passphrase?: string }; return Response.json({ room: publicCanvasRoom(await createCanvasRoom(player(session), body.passphrase)) }); } catch (error) { return fail(error); } }

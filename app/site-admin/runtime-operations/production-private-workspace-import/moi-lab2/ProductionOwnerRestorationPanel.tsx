...
export function ProductionOwnerRestorationPanel() {
  ...
  const [diagnosticHttpStatus, setDiagnosticHttpStatus] = useState<number | null>(null);
  ...
  return <section className="rounded-2xl border border-violet-300/30 bg-violet-300/10 p-5" data-production-owner-restoration>
    ...
    {diagnosticState.consumed && <p className="mt-2 text-sm" data-completed-import-diagnostic-consumed>HTTP {diagnosticHttpStatus ?? "未確定"} / 診断送信済み（再送不可）</p>}

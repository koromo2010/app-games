...
import { performProductionPrivateWorkspaceImportTotpStepUp } from "@/lib/production-private-workspace-import-step-up-client";
...
export function ProductionPrivateWorkspaceImportPanel({
  ...
  const stepUp = async (event: FormEvent) => {
    ...
    setMessage("");
    try {
      const result = await performProductionPrivateWorkspaceImportTotpStepUp(totp);
      if (result.kind === "verified") router.refresh();
      else setMessage(`Authenticator確認を完了できません (${result.code})。`);
    } catch {
      setMessage("Authenticator確認結果が不明です。追加送信せず停止してください。");
    } finally {
      setTotp("");
      setStepUpBusy(false);
    }
  ...
  if (initialAccess === "step-up-required") return (
    <section className="rounded-2xl border border-cyan-300/30 bg-cyan-300/10 p-5">
      ...
      <form className="mt-4 space-y-3" onSubmit={stepUp}>
        <label className="block text-sm font-bold text-cyan-50">Authenticatorの6桁コード
          <input type="password" inputMode="numeric" autoComplete="one-time-code" pattern="[0-9]{6}" maxLength={6} required value={totp} onChange={(event) => setTotp(event.target.value.replace(/\D/g, "").slice(0, 6))} className="mt-2 w-full rounded-xl border border-cyan-200/30 bg-slate-950 px-4 py-3 font-mono text-lg tracking-[0.35em]" />
        </label>

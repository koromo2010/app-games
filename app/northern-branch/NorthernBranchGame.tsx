...
export function NorthernBranchGame() {
  ...
  const createRoom = async () => {
    ...
    try {
      await fetch(`/api/northern-branch/rooms?ownerId=${encodeURIComponent(ownerId)}&fallbackHostId=${encodeURIComponent(session.id)}`, { method: "DELETE" });
      const now = Date.now();
      ...
      <div className="mx-auto grid max-w-7xl gap-4 px-4 py-5 xl:grid-cols-[260px_minmax(0,1fr)_280px]">
        ...
        <div className="space-y-4">
          ...
          {game && <>
            ...
            {winner && <section className="rounded-2xl border border-amber-300 bg-amber-300/15 p-6 text-center"><p className="text-sm font-bold text-amber-200">GAME FINISHED</p><p className="mt-2 text-3xl font-black">{winner.name}の勝利！</p>{isHost ? <button type="button" disabled={isSaving} onClick={() => void runAction({ type: "reset-game", actorId: playerId })} className="mt-5 rounded-xl bg-amber-300 px-5 py-3 font-black text-slate-950">同じ部屋でもう一度</button> : <p className="mt-4 text-sm text-slate-300">ホストの操作を待っています。</p>}</section>}

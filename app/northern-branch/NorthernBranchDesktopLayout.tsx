...
export function NorthernBranchDesktopLayout({ controller }: { controller: NorthernBranchController }) {
  ...
  const isPlayingRoom = room?.phase === "playing";
  ...
  return (
    <main className={`min-h-screen bg-[radial-gradient(circle_at_top,#365314_0%,#172033_35%,#020617_75%)] text-white ${gameTopBannerOffsetClass}`}>
      ...
      />
      {room.phase === "lobby" && <div className="mx-auto w-full px-4 pt-4"><GameLoungeVisual gameId="northern-branch" /></div>}
      <div className="mx-auto grid w-full gap-4 px-4 py-5 xl:grid-cols-[260px_minmax(0,1fr)_280px]">
        ...
        <div className="space-y-4">
          ...
          >
            <ol aria-label="最終順位" className="grid gap-2 sm:grid-cols-2">
              {game.players.map((player) => <li key={player.id} className="rounded-xl border border-white/10 bg-white/[0.06] p-3"><div className="flex items-center justify-between gap-3"><span className="font-black">{player.name}</span><strong className="text-amber-300">{player.points}点</strong></div><p className="mt-1 text-xs text-slate-400">手札 {player.handCount ?? player.hand.length}枚・建物 {player.buildings.length}棟</p></li>)}
            </ol>
            <section aria-labelledby="northern-branch-result-log" className="mt-5 border-t border-white/10 pt-4">
              <h3 id="northern-branch-result-log" className="text-xs font-black uppercase tracking-wide text-lime-300">保存済みプレイログ</h3>
              <ol className="mt-2 space-y-2 text-sm leading-6 text-slate-300">
                {game.log.slice(0, 30).map((entry, index) => <li key={`${index}:${entry}`} className="rounded-lg bg-white/[0.06] px-3 py-2">{entry}</li>)}
              </ol>
            </section>

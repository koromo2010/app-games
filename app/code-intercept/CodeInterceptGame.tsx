...
export function CodeInterceptGame() {
  ...
  const createRoom = async () => {
    ...
    const draft: CodeInterceptRoom = {
      ...
      cardCount: codeInterceptDefaults.cardCount, codeLengthMode: codeInterceptDefaults.codeLengthMode, fixedCodeLength: codeInterceptDefaults.fixedCodeLength, initialPoints: codeInterceptDefaults.initialPoints, miscommunicationDamage: codeInterceptDefaults.miscommunicationDamage, interceptionDamage: codeInterceptDefaults.interceptionDamage, actionTimeLimitSeconds: 0, phaseStartedAt: null,
      debugMode: false, debugReplayEnabled: false, teams: codeInterceptTeamIds.map((id) => ({ id, name: teamLabel(id), points: codeInterceptDefaults.initialPoints, secretWords: [] })), clueGiverIds: {}, codeLengthChoices: {}, roundCodeLengths: {}, secretCodes: {}, clues: {}, allyAnswers: {}, interceptAnswers: {}, roundHistory: [], winner: null, debugLog: [], createdAt: now, updatedAt: now,
    };
  ...
  if (!room) return <main className={`min-h-screen bg-[radial-gradient(circle_at_top,#7f1d1d_0%,#172033_42%,#020617_82%)] px-4 pb-8 text-white ${gameTopBannerOffsetClass}`}>
    <GameTopBanner eyebrow="PRIVATE TEAM PROTOTYPE" title="コードインターセプト"><Link href="/games" className={gameTopBannerActionClass}>ゲームロビーへ戻る</Link><GameTopMenu><button type="button" data-menu-close="true" onClick={() => setRulesOpen(true)} className={gameTopMenuItemClass}>ルール</button></GameTopMenu><GamePlayerMenu id={session.id} name={session.name} avatarColor={session.avatarColor} avatarImage={session.avatarImage} hasRecoveryEmail={session.hasRecoveryEmail} /></GameTopBanner>
  ...
  return <main className={`min-h-screen bg-[radial-gradient(circle_at_top,#7f1d1d_0%,#172033_38%,#020617_78%)] text-white ${gameTopBannerOffsetClass}`}>
    ...
    <div className="mx-auto grid max-w-7xl gap-4 px-4 py-5 lg:grid-cols-[300px_minmax(0,1fr)]">
      <aside className="space-y-4"><section className="rounded-2xl border border-white/10 bg-slate-950/75 p-4"><div className="flex items-center justify-between"><h2 className="font-black">参加者</h2><span className="text-sm text-slate-400">{room.players.length}/{room.playerCapacity}人</span></div><ul className="mt-3 space-y-2">{room.players.map((player) => <PlayerCard key={player.id} player={player} room={room} me={playerId} saving={isSaving} onTeamChange={(teamId) => void runAction({ type: "set-team", actorId: playerId, teamId })} />)}</ul></section>
        <RoomConfigSummary items={[{ label: "秘密カード C", value: `${room.cardCount}枚` }, { label: "暗号桁数", value: room.codeLengthMode === "fixed" ? `固定・${room.fixedCodeLength}桁` : "毎ラウンド選択" }, { label: "初期ポイント X", value: `${room.initialPoints}点` }, { label: "伝達失敗", value: `−${room.miscommunicationDamage}` }, { label: "傍受成功", value: `−${room.interceptionDamage}` }, { label: "傍受開始", value: "第2ラウンド" }, { label: "時間制限", value: "なし" }]} />
      </aside>
      <div className="space-y-4">
        ...
        {room.phase === "clue" && <section className="rounded-2xl border border-white/10 bg-slate-950/80 p-6">
          <p className="text-sm font-black text-amber-300">第{room.roundNumber}ラウンド・ヒント入力</p>
          ...
          {isClueGiver && myCode ? <><h2 className="mt-4 text-2xl font-black">あなたが出題者です</h2><div className="mt-4 rounded-2xl border border-amber-300/30 bg-amber-300/10 p-5 text-center"><p className="text-xs font-black text-amber-200">今回の暗号</p><p className="mt-2 font-mono text-5xl font-black tracking-widest text-amber-200">{myCode.join("・")}</p></div>{room.clues[myTeamId!] ? <p className="mt-4 rounded-xl border border-emerald-300/30 bg-emerald-300/10 p-4 font-black text-emerald-100">提出済み：{room.clues[myTeamId!]?.join("／")}</p> : <div className="mt-4 grid gap-3 sm:grid-cols-4">{Array.from({ length: myCodeLength }, (_, index) => <label key={index} className="text-sm font-bold">{index + 1}つ目のヒント<input value={clueDrafts[index] ?? ""} maxLength={40} onChange={(event) => setClueDraftsByRound((current) => { const next = [...(current[draftRound] ?? Array.from({ length: myCodeLength }, () => ""))]; next[index] = event.target.value; return { ...current, [draftRound]: next }; })} className="mt-1 w-full rounded-xl border border-white/15 bg-white/10 px-3 py-3" /></label>)}</div>}{!room.clues[myTeamId!] && <button type="button" disabled={isSaving || clueDrafts.length < myCodeLength || clueDrafts.slice(0, myCodeLength).some((clue) => !clue.trim())} onClick={() => void runAction({ type: "submit-clues", actorId: playerId, clues: clueDrafts.slice(0, myCodeLength).map((clue) => clue.trim()) })} className="mt-4 w-full rounded-xl bg-amber-300 px-4 py-3 font-black text-slate-950 disabled:opacity-40">ヒントを確定</button>}</> : <><h2 className="mt-4 text-2xl font-black">出題者がヒントを作成中</h2><p className="mt-3 text-slate-300">今回の出題者：{room.players.find((player) => player.id === room.clueGiverIds[myTeamId!])?.name ?? "確認中"}</p></>}
        ...
        {room.phase === "answer" && myTeamId && <section className="rounded-2xl border border-white/10 bg-slate-950/80 p-6"><p className="text-sm font-black text-amber-300">第{room.roundNumber}ラウンド・同時回答</p><h2 className="mt-2 text-2xl font-black">両チームのヒントが公開されました</h2><div className="mt-5 grid gap-3 sm:grid-cols-2">{codeInterceptTeamIds.map((teamId) => <div key={teamId} className={`rounded-xl border p-4 ${teamStyle(teamId)}`}><p className="font-black">{teamLabel(teamId)}・{codeLengthForTeam(room, teamId)}桁</p><ol className="mt-2 space-y-1">{room.clues[teamId]?.map((clue, index) => <li key={index}><span className="mr-2 font-mono text-slate-400">{index + 1}.</span><strong>{clue}</strong></li>)}</ol></div>)}</div>
          {isClueGiver ? <p className="mt-5 rounded-xl bg-white/[0.05] p-4 text-sm text-slate-300">あなたは今回の出題者です。味方の代表回答と傍受回答が確定するまで待ってください。</p> : <div className="mt-5 grid gap-5 lg:grid-cols-2"><div className={`rounded-2xl border p-4 ${teamStyle(myTeamId)}`}><h3 className="font-black">味方の暗号を回答（{myCodeLength}桁）</h3>{room.allyAnswers[myTeamId] ? <p className="mt-3 font-mono text-2xl font-black text-emerald-200">提出済み {room.allyAnswers[myTeamId]?.join("・")}</p> : <><div className="mt-3"><CodePicker value={allyDraft} cardCount={room.cardCount} codeLength={myCodeLength} onChange={(value) => setAllyDraftsByRound((current) => ({ ...current, [draftRound]: value }))} /></div><button type="button" disabled={isSaving || !isValidCodeInterceptAnswer(allyDraft, room.cardCount, myCodeLength)} onClick={() => void runAction({ type: "submit-ally-answer", actorId: playerId, answer: allyDraft })} className="mt-3 w-full rounded-xl bg-emerald-300 px-4 py-3 font-black text-emerald-950 disabled:opacity-40">チーム回答を確定</button></>}</div>
            <div className={`rounded-2xl border p-4 ${teamStyle(otherCodeInterceptTeam(myTeamId))}`}><h3 className="font-black">敵の暗号を傍受（{enemyCodeLength}桁）</h3>{room.roundNumber === 1 ? <p className="mt-3 text-sm text-slate-300">第1ラウンドは傍受回答を行いません。</p> : room.interceptAnswers[myTeamId] ? <p className="mt-3 font-mono text-2xl font-black text-emerald-200">提出済み {room.interceptAnswers[myTeamId]?.join("・")}</p> : <><div className="mt-3"><CodePicker value={interceptDraft} cardCount={room.cardCount} codeLength={enemyCodeLength} onChange={(value) => setInterceptDraftsByRound((current) => ({ ...current, [draftRound]: value }))} /></div><button type="button" disabled={isSaving || !isValidCodeInterceptAnswer(interceptDraft, room.cardCount, enemyCodeLength)} onClick={() => void runAction({ type: "submit-intercept-answer", actorId: playerId, answer: interceptDraft })} className="mt-3 w-full rounded-xl bg-sky-300 px-4 py-3 font-black text-sky-950 disabled:opacity-40">傍受回答を確定</button></>}</div></div>}

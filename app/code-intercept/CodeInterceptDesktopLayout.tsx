    ...
    <div className="mx-auto grid max-w-7xl gap-4 px-4 py-5 lg:grid-cols-[300px_minmax(0,1fr)]">
      ...
      <div className="space-y-4">
        ...
        {room.phase === "lobby" && <section className="rounded-2xl border border-white/10 bg-slate-950/80 p-6">
          ...
          {isHost && room.debugMode && <div className="mt-5"><DebugWordGenerationTest onGenerate={() => sampleDebugWords(room.code)} heading="秘密カード候補10語を抽出" description="現在の本番抽選条件で、共通DBから秘密カード候補を10語だけ確認します。ゲーム状態や履歴は変更しません。" showModeToggle={false} fixedButtonLabel="候補10語を抽出" fixedRepeatLabel="別の10語を抽出" /></div>}

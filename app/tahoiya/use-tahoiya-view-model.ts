...
export function useTahoiyaViewModel(room: TahoiyaRoom | null, activePlayer: TahoiyaPlayer | null, selectedOptionId: string, definitionIndex: number, actionWindowRemainingSeconds: number | null) {
  return useMemo(() => {
    ...
    return { isAllVoteMode, answererCandidates, answerer, isAnswerer, activePlayerDefinitions, hasActivePlayerSubmitted, hasActivePlayerVoted, displayedVoteOptionId: selectedOptionId || savedVoteOptionId, definitionWriterCount, definitionTargetCount, writingDone, voterTarget, votingDone, remainingSeconds: effectiveTimeLimitSeconds > 0 ? actionWindowRemainingSeconds : null, nextWriter, nextVoter, sortedScores, roomConfigItems };
  }, [actionWindowRemainingSeconds, activePlayer, definitionIndex, room, selectedOptionId]);
}

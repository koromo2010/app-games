...
export function TahoiyaRulesDialog({ open, onClose }: { open: boolean; onClose: () => void }) {
  return <GameRulesDialog open={open} title="たほい屋のルール" onClose={onClose} gameId="tahoiya">
    ...
    <ol className="mt-2 list-decimal space-y-2 pl-5"><li>お題のことばと読み方が表示されます。</li><li>全員が、本物らしく見える偽説明を設定された数だけ入力します。全員が書き終わるまでは、自分の説明を直せます。</li><li>本物の説明と偽説明を、順番を混ぜて一斉に公開します。</li><li>全員が、本物だと思う説明を1つ選びます。自分が書いた説明はどれも選べません。</li><li>投票がそろうと、本物、偽説明の作者、誰がどれを選んだか、今回の得点を公開します。</li></ol>
    <h3 className="mt-4 font-black text-white">得点</h3>
    <ul className="mt-2 list-disc space-y-2 pl-5"><li>本物の説明を選んだ人は、1点をもらいます。</li><li>自分が書いた偽説明に1人が投票するたびに、作者が1点をもらいます。</li><li>同じラウンドで、本物を当てた得点と、偽説明でだました得点の両方をもらえます。</li><li>次のラウンドへ進むと得点は加算され、減点はありません。</li></ul>
    ...
    <p className="mt-2">全員が偽説明を書き、全員が本物だと思う説明へ投票します。自分の偽説明には投票できません。</p>
    <p className="mt-2">部屋設定で、1人あたりの偽説明を1〜3つから選べます。複数にした場合は、すべてが別々の候補として並びます。</p>
    <h3 className="mt-4 font-black text-white">お題の難易度</h3>
    <p className="mt-2"><span className="font-bold text-white">秘境：</span>AIが一般成人の推定認知率を1%超〜14%と判定した語を使います。</p>
    <p className="mt-2"><span className="font-bold text-white">魔境：</span>AIが一般成人の推定認知率を0〜1%と判定した、特に珍しい語を使います。</p>
    <p className="mt-2">どちらも共通単語DBの同じ未判定母集団から10語ずつ審査し、実質Zipfの0と0より大きい値で抽出を分けません。</p>

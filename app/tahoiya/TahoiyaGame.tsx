...
export function TahoiyaGame() {
  ...
  const testWordGeneration = async (forceNew: boolean): Promise<DebugWordGenerationResult> => {
    ...
    return {
      fields: [
        { label: "ワード", value: topic.word },
        { label: "読み", value: topic.reading ?? "" },
        { label: "本物の説明", value: topic.realDefinition },
        { label: "注記", value: topic.note },
      ],
      notice: topic.notice,
      generation: topic.generation,
    };
    ...
    <main className={`min-h-screen bg-slate-950 text-slate-950 ${gameTopBannerOffsetClass}`}>
      ...
      <GameRulesDialog open={rulesOpen} title="たほい屋のルール" onClose={() => setRulesOpen(false)}>
        ...
        <ul className="mt-2 list-disc space-y-2 pl-5">
          <li>お題には、ふだん見かけないことばが選ばれます。最初から意味を知っていなくても問題ありません。</li>
          <li>偽説明は、面白い文章よりも「辞書にありそうな文章」を目指すと、ほかの人をだましやすくなります。</li>
          <li>本物とすべての偽説明は、投票が終わるまで作者を隠して表示されます。</li>
        </ul>
        <h3 className="mt-4 font-black text-white">1ラウンドの流れ</h3>
        <ol className="mt-2 list-decimal space-y-2 pl-5">
          <li>お題のことばと読み方が表示されます。</li>
          <li>説明を書く人は、本物らしく見える偽説明を1人1つ入力します。全員が書き終わるまでは、自分の説明を直せます。</li>
          <li>本物の説明と偽説明を、順番を混ぜて一斉に公開します。</li>
          <li>投票する人は、本物だと思う説明を1つ選びます。全員投票モードでは、自分が書いた説明は選べません。</li>
          <li>投票がそろうと、本物、偽説明の作者、誰がどれを選んだか、今回の得点を公開します。</li>
        </ol>
        <h3 className="mt-4 font-black text-white">得点</h3>
        <ul className="mt-2 list-disc space-y-2 pl-5">
          <li>本物の説明を選んだ人は、1点をもらいます。</li>
          <li>自分が書いた偽説明に1人が投票するたびに、作者が1点をもらいます。2人をだませば2点、3人なら3点です。</li>
          <li>全員投票モードでは、本物を当てた1点と、自分の偽説明でだました得点の両方を同じラウンドでもらえます。</li>
          <li>同じ部屋で次のラウンドへ進むと得点は足されていきます。減点はありません。</li>
        </ul>
        <div className="mt-3 rounded-xl border border-amber-300/20 bg-amber-300/10 p-3 text-amber-100">
          <p className="font-bold">得点の例</p>
          <p className="mt-1">あなたが本物を当て、さらに2人があなたの偽説明を選んだ場合は、1点+2点で合計3点です。</p>
        </div>
        <h3 className="mt-4 font-black text-white">2つの遊び方</h3>
        <p className="mt-2"><span className="font-bold text-white">回答者1人：</span>1人だけが説明を選び、それ以外の人が偽説明を書きます。回答者はラウンドごとに交代します。</p>
        <p className="mt-2"><span className="font-bold text-white">全員作成・全員投票：</span>全員が偽説明を書き、そのあと全員が投票します。自分の偽説明には投票できません。</p>
        <h3 className="mt-4 font-black text-white">時間切れ</h3>
        <p className="mt-2 text-amber-200">全員の提出や投票がそろうと自動で進みます。偽説明を書かないまま時間が切れると、その人の説明は候補に入りません。投票時間が切れた人は未投票のまま採点され、その投票による得点は入りません。</p>
      </GameRulesDialog>

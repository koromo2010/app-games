...
export function KotobaSenpukuDesktopLayout({ controller }: { controller: KotobaSenpukuController }) {
  ...
  return (
    <main className={`min-h-screen bg-[radial-gradient(circle_at_top,#701a75_0%,#172033_35%,#020617_75%)] text-white ${gameTopBannerOffsetClass}`}>
      ...
      <div className="mx-auto w-full px-4 pt-4"><PlayerTimeoutNotice playerTimeouts={room.playerTimeouts} playerTimeoutNotice={room.playerTimeoutNotice} currentPlayerId={playerId} disabled={isSaving} onRecover={() => runAction({ type: "recover-player", actorId: playerId }).then(() => undefined)} /></div>
      <GameAdSlot
        gameId="kotoba-senpuku"
        surface={room.phase === "lobby" ? "room-lobby" : room.phase === "result" ? "result" : null}
        disabled={room.debugMode}
      />
      {room.phase === "lobby" && <div className="mx-auto w-full px-4 pt-4"><GameLoungeVisual gameId="kotoba-senpuku" /></div>}
      <div className="mx-auto grid w-full gap-4 px-4 py-5 md:grid-cols-[minmax(0,1fr)_240px] xl:grid-cols-[270px_minmax(0,1fr)_280px]">
        ...
        <div className="space-y-4">
          ...
          {room.phase === "secret" && <section className="rounded-2xl border border-white/10 bg-slate-950/80 p-6"><div className="flex flex-wrap items-center justify-between gap-3"><div><h2 className="text-xl font-black">秘密語を入力</h2><p className="mt-1 text-sm text-slate-400">提出 {room.submittedIds.length}/{room.players.length}人</p></div>{room.phaseStartedAt && <GamePhaseTimer key={room.phaseStartedAt} durationSeconds={room.secretTimeLimitSeconds} startedAt={room.phaseStartedAt} label="入力時間" scope={{ roomCode: room.code, generation: `${room.gameNumber}:${room.round}:${room.phaseStartedAt}`, phase: room.phase }} />}</div>{room.secrets[playerId] ? <div className="mt-5 rounded-xl border border-emerald-300/30 bg-emerald-300/10 p-4"><p className="font-black text-emerald-100">あなたの秘密語：{room.secrets[playerId]}</p><p className="mt-1 text-sm text-slate-300">全員の入力を待っています。他の秘密語は見えません。</p></div> : <div className="mt-5"><label className="block text-sm font-bold">ひらがな（{room.players.length === 2 ? "2文字以上・" : ""}文字数上限なし）<input value={secretWord} onChange={(event) => setSecretWord(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter") submitSecret(); }} className="mt-2 w-full rounded-xl border border-white/15 bg-white/10 px-4 py-3 text-lg text-white outline-none focus:border-fuchsia-300" /></label><button type="button" disabled={isSaving} onClick={submitSecret} className="mt-3 w-full rounded-xl bg-fuchsia-400 px-4 py-3 font-black text-fuchsia-950 disabled:opacity-50">秘密語を確定</button></div>}</section>}

          {room.phase === "battle" && <><section className="rounded-2xl border border-amber-300/30 bg-amber-300/10 p-5 sm:p-6"><p className="text-xs font-black uppercase tracking-[0.18em] text-amber-300">あなたの秘密語</p><p className="mt-2 break-words text-3xl font-black leading-tight text-amber-100 sm:text-5xl">{room.secrets[playerId]}</p></section><section className="rounded-2xl border border-white/10 bg-slate-950/80 p-6"><div className="flex flex-wrap items-center justify-between gap-3"><div><p className="text-sm font-bold text-fuchsia-300">第{room.turnNumber}手</p><h2 className="text-2xl font-black">{activePlayer?.name}の手番</h2></div>{room.phaseStartedAt && <GamePhaseTimer key={room.phaseStartedAt} durationSeconds={room.turnTimeLimitSeconds} startedAt={room.phaseStartedAt} label="手番時間" scope={{ roomCode: room.code, generation: `${room.gameNumber}:${room.round}:${room.turnNumber}:${room.phaseStartedAt}`, phase: room.phase }} />}</div><p className="mt-3 rounded-xl bg-white/[0.05] p-3 text-sm text-slate-300">秘密語が全部公開された人は脱落します。脱落者の手番は自動的に飛ばされます。</p>{room.debugMode && isHost && activePlayer?.id !== playerId && <p className="mt-3 rounded-xl border border-cyan-300/20 bg-cyan-300/10 p-3 text-sm font-bold text-cyan-50">デバッグ中：ホストが{activePlayer?.name}の手番を操作できます。</p>}</section>

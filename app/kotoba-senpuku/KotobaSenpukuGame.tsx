  ...
  return (
    <main className={`min-h-screen bg-[radial-gradient(circle_at_top,#701a75_0%,#172033_35%,#020617_75%)] text-white ${gameTopBannerOffsetClass}`}>
      ...
      <div className="mx-auto grid max-w-7xl gap-4 px-4 py-5 xl:grid-cols-[270px_minmax(0,1fr)_280px]">
        {isHost && room.debugMode && <button type="button" disabled={isSaving} aria-pressed={room.debugReplayEnabled} onClick={() => void runAction({ type: "set-debug-replay", actorId: playerId, enabled: !room.debugReplayEnabled })} className={`fixed bottom-3 right-3 z-30 rounded-lg border px-3 py-1.5 text-xs font-bold opacity-70 shadow-lg transition hover:opacity-100 disabled:opacity-40 ${room.debugReplayEnabled ? "border-emerald-300/50 bg-emerald-950/90 text-emerald-200" : "border-white/15 bg-slate-950/90 text-slate-400"}`}>デバッグ用プレイバック {room.debugReplayEnabled ? "ON" : "OFF"}</button>}

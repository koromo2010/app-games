...
import { DebugReplayButton } from "@/app/components/DebugReplayButton";
  ...
  const createRoom = async () => {
    ...
    try {
      ...
      const nextRoom: KotobaSenpukuRoom = {
        ...
        ...defaults, debugMode: false, debugReplayEnabled: false, round: 1, theme: null, secrets: {}, submittedIds: [], masks: {}, calledKana: [], exposedIds: [],
  ...
  return (
    <main className={`min-h-screen bg-[radial-gradient(circle_at_top,#701a75_0%,#172033_35%,#020617_75%)] text-white ${gameTopBannerOffsetClass}`}>
      ...
      <div className="mx-auto grid max-w-7xl gap-4 px-4 py-5 md:grid-cols-[minmax(0,1fr)_240px] xl:grid-cols-[270px_minmax(0,1fr)_280px]">
        {isHost && room.debugMode && <div className="md:col-span-2 xl:col-span-3"><DebugReplayButton enabled={room.debugReplayEnabled} disabled={isSaving} onChange={(enabled) => runAction({ type: "set-debug-replay", actorId: playerId, enabled }).then(() => undefined)} /></div>}

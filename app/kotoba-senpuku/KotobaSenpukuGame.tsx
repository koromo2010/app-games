...
function PlayerRow({ player, isHost, isMe }: { player: KotobaSenpukuPlayer; isHost: boolean; isMe: boolean }) {
  return (
...
export function KotobaSenpukuGame() {
  ...
  const ranking = room ? [...room.players].sort((left, right) => (room.totalScores[right.id] ?? 0) - (room.totalScores[left.id] ?? 0)) : [];
  const configItems = room ? [
    { label: "参加人数", value: `${room.players.length}/${kotobaSenpukuMaximumPlayers}人` },
    ...
    { label: "文字上限", value: `${kotobaSenpukuMaximumCalls}文字` },
  ...
  const runAction = async (action: KotobaSenpukuRoomAction) => {
    ...
    try {
      ...
      if (!response.ok || !data.room) {
        const invalidWord = data.error === "Invalid secret word" ? "秘密語は2〜8文字のひらがなで入力してください。" : "";
  ...
  const rulesDialog = <GameRulesDialog open={rulesOpen} title="ことばソナーのルール" onClose={() => setRulesOpen(false)}>
    ...
    <ol className="mt-2 list-decimal space-y-2 pl-5"><li>秘密語は2〜8文字のひらがな・長音符で入力します。</li><li>文字スキャンは、その文字群を含む未公開の相手1人につき+1点。反応なしは-1点です。</li><li>スキャンで相手の語を全部開くと、その相手1人につき追加+2点です。</li><li>直接推理の完全一致は+4点、外れは-1点。得点は0未満になりません。</li></ol>
    <p className="mt-4">濁点・半濁点と小さいかなは元の清音と同じ文字群です。長音符「ー」は独立した文字としてスキャンします。</p>
    <h3 className="mt-4 font-black text-white">ラウンド終了と勝利</h3>
    <p className="mt-2">未公開の語が1つ以下になるか、18文字を呼ぶと終了。最後に1語だけ残ればその人へ+3点、複数残れば各+2点。設定ラウンド終了時の合計点が最も高い人が勝利です。</p>
  </GameRulesDialog>;
      ...
      <main className="min-h-screen bg-[radial-gradient(circle_at_top,#701a75_0%,#172033_42%,#020617_82%)] px-4 py-8 text-white">
        <div className="mx-auto max-w-4xl">
          ...
          <section className="mt-5 overflow-hidden rounded-3xl border border-white/10 bg-slate-950/80 shadow-2xl">
            <div className="bg-gradient-to-r from-fuchsia-400 via-cyan-300 to-amber-300 px-6 py-8 text-slate-950"><p className="text-xs font-black uppercase tracking-[0.28em]">Original online word game</p><h1 className="mt-2 text-4xl font-black sm:text-6xl">ことばソナー</h1><p className="mt-3 font-bold">秘密のことばへ文字スキャンをかけ、反応と直接推理で信号点を集める。</p></div>
            ...
            {showChoices && <div className="border-t border-white/10 p-6"><h2 className="font-black">参加できる部屋</h2><div className="mt-3 grid gap-2 sm:grid-cols-2">{choices.map((choice) => <button key={choice.code} type="button" onClick={() => { setJoinCode(choice.code); void joinRoom(choice.code); }} className="rounded-xl border border-white/10 bg-white/[0.05] p-4 text-left"><span className="font-mono text-lg font-black text-fuchsia-300">{choice.code}</span><span className="ml-3 font-bold">{choice.hostName}</span><span className="mt-1 block text-xs text-slate-400">{choice.playerCount}/{kotobaSenpukuMaximumPlayers}人・{choice.roundsTotal}ラウンド・合言葉{choice.hasPassphrase ? "あり" : "なし"}</span></button>)}</div></div>}
  ...
  return (
    <main className="min-h-screen bg-[radial-gradient(circle_at_top,#701a75_0%,#172033_35%,#020617_75%)] text-white">
      <header className="border-b border-white/10 bg-slate-950/70"><div className="mx-auto flex max-w-7xl flex-wrap items-center justify-between gap-3 px-4 py-4"><div><Link href="/games" className="text-xs font-bold text-fuchsia-200">← ゲームロビー</Link><h1 className="mt-1 text-2xl font-black">ことばソナー <span className="font-mono text-base text-amber-300">#{room.code}</span></h1></div><div className="flex flex-wrap items-center gap-2">{isHost && <DebugModeButton enabled={room.debugMode} disabled={isSaving || room.phase !== "lobby"} onAbort={room.debugMode && room.phase !== "lobby" ? () => runAction({ type: "abort-game", actorId: playerId }).then(() => undefined) : undefined} onChange={(enabled) => runAction({ type: "set-debug", actorId: playerId, enabled }).then(() => undefined)} />}<button type="button" onClick={() => setRulesOpen(true)} className="rounded-lg border border-white/15 px-3 py-2 text-sm font-bold">ルール</button><span className="rounded-lg bg-white/10 px-3 py-2 text-sm font-bold">{session.name}</span>{isHost ? <button type="button" onClick={() => void dissolveRoom()} className="rounded-lg border border-rose-300/30 px-3 py-2 text-sm font-bold text-rose-100">部屋を解散</button> : room.phase === "lobby" && <button type="button" onClick={() => void leaveRoom()} className="rounded-lg border border-white/15 px-3 py-2 text-sm font-bold">退出</button>}</div></div></header>
      <div className="mx-auto grid max-w-7xl gap-4 px-4 py-5 xl:grid-cols-[270px_minmax(0,1fr)_280px]">
        ...
        <div className="space-y-4">
          ...
          {room.phase === "lobby" && <section className="rounded-2xl border border-white/10 bg-slate-950/80 p-6"><h2 className="text-2xl font-black">ゲーム開始前</h2>{isHost ? <div className="mt-5 grid gap-4 sm:grid-cols-2"><label className="text-sm font-bold text-white">ラウンド数<select value={room.roundsTotal} onChange={(event) => void updateConfig({ roundsTotal: Number(event.target.value) })} className="mt-1 w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-slate-950">{[1,2,3,4,5].map((value) => <option key={value} value={value}>{value}回</option>)}</select></label><div className="sm:row-span-3 rounded-xl border border-fuchsia-300/20 bg-fuchsia-300/10 p-4 text-sm leading-6 text-fuchsia-50">秘密語は2〜8文字のひらがな。文字スキャンは他人の語に反応すると信号点を獲得し、外すと1点減ります。直接推理の正解は4点です。</div><RoomTimeLimitControl label="秘密語の入力時間" value={room.secretTimeLimitSeconds} onChange={(seconds) => void updateConfig({ secretTimeLimitSeconds: seconds })} /><RoomTimeLimitControl label="1手番の時間" value={room.turnTimeLimitSeconds} onChange={(seconds) => void updateConfig({ turnTimeLimitSeconds: seconds })} /></div> : <p className="mt-4 rounded-xl bg-white/[0.05] p-4 text-slate-300">ホストが設定してゲームを開始するまでお待ちください。</p>}{isHost && room.debugMode && <div className="mt-5 rounded-xl border border-cyan-300/25 bg-cyan-300/10 p-4"><p className="text-sm font-bold text-cyan-50">ダミーを追加すると、ホスト1人で複数人の流れを確認できます。</p><button type="button" disabled={isSaving || room.players.length >= kotobaSenpukuMaximumPlayers} onClick={() => void runAction({ type: "debug-add-player", actorId: playerId })} className="mt-3 w-full rounded-lg bg-cyan-200 px-4 py-2 font-black text-cyan-950 disabled:opacity-40">ダミーユーザーを追加</button></div>}{isHost && <button type="button" disabled={isSaving || room.players.length < 2} onClick={() => void runAction({ type: "start-game", actorId: playerId })} className="mt-6 w-full rounded-xl bg-amber-300 px-4 py-4 text-lg font-black text-slate-950 disabled:opacity-40">{room.players.length < 2 ? "2人以上で開始できます" : "このメンバーで開始"}</button>}</section>}

          {room.phase !== "lobby" && <section className="rounded-2xl border border-white/10 bg-slate-950/80 p-6"><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="text-xs font-black uppercase tracking-[0.22em] text-amber-300">第{room.round}/{room.roundsTotal}ラウンド</p><h2 className="mt-2 text-2xl font-black sm:text-4xl">{room.theme?.title}</h2><p className="mt-2 text-sm text-slate-400">{room.theme?.guide}</p></div><span className="rounded-xl bg-fuchsia-400 px-4 py-2 font-black text-fuchsia-950">呼出 {room.calledKana.length}/{kotobaSenpukuMaximumCalls}</span></div></section>}

          {room.phase === "secret" && <section className="rounded-2xl border border-white/10 bg-slate-950/80 p-6"><div className="flex flex-wrap items-center justify-between gap-3"><div><h2 className="text-xl font-black">秘密語を入力</h2><p className="mt-1 text-sm text-slate-400">提出 {room.submittedIds.length}/{room.players.length}人</p></div>{room.phaseStartedAt && <GamePhaseTimer key={room.phaseStartedAt} durationSeconds={room.secretTimeLimitSeconds} startedAt={room.phaseStartedAt} label="入力時間" />}</div>{room.secrets[playerId] ? <div className="mt-5 rounded-xl border border-emerald-300/30 bg-emerald-300/10 p-4"><p className="font-black text-emerald-100">あなたの秘密語：{room.secrets[playerId]}</p><p className="mt-1 text-sm text-slate-300">全員の入力を待っています。他の秘密語は見えません。</p></div> : <div className="mt-5"><label className="block text-sm font-bold">2〜8文字のひらがな<input value={secretWord} maxLength={8} onChange={(event) => setSecretWord(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter") submitSecret(); }} className="mt-2 w-full rounded-xl border border-white/15 bg-white/10 px-4 py-3 text-lg text-white outline-none focus:border-fuchsia-300" /></label><button type="button" disabled={isSaving} onClick={submitSecret} className="mt-3 w-full rounded-xl bg-fuchsia-400 px-4 py-3 font-black text-fuchsia-950 disabled:opacity-50">秘密語を確定</button></div>}{isHost && room.debugMode && <button type="button" disabled={isSaving} onClick={() => void runAction({ type: "debug-fill-secrets", actorId: playerId, round: room.round })} className="mt-4 w-full rounded-xl border border-cyan-300/30 bg-cyan-300/10 px-4 py-3 text-sm font-black text-cyan-100">デバッグ：未入力を自動で埋める</button>}</section>}

          {room.phase === "battle" && <><section className="rounded-2xl border border-white/10 bg-slate-950/80 p-6"><div className="flex flex-wrap items-center justify-between gap-3"><div><p className="text-sm font-bold text-fuchsia-300">第{room.turnNumber}手</p><h2 className="text-2xl font-black">{activePlayer?.name}の手番</h2></div>{room.phaseStartedAt && <GamePhaseTimer key={room.phaseStartedAt} durationSeconds={room.turnTimeLimitSeconds} startedAt={room.phaseStartedAt} label="手番時間" />}</div><p className="mt-3 rounded-xl bg-white/[0.05] p-3 text-sm text-slate-300">文字スキャンか直接推理のどちらかを1回行います。脱落はなく、全員に手番が回ります。</p>{room.debugMode && isHost && activePlayer?.id !== playerId && <p className="mt-3 rounded-xl border border-cyan-300/20 bg-cyan-300/10 p-3 text-sm font-bold text-cyan-50">デバッグ中：ホストが{activePlayer?.name}の手番を操作できます。</p>}</section>
            <section className="rounded-2xl border border-white/10 bg-slate-950/80 p-6"><h2 className="text-xl font-black">文字スキャン</h2><p className="mt-1 text-sm text-slate-400">濁点・半濁点、小さい文字は元の文字と同じグループ。「ー」は独立して反応します。</p><div className="mt-4 grid grid-cols-5 gap-2 sm:grid-cols-10">{kotobaSenpukuKana.map((kana) => <button key={kana} type="button" disabled={!canControlTurn || room.calledKana.includes(kana) || isSaving} onClick={() => void runAction({ type: "scan-kana", actorId: playerId, round: room.round, kana })} className={`aspect-square rounded-lg border text-lg font-black transition disabled:opacity-25 ${room.calledKana.includes(kana) ? "border-white/5 bg-slate-800 text-slate-500" : "border-fuchsia-300/30 bg-fuchsia-300/10 text-fuchsia-100 hover:bg-fuchsia-300/20"}`}>{kana}</button>)}</div></section>
            <section className="rounded-2xl border border-white/10 bg-slate-950/80 p-6"><h2 className="text-xl font-black">直接推理</h2><p className="mt-1 text-sm text-slate-400">相手の秘密語を完全一致で当てると4信号点。外すと1点減ります。</p><div className="mt-4 grid gap-3 sm:grid-cols-[minmax(0,0.8fr)_minmax(0,1.2fr)_auto]"><select value={effectiveTarget} disabled={!canControlTurn} onChange={(event) => setChallengeTarget(event.target.value)} className="rounded-xl border border-slate-300 bg-white px-3 py-3 text-slate-950">{challengeTargets.map((player) => <option key={player.id} value={player.id}>{player.name}</option>)}</select><input value={challengeGuess} disabled={!canControlTurn} maxLength={8} onChange={(event) => setChallengeGuess(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter") challengeWord(); }} placeholder="ひらがなで推理" className="rounded-xl border border-white/15 bg-white/10 px-4 py-3 text-white outline-none" /><button type="button" disabled={!canControlTurn || !effectiveTarget || !challengeGuess.trim() || isSaving} onClick={challengeWord} className="rounded-xl bg-amber-300 px-5 py-3 font-black text-slate-950 disabled:opacity-40">推理する</button></div>{isHost && room.debugMode && <button type="button" disabled={isSaving} onClick={() => void runAction({ type: "debug-auto-turn", actorId: playerId, round: room.round })} className="mt-4 w-full rounded-xl border border-cyan-300/30 bg-cyan-300/10 px-4 py-3 text-sm font-black text-cyan-100">デバッグ：この手番を自動実行</button>}</section>
          </>}

          {room.phase === "result" && latestResult && <section className="rounded-2xl border border-white/10 bg-slate-950/80 p-6"><h2 className="text-2xl font-black">ラウンド結果</h2><div className="mt-4 grid gap-3 sm:grid-cols-2">{room.players.map((player) => <div key={player.id} className="rounded-xl border border-white/10 bg-white/[0.05] p-4"><div className="flex items-center justify-between gap-3"><p className="font-black">{player.name}</p><span className="text-xl font-black text-amber-300">+{latestResult.signals[player.id] ?? 0}</span></div><p className="mt-2 font-mono text-2xl font-black tracking-widest text-fuchsia-200">{latestResult.secrets[player.id]}</p><p className="mt-1 text-xs text-slate-400">潜伏ボーナス +{latestResult.survivalBonus[player.id] ?? 0}</p></div>)}</div>{isHost ? <button type="button" disabled={isSaving} onClick={() => void runAction(room.round >= room.roundsTotal ? { type: "reset-game", actorId: playerId } : { type: "next-round", actorId: playerId, round: room.round })} className="mt-5 w-full rounded-xl bg-amber-300 px-4 py-3 font-black text-slate-950 disabled:opacity-50">{room.round >= room.roundsTotal ? "同じ部屋でもう一度" : "次のラウンドへ"}</button> : <p className="mt-4 text-center text-sm font-bold text-slate-300">ホストの操作を待っています。</p>}</section>}
        </div>

...
export function CanvasDesktopLayout({ controller }: { controller: CanvasController }) {
  ...
  return <main className={`min-h-screen bg-[radial-gradient(circle_at_top,#e0f2fe_0%,#f8fafc_42%,#fef3c7_100%)] text-slate-900 ${gameTopBannerOffsetClass}`}>
    ...
    {!room && <div className="mx-auto w-full px-3 pt-5 sm:px-6"><GameLoungeVisual gameId="canvas" /></div>}

    <section className="mx-auto grid w-full gap-4 px-3 py-5 sm:px-6 xl:grid-cols-[260px_minmax(0,1fr)] xl:items-start">
      ...
      <div ref={boardViewportRef} tabIndex={0} data-game-keyboard-owner="true" aria-describedby="canvas-keyboard-instructions" onFocus={() => setKeyboardCursorVisible(true)} className={`${boardFullscreen ? "h-full max-h-none rounded-none border-0" : "max-h-[72vh] rounded-2xl border-4 border-white"} overflow-auto bg-white shadow-2xl shadow-slate-400/40 outline-none focus-visible:ring-4 focus-visible:ring-cyan-500 focus-visible:ring-offset-2`}>
        <div className={room ? "aspect-[4/3] min-h-[320px] w-full" : "h-[1200px] w-[1600px]"} style={{ zoom }}><DrawingCanvas strokes={visibleStrokes} layerIds={layers.filter((layer) => !hiddenLayerIds.has(layer.id)).map((layer) => layer.id)} activeLayerId={activeLayerId} color={color} width={width} opacity={opacity} tool={tool} keyboardCursor={keyboardCursorVisible ? keyboardCursor : undefined} onPointerInteraction={() => { boardViewportRef.current?.focus({ preventScroll: true }); setKeyboardCursorVisible(false); if (!room) activateLobbySync(); }} onPointerPosition={setKeyboardCursor} onColorPick={(picked) => { setColor(picked); setTool("pen"); }} onStrokeProgress={submitStrokeProgress} onPan={(deltaX, deltaY) => boardViewportRef.current?.scrollBy(deltaX, deltaY)} onStrokeComplete={submitStroke} /></div>
      </div>
      </div>
      <div id="canvas-keyboard-instructions" className="flex flex-wrap items-center justify-between gap-2 px-1 text-xs font-semibold text-slate-500"><span>{syncNotice}</span><span>A ペン・S 消しゴム・C 細く・V 太く・Z 戻す・Y やり直す</span><span>＋/− 拡大縮小・0 等倍・Ctrl＋ホイール</span><span>矢印 移動・Space＋矢印 描画・Shift 高速</span><span>{strokes.length}ストローク</span></div>
      </div>
    </section>

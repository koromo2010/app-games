...
export function ContactForm() {
  const [category, setCategory] = useState("general"); const [name, setName] = useState(""); const [email, setEmail] = useState(""); const [message, setMessage] = useState(""); const [status, setStatus] = useState(""); const [receiptId, setReceiptId] = useState(""); const [saving, setSaving] = useState(false);
  const submit = async (event: FormEvent) => { event.preventDefault(); setSaving(true); setStatus(""); setReceiptId(""); try { const response = await fetch("/api/contact", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ category, name, email, message }) }); const data = await response.json().catch(() => null) as { contact?: { id?: string } } | null; if (!response.ok) throw new Error(); setMessage(""); setReceiptId(data?.contact?.id ?? ""); setStatus("送信しました。内容を確認のうえ対応します。"); } catch { setStatus("送信できませんでした。時間をおいてもう一度お試しください。"); } finally { setSaving(false); } };

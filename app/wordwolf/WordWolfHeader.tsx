...
type Props = {
  ...
  debugControlledPlayerId: string;
  ...
  onSelectDebugPlayer: (playerId: string) => void;
  ...
  return <>
    <GameTopBanner eyebrow="Room based social deduction" title="ワードウルフ・ラウンジ">
      ...
      {props.room && props.isHost && <DebugModeButton
        ...
        gameTools={<DebugToolsSection title="操作プレイヤー" description="ダミー視点・代理操作の対象を切り替えます。">
            <DebugPlayerSwitcher
              players={props.room.players}
              value={props.debugControlledPlayerId}
              onChange={props.onSelectDebugPlayer}
            />
          </DebugToolsSection>}
        wordGenerationTools={props.room.phase === "lobby"
          ? <DebugWordGenerationTest onGenerate={props.onTestWordGeneration} />
          : undefined}
      />}
      <GameTopMenu>
    ...
    {props.room && <div className="mx-auto w-full px-4 pt-4"><PlayerTimeoutNotice playerTimeouts={props.room.playerTimeouts} playerTimeoutNotice={props.room.playerTimeoutNotice} currentPlayerId={props.currentPlayerId} onRecover={props.onRecover} /></div>}

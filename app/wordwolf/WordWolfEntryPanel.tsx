...
export function WordWolfEntryPanel({ playerName, roomPassphrase, joinCode, joinableRooms, isJoinListOpen, onRoomPassphraseChange, onJoinCodeChange, onCreateRoom, onShowJoinChoices, onJoinRoom }: Props) {
  ...
  return <div className={panelClass}>
    ...
    <div className="mt-3 grid grid-cols-2 gap-2">
      <button onClick={onCreateRoom} disabled={!hasPlayerName} className={cyanButtonClass}>部屋を作成</button>
      <button onClick={onShowJoinChoices} disabled={!hasPlayerName} className={subtleButtonClass}>参加</button>
    </div>
    ...
    {isJoinListOpen && <div className="mt-3 rounded-lg border border-slate-200 bg-slate-50 p-3">
      <div className="flex items-center justify-between gap-2"><p className="text-sm font-semibold text-slate-800">参加できる部屋</p><button onClick={onShowJoinChoices} className="rounded-lg border border-slate-300 bg-white px-2 py-1 text-xs font-semibold text-slate-700 transition hover:bg-slate-100">更新</button></div>
      <div className="mt-3 space-y-2">
        {joinableRooms.length === 0 ? <p className="rounded-lg bg-white px-3 py-4 text-center text-sm text-slate-500">未開始の部屋はありません。</p> : joinableRooms.map((choice) => <button key={choice.code} onClick={() => onJoinRoom(choice.code)} className="w-full rounded-lg border border-slate-200 bg-white px-3 py-2 text-left text-sm transition hover:border-cyan-400 hover:bg-cyan-50"><div className="flex items-center justify-between gap-2"><span className="font-mono text-base font-bold">{choice.code}</span><span className="text-xs text-slate-500">{choice.hasPassphrase ? "合言葉あり" : "合言葉なし"}</span></div><div className="mt-1 flex flex-wrap gap-x-3 gap-y-1 text-xs text-slate-600"><span>host: {choice.hostName}</span><span>{choice.playerCount}人</span><span>{choice.roundsTotal}周</span></div></button>)}
      </div>
      {joinCode.trim() && <button onClick={() => onJoinRoom()} className={`mt-3 w-full ${subtleButtonClass}`}>入力したコードで参加</button>}
    </div>}
  </div>;
}

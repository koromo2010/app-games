...
function PlayerRow({ player, room, playerId, controlledPlayerId }: { player: DaifugoRoomPlayer; room: DaifugoRoomView; playerId: string; controlledPlayerId: string }) {
  ...
  return <li>
    <div className={`flex w-full items-center gap-3 rounded-xl border p-3 text-left ${selected ? "border-cyan-300 bg-cyan-300/10" : "border-white/10 bg-white/[0.04]"}`}>
  ...
  return <main className={`min-h-screen bg-[radial-gradient(circle_at_top,#164e63_0%,#0f172a_45%,#020617_100%)] text-white ${gameTopBannerOffsetClass}`}>
    <GameTopBanner eyebrow="ONLINE CARD GAME" title={<>{d.title} <span className="font-mono text-base text-amber-300">#{room.code}</span></>}>
      ...
      {isHost && <DebugModeButton
        ...
        disabled={saving || room.phase !== "lobby"}
        ...
        gameTools={<>
          {game && <DebugToolsSection title={locale === "en" ? "Controlled player" : "操作プレイヤー"} description={locale === "en" ? "Inspect and control the host or a dummy hand." : "ホストまたはダミーの手札を表示して代理操作します。"}>
            <DebugPlayerSwitcher
              ...
              players={room.players.filter((player) => player.id === playerId || player.isDummy)}
              value={controlledPlayerId}
              onChange={(targetPlayerId) => {
                setDebugControlledPlayerId(targetPlayerId === playerId ? "" : targetPlayerId);
                setSelectedCardIds([]);
              }}
            />
          </DebugToolsSection>}

    ...
    <>
      ...
      {isOpen && createPortal(
        <div className="fixed inset-0 z-[9998]" onClick={() => setIsOpen(false)}>
          <div role="dialog" aria-label="ゲームメニュー" className="fixed w-[min(18rem,calc(100vw-1.5rem))] rounded-lg border border-slate-200 bg-white p-3 text-slate-900 shadow-2xl" style={position} onClick={(event) => { event.stopPropagation(); closeSelectedItem(event); }}>
            <div className="flex items-center justify-between gap-2"><p className="text-xs font-bold uppercase tracking-wide text-slate-500">Game menu</p><button type="button" onClick={() => setIsOpen(false)} className="rounded border border-slate-200 px-2 py-1 text-xs font-bold text-slate-500">閉じる</button></div>

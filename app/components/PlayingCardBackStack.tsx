...
export function PlayingCardBackStack({ count, size = "sm", label = `${count}枚の非公開カード`, maximumVisibleCards = 7 }: PlayingCardBackStackProps) {
  ...
  return <div className="flex items-end" role="img" aria-label={label}>
    ...
    <span className="relative z-20 -ml-3 rounded-full bg-amber-300 px-2 py-1 text-xs font-black text-slate-950 shadow">{safeCount}枚</span>
  </div>;
}

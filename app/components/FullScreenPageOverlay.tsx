...
export function FullScreenPageOverlay({ open, href, title, onClose, keepAlive = false }: Props) {
  ...
  useKeyboardLayer({ open, containerRef: dialogRef, initialFocusRef: backButtonRef, onDismiss: close });
  ...
  return createPortal(
    <div ref={dialogRef} className={open ? "fixed inset-0 z-[10000] bg-slate-950/80 p-2 text-white backdrop-blur-sm sm:p-4" : "hidden"} role="dialog" aria-modal={open ? true : undefined} aria-hidden={!open} aria-label={title} tabIndex={-1}>

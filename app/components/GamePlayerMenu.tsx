...
export function GamePlayerMenu(props: Props) {
  ...
  const updateAvatar = async (nextColor: string, nextImage: string) => {
    ...
    try {
      ...
      if (!result.persistent) setMessage(t("account.avatarSavedLocal"));
    } catch {
  ...
  return (
    <>
      <button ref={buttonRef} type="button" onClick={openMenu} className="flex items-center gap-2 rounded-lg border border-white/15 bg-white/10 px-2.5 py-1.5 font-semibold text-white transition hover:bg-white/15" aria-haspopup="dialog" aria-expanded={isOpen} aria-label={t("account.openMenu", { name: props.name })}>
      ...
      {isOpen && createPortal(<div className="fixed inset-0 z-[9999]" onClick={() => setIsOpen(false)}><div role="dialog" aria-label={t("account.openMenu", { name: props.name })} className="fixed w-[min(18rem,calc(100vw-1.5rem))] rounded-lg border border-slate-200 bg-white p-3 text-slate-900 shadow-2xl" style={position} onClick={(event) => event.stopPropagation()}>
        ...
        <p className="mt-3 text-xs font-bold">{t("account.avatarImage")}</p>
        ...
        <button type="button" onClick={() => { setIsOpen(false); setIsMyPageOpen(true); }} className="mt-3 flex w-full items-center justify-center rounded-md bg-cyan-600 px-3 py-2 text-sm font-bold text-white hover:bg-cyan-500">{t("account.openMyPage")}</button>
        {props.id && <button type="button" disabled={isLoggingOut} onClick={() => void logout()} className="mt-2 w-full rounded-md border border-rose-200 bg-rose-50 px-3 py-2 text-sm font-bold text-rose-700 hover:bg-rose-100 disabled:opacity-40">{isLoggingOut ? t("account.loggingOut") : t("account.logout")}</button>}

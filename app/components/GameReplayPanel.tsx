...
export function GameReplayPanel() {
  ...
  const [selectedReplay, setSelectedReplay] = useState<TahoiyaReplayDetail | null>(null);
  ...
  const loadReplays = useCallback(async (signal?: AbortSignal) => {
    try {
      const response = await fetch("/api/player-replays?gameType=tahoiya", { cache: "no-store", signal });
  ...
  useEffect(() => {
    ...
    fetch("/api/player-replays?gameType=tahoiya", { cache: "no-store", signal: controller.signal })
      .then(async (response) => {
        const next = (await response.json()) as GameReplayListResponse;
        if (!response.ok) throw new Error("REPLAY_LIST_FAILED");
        setData(next);
  ...
  const openReplay = async (summary: GameReplaySummary) => {
    ...
    try {
      ...
      const body = (await response.json()) as { replay?: TahoiyaReplayDetail };
  ...
  const toggleFavorite = async (replay: GameReplaySummary, favorite: boolean) => {
    ...
    try {
      ...
      const body = (await response.json()) as { replay?: TahoiyaReplayDetail; error?: string };
      if (!response.ok) {
  ...
  const shareReplay = async (replay: GameReplaySummary) => {
    ...
    const url = new URL("/tahoiya", window.location.origin).toString();
    try {
      const outcome = await shareGameResult({ title: "Game Fields たほい屋", text, url });
    ...
    <section className="rounded-lg border border-white/10 bg-white/[0.96] p-4 shadow-[0_18px_50px_rgba(15,23,42,0.18)]" aria-labelledby="replay-heading">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <h2 id="replay-heading" className="text-lg font-bold text-slate-950">たほい屋プレイバック</h2>
      ...
      {isLoading ? (
        ...
        <p className="mt-4 text-sm leading-6 text-slate-500">まだプレイバックはありません。実装後に完了した、デバッグ以外のラウンドから記録されます。</p>
      )}
      ...
      {selectedReplay && (
        <div className="mt-4 rounded-lg border border-violet-200 bg-violet-50/70 p-4">
          <div className="flex items-start justify-between gap-3">
            <div>
              <p className="text-xs font-semibold text-violet-700">ROUND {selectedReplay.round}</p>
              <h3 className="text-lg font-black text-slate-950">{selectedReplay.title}</h3>
              {selectedReplay.reading && <p className="text-xs text-slate-500">{selectedReplay.reading}</p>}
            </div>
            <button
              type="button"
          ...
          <div className="mt-4 space-y-2">
            {selectedReplay.definitions.map((definition) => (
              <div key={definition.id} className={`rounded-lg border p-3 ${definition.isReal ? "border-emerald-300 bg-emerald-50" : definition.isMine ? "border-cyan-300 bg-cyan-50" : "border-slate-200 bg-white"}`}>
                ...
                {definition.voterNames.length > 0 && (
                  <p className="mt-1 text-[11px] text-slate-500">投票: {definition.voterNames.join("、")}</p>
                )}
                {selectedReplay.viewerVoteDefinitionId === definition.id && (
                  <p className="mt-1 text-[11px] font-bold text-violet-700">あなたが選んだ説明</p>
                )}
              </div>
            ))}
          </div>

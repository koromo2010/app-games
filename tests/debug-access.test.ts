...
test("デバッグ専用操作だけをアカウント認証対象にする", () => {
  ...
  assert.equal(actionRequiresDebugAccess({ type: "advance-phase", force: true }), true);
...
test("プレイヤーと管理者の正規化済みメールが一致するときだけデバッグ利用を許可する", () => {
  assert.equal(playerEmailHasAdminDebugAccess(" Admin@example.com ", 100, ["admin@example.com"]), true);
  assert.equal(playerEmailHasAdminDebugAccess("Ａ@example.com", 100, ["a@example.com"]), true);
  assert.equal(playerEmailHasAdminDebugAccess("player@example.com", 100, ["admin@example.com"]), false);
  assert.equal(playerEmailHasAdminDebugAccess("admin@example.com", null, ["admin@example.com"]), false);
  assert.equal(playerEmailHasAdminDebugAccess(null, 100, ["admin@example.com"]), false);
});

test("管理画面からの個別付与はメール未登録でもデバッグ利用を許可する", () => {
  assert.equal(playerHasResolvedDebugAccess(null, null, [], true), true);
  assert.equal(playerHasResolvedDebugAccess("player@example.com", 100, ["admin@example.com"], false), false);
  assert.equal(playerHasResolvedDebugAccess("player@example.com", null, ["PLAYER@example.com"], false), false);
  assert.equal(playerHasResolvedDebugAccess("player@example.com", 100, ["PLAYER@example.com"], false), true);
});

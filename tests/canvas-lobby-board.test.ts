...
import { activeCanvasLobbyStrokes, canvasLobbyRetentionMs, removeCanvasLobbyAuthorStrokes } from "../lib/canvas-lobby-board.ts";
...
test("落書きボードの全消去は本人の線だけを取り除く", () => {
  const own = { ...stroke("own", 1), authorId: "player-1" };
  const other = { ...stroke("other", 1), authorId: "player-2" };
  assert.deepEqual(removeCanvasLobbyAuthorStrokes([own, other], "player-1").map((item) => item.id), ["other"]);
});

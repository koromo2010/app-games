...
test("platform adapterは認証identityを注入し、Redis CASで小規模ゲームを進行する", async () => {
  ...
  try {
    ...
    assert.deepEqual(
      created.view.common.players.map((roomPlayer) => roomPlayer.displayName),
      ["Host"],
    );
    ...
    const first = adapter.sendCommand({
      code: "RACE",
      envelope: { expectedRevision: 3, command: { type: "game/count-up" } },
    });
    identity = player;
    ...
    const stored = JSON.parse(storedRaw ?? "null") as {
      revision?: number;
      room?: {
        app?: {
          lastActorPlayerId?: string;
        };
      };
    };
    ...
    assert.ok([
      host.playerId,
      player.playerId,
    ].includes(stored.room?.app?.lastActorPlayerId ?? ""));

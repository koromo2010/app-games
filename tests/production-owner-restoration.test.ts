...
test("diagnostic UI projection fail-closes unknown, empty, malformed, HTTP, and transport responses", () => {
  for (const payload of [{ unknown: true }, null, { schemaVersion: 1 }]) {
    const result = projectCompletionDiagnosticResponse(200, payload);
    assert.deepEqual(result, { consumed: true, phase: "failure", code: "OWNER_RESTORATION_DIAGNOSTIC_UPSTREAM_RESPONSE_INVALID" });
  }
  assert.deepEqual(projectCompletionDiagnosticResponse(503, { error: "not-allowlisted" }), {
    consumed: true, phase: "failure", code: "OWNER_RESTORATION_DIAGNOSTIC_UPSTREAM_UNAVAILABLE",
  });
  assert.deepEqual(projectCompletionDiagnosticResponse(503, { error: "OWNER_RESTORATION_DIAGNOSTIC_REQUIRED_TABLE_UNAVAILABLE" }), {
    consumed: true, phase: "failure", code: "OWNER_RESTORATION_DIAGNOSTIC_REQUIRED_TABLE_UNAVAILABLE",
  });
...
test("completed-import diagnosis fails closed for malformed metadata and selector mismatch", async () => {
  ...
  const mismatch = projectCompletedProductionPrivateWorkspaceImportDiagnostic({
    ...
    diagnosticDatabaseContext: { ...diagnosticContext([]), selectedKey: "PLATFORM_DATABASE_URL" },

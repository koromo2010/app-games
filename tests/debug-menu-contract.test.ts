...
test("共通DEBUGメニューはゲーム操作を妨げない画面内ウィンドウとして表示する", () => {
  ...
  assert.match(windowSource, /persistentContent\?: ReactNode/);
  assert.match(windowSource, /data-debug-window-persistent/);
  assert.match(windowSource, /height: isMinimized \? "auto"/);

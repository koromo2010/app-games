...
test("ワードウルフの部屋操作はロビーと結果に一組ずつ表示する", () => {
  ...
  assert.ok(permissions.includes('canDissolve: isHost && Boolean(isLobby || isResult)'));
  assert.ok(lifecycleActions.includes('if (surface === "playing") return null;'));
  assert.ok(lifecycleActions.includes('if (surface === "lobby")'));

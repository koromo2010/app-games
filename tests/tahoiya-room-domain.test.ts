...
import { normalizeTahoiyaFakeDefinitions, normalizeTahoiyaFakeDefinitionsPerPlayer } from "../lib/tahoiya-definitions.ts";
...
test("1人2つの偽説明がそろうまで執筆完了にならず、両方を候補へ入れる", () => {
  const incomplete = room({ fakeDefinitionsPerPlayer: 2, fakeDefinitions: { writer: ["偽説明その1。", ""] } });
  assert.equal(writingComplete(incomplete), false);

  const complete = room({ fakeDefinitionsPerPlayer: 2, fakeDefinitions: { writer: ["偽説明その1。", "偽説明その2。"] } });
  assert.equal(writingComplete(complete), true);
  const fakeOptions = createDefinitionOptions(complete).filter((option) => !option.isReal);
  assert.equal(fakeOptions.length, 2);
  assert.deepEqual(new Set(fakeOptions.map((option) => option.authorId)), new Set(["writer"]));
});

test("複数の偽説明本文は作者本人だけに見せ、他プレイヤーには提出状態だけを返す", () => {
  const current = room({
    fakeDefinitionsPerPlayer: 2,
    fakeDefinitions: { writer: ["偽説明その1。", "偽説明その2。"] },
  });
  assert.deepEqual(sanitizeTahoiyaRoom(current, "writer").fakeDefinitions.writer, ["偽説明その1。", "偽説明その2。"]);
  assert.deepEqual(sanitizeTahoiyaRoom(current, "host").fakeDefinitions.writer, ["__submitted__", "__submitted__"]);
});

test("複数偽説明の未提出枠は時間切れで埋め、候補から除外する", () => {
  const current = room({
    fakeDefinitionsPerPlayer: 3,
    fakeDefinitions: { writer: ["提出済み。", "", ""] },
    phaseStartedAt: Date.now() - 10_000,
    actionTimeLimitSeconds: 1,
    playerTimeouts: {
      host: { consecutiveTimeouts: 0, reducedTime: false },
      writer: { consecutiveTimeouts: 0, reducedTime: false },
    },
  });
  const reconciled = reconcileProgress(current);
  assert.equal(reconciled.phase, "voting");
  assert.equal(reconciled.options.filter((option) => !option.isReal).length, 1);
  assert.equal(reconciled.options.find((option) => !option.isReal)?.text, "提出済み。");
});

test("旧ルームの1件文字列を1件配列へ移行し、個数設定は1を補う", () => {
  assert.equal(normalizeTahoiyaFakeDefinitionsPerPlayer(undefined), 1);
  assert.deepEqual(normalizeTahoiyaFakeDefinitions({ writer: "旧形式の偽説明。" }), { writer: ["旧形式の偽説明。"] });
});

...
test("時間切れ時は入力済みの連想語を残し、空欄だけ未提出にする", () => {
  assert.deepEqual(normalizeNigoichiTimeoutAssociations(["  海  辺  ", "", "  夏  "], 3), ["海 辺", "未提出", "夏"]);
});
...
test("連想語の時間切れは未提出を補い、予想フェーズの時計を開始する", () => {
  ...
  assert.equal(isNigoichiPhaseExpired(room, 65_999), false);
  assert.equal(isNigoichiPhaseExpired(room, 66_000), true);
  const expired = expireNigoichiPhase(room, 66_000);
  assert.equal(expired.phase, "guess");
  assert.equal(expired.phaseStartedAt, 66_000);

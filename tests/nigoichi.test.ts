...
test("全手札をMグループへ一度ずつ割り当て、各組2枚以上を要求する", () => {
  const hand = [0, 1, 2, 3, 4, 5];
  assert.equal(areValidNigoichiAssociationGroups(hand, [
    { id: "g1", clue: "前半", cardIds: [0, 1, 2] },
    { id: "g2", clue: "後半", cardIds: [3, 4, 5] },
  ], 2), true);
  assert.equal(areValidNigoichiAssociationGroups(hand, [
    { id: "g1", clue: "二枚", cardIds: [0, 1] },
    { id: "g2", clue: "四枚", cardIds: [2, 3, 4, 5] },
  ], 2), true);
  assert.equal(areValidNigoichiAssociationGroups(hand, [
    { id: "g1", clue: "一枚", cardIds: [0] },
    { id: "g2", clue: "五枚", cardIds: [1, 2, 3, 4, 5] },
  ], 2), false);
  assert.equal(areValidNigoichiAssociationGroups(hand, [
    { id: "g1", clue: "重複", cardIds: [0, 1, 2] },
    { id: "g2", clue: "重複", cardIds: [2, 3, 4, 5] },
  ], 2), false);
  assert.equal(areValidNigoichiAssociationGroups(hand, [
    { id: "g1", clue: "空欄でない", cardIds: [0, 1, 2] },
    { id: "g2", clue: "", cardIds: [3, 4, 5] },
  ], 2), false);
});

test("連想グループと予想は全参加者が提出したときだけ完了する", () => {
  const group = [{ id: "g", clue: "A", cardIds: [0, 1] }];
  assert.equal(allNigoichiAssociationsSubmitted({ players, associations: { a: group, b: group } }), false);
  assert.equal(allNigoichiAssociationsSubmitted({ players, associations: { a: group, b: group, c: group } }), true);
...
test("共有プレイログへ所有者・グループ・連想語を匿名化して記載する", () => {
  const text = nigoichiShareText({
    ...
    associations: {
      a: [{ id: "a1", clue: "ペット", cardIds: [0, 1] }],
      b: [{ id: "b1", clue: "生き物", cardIds: [2, 3] }],
      c: [{ id: "c1", clue: "夜空", cardIds: [4, 5] }],
    },
    guesses: { a: 6, b: 5, c: 6 },
    missingNumber: 6,
  });
  ...
  assert.match(text, /PLAYER1 1：1\.猫 ＋ 2\.犬 → ペット/);
  assert.match(text, /公開名 1：3\.鳥 ＋ 4\.魚 → 生き物/);
...
test("結果前は本人の手札と連想だけを返し、余り番号を隠す", () => {
  const room: NigoichiRoom = {
    ...
    gameNumber: 1, cardsPerPlayer: 2, associationWordCount: 1, wordDifficulty: "normal", debugMode: false, debugReplayEnabled: false,
    words: words.slice(0, 7), hands: { a: [0, 1], b: [2, 3], c: [4, 5] },
    associations: {
      a: [{ id: "a1", clue: "自分の語", cardIds: [0, 1] }],
      b: [{ id: "b1", clue: "他人の語", cardIds: [2, 3] }],
    }, guesses: { a: 6, b: 5 }, missingNumber: 6,
  ...
  assert.deepEqual(sanitized.associations, { a: [{ id: "a1", clue: "自分の語", cardIds: [0, 1] }] });
...
test("予想中は連想語を全員へ公開するがカードとの対応を隠す", () => {
  const room = {
    code: "AB12", revision: 1, hostId: "a", passphrase: "", phase: "guess" as const,
    players: players.map((player, index) => ({ ...player, name: player.id, joinedAt: index })), playerCapacity: 3,
    gameNumber: 1, cardsPerPlayer: 2, associationWordCount: 1, wordDifficulty: "normal" as const, debugMode: false, debugReplayEnabled: false,
    words: words.slice(0, 7), hands: { a: [0, 1] as const, b: [2, 3] as const, c: [4, 5] as const },
    associations: {
      a: [{ id: "a1", clue: "A", cardIds: [0, 1] }],
      b: [{ id: "b1", clue: "B", cardIds: [2, 3] }],
      c: [{ id: "c1", clue: "C", cardIds: [4, 5] }],
    }, guesses: {}, missingNumber: 6, debugLog: [], createdAt: 1, updatedAt: 1,
  } satisfies NigoichiRoom;
  const sanitized = sanitizeNigoichiRoomForPlayer(room, "b");
  assert.equal(sanitized.associations.a[0].clue, "A");
  assert.deepEqual(sanitized.associations.a[0].cardIds, []);
  assert.equal(sanitized.missingNumber, null);
});

test("デバッグホストは全手札・グループ・余り番号を確認できる", () => {
  const room = {
    code: "AB12", revision: 1, hostId: "a", passphrase: "", phase: "guess" as const,
    players: players.map((player, index) => ({ ...player, name: player.id, joinedAt: index })), playerCapacity: 3,
    gameNumber: 1, cardsPerPlayer: 2, associationWordCount: 1, wordDifficulty: "normal" as const, debugMode: true, debugReplayEnabled: false,
    words: words.slice(0, 7), hands: { a: [0, 1] as const, b: [2, 3] as const, c: [4, 5] as const },
    associations: {
      a: [{ id: "a1", clue: "A", cardIds: [0, 1] }],
      b: [{ id: "b1", clue: "B", cardIds: [2, 3] }],

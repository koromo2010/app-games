...
test("正解点P+1から自分のカードへの被投票数を減点する", () => {
  ...
  assert.deepEqual(scores, [
    { playerId: "a", isCorrect: true, correctBonus: 4, receivedWrongVotes: 1, roundScore: 3, totalScoreAfterRound: 13 },
  ...
  assert.deepEqual(maximumPenalty[0], {
    playerId: "a", isCorrect: true, correctBonus: 5, receivedWrongVotes: 3, roundScore: 2, totalScoreAfterRound: 2,
  });
});
...
test("自分のカードを回答候補から除外し、ラウンド得点とログを確定する", () => {
  const room = {
    ...
    gameNumber: 2, cardsPerPlayer: 2, associationWordCount: 1, wordDifficulty: "normal" as const, debugMode: false, debugReplayEnabled: false,
  ...
  assert.equal(finished.roundScores.a.roundScore, 3);
  assert.deepEqual(finished.totalScores, { a: 13, b: 4, c: 0 });
...
test("共有プレイログへ所有者・手札・連想語を匿名化して記載する", () => {
  const text = nigoichiShareText({
    ...
    guesses: { a: 6, b: 5, c: 6 },
    missingNumber: 6,
    totalScores: { a: 4, b: 0, c: 3 },
    roundScores: {
      a: { playerId: "a", isCorrect: true, correctBonus: 4, receivedWrongVotes: 0, roundScore: 4, totalScoreAfterRound: 4 },
      b: { playerId: "b", isCorrect: false, correctBonus: 0, receivedWrongVotes: 0, roundScore: 0, totalScoreAfterRound: 0 },
      c: { playerId: "c", isCorrect: true, correctBonus: 4, receivedWrongVotes: 1, roundScore: 3, totalScoreAfterRound: 3 },
    },
  });
  ...
  assert.match(text, /正解点\+4・被投票-1・ラウンド\+3・累計3/);
